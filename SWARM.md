...
- **Aantal (voor code): 2–4 workers is het optimum.** Veel subagents (10+) lonen alleen bij
  _breedte-onderzoek_, niet bij code. Code is sterk onderling afhankelijk; boven ~4 worktrees
...
- **Verificatiepoort (niet-onderhandelbaar)**: de lead draait `typecheck`+`lint`+`test`+`build` +`prettier --check .` ná integratie en **gooit zwakke output weg**. E2e draait automatisch
  in CI (Playwright met bundled Chromium, project `ci`). Nooit mergen op "ziet er goed uit".

## Werkwijze van de lead (per ronde)

1. `git fetch` + rebase op `main`.
...
**Mensenwerk (eenmalig):** voeg repo-secret **`CLAUDE_OAUTH_TOKEN`** toe (Settings → Secrets →
Actions). Draai daarna elke workflow één keer handmatig ("Run workflow") om te valideren, en houd
de eerste runs in de gaten (rate limits). Zonder dit secret draaien de workflows niet.
