...
- [x] (ZZP2-158) **L** Herplaatsing bij uitval (Zorgwerk/ZZP-Markt "SOS"): annulering van een ACTIVE samenwerking → dienst weer 'open' (CLOSED→PUBLISHED) + direct passende inzetbare ZZP'ers voorgesteld op de samenwerking-detailpagina (`ReplacementPanel` + `suggestedFreelancersForJob` + "Bericht sturen"), plus herplaatsings-notificatie aan de opdrachtgever. Pure `planReplacement` (alleen ACTIVE→CANCELLED; respecteert bewust gedepubliceerde DRAFT-dienst). De openstaande-factuur-veiligheidsrem in de cancel-actie bleef volledig intact (de herplaatsing draait er pas ná).
