...
2. **Eén fase per keer.** Begin geen nieuwe fase voor de vorige zijn *Definition of Done* (DoD) haalt én jij het hebt afgevinkt in `CURRENT_TASK.md`.
...
> **Coherentie-eis (hard):** voor *elke* actie die een rol kan doen, beschrijf en implementeer je expliciet welke vervolgactie(s) bij welke andere rol(len) ontstaan. Geen enkele knop is een doodlopend einde. Documenteer dit in `WORKFLOW_MAP.md` als event → effect-tabel.
...
- **Administratie ZZP'er:** factuur op "Uitstaand / Debiteuren"; te dragen **BTW (omzetbelasting)** geregistreerd; omzet als *nog te realiseren* gemarkeerd.
...
*DoD:* documenten staan er; gap-analyse helder; geen code gewijzigd zonder noodzaak.
...
*DoD:* events publiceren/consumeren werkt; ongeldige statusovergangen worden geweigerd; tests groen.
...
*DoD:* een proeftransactie genereert correcte administratie bij ZZP'er én opdrachtgever; BTW klopt; nummering uniek; geen geldverwerking aanwezig.
...
*DoD:* van contract-getekend t/m betaalregistratie verloopt de keten automatisch; beide administraties kloppen; integratietest dekt het hele pad; goedkeuringsstap werkt voor zowel uurtarief als milestone/fixed price.
...
*DoD:* elk zijpad getest en zichtbaar in de juiste rol-UI; DBA-signalen vuren op de drempels; geen enkele copy impliceert juridisch advies/garantie.
...
*DoD:* elke rol kan zijn volledige flow lopen; design tokens consistent; toegankelijkheid gecheckt.
...
*DoD:* reminders vuren op tijd; exports kloppen met de administratie.
...
*DoD:* E2E groen; geen openstaande TODO's in de happy path; verslag van bekende beperkingen.
