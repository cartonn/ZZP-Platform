...
test("mobile landing has no horizontal overflow and explains payments accessibly", async ({
  ...
  const question = page.locator("summary").filter({ hasText: "Hoe worden facturen betaald?" });
  await question.focus();
  await page.keyboard.press("Enter");
  await expect(page.getByText(/De opdrachtgever betaalt de zzp’er rechtstreeks/)).toBeVisible();
  await page.getByRole("link", { name: "Start als opdrachtgever", exact: true }).click();
  await expect(page).toHaveURL(/\/register$/);
});
