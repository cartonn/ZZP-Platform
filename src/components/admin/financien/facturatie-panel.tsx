...
} from "@/app/(protected)/admin/facturatie/actions";
...
/**
 * Platform-facturatie-paneel: facturen van het platform aan bemiddelingen (transactie-fee) en
 * ZZP'ers (abonnement). Laadt zelf zijn data; filtert op status. De filter-pills wijzen naar
 * `basePath` (mag al een query bevatten, bv. ?tab=facturatie) zodat het paneel zowel op
 * /admin/facturatie als binnen de Financiën-hub werkt. Rendert geen eigen paginakop.
 *
 * @param searchParams genormaliseerde searchParams uit de host (status/gegenereerd).
 * @param basePath     pad waarheen de filter-links wijzen; mag al een query bevatten.
 */
export async function FacturatiePanel({
  searchParams,
  basePath,
}: {
  ...
  const gegenereerd = first(searchParams.gegenereerd);
  const statusParam = first(searchParams.status);
  const generated = gegenereerd !== "" ? Number(gegenereerd) : null;
  const now = new Date();
    ...
    <div className="space-y-6">
      ...
      {invoices.length === 0 ? (
        ...
        <div className="space-y-3">
          {/* Filter-tabs — wijzen naar basePath zodat ze binnen de hub-tab blijven. */}
          <div className="flex flex-wrap gap-1.5">
            {FILTER_TABS.map((tab) => {
              ...
              return (
                <Link
                  key={tab.label}
                  href={tab.value ? withParams(basePath, { status: tab.value }) : basePath}
