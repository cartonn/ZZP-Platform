...
// Concept 469 — "Vlechtwerk" · gevlochten mandenmaken, ambachtelijk interlace.
// Warme rieten/hennep-tinten (#c99a5b riet, #a8946f hennep, #8a6a3c donker vlechtwerk) op een
// warme crème grond (#f4ece0). Over-onder-weefstructuur als dividers en accenten, tactiel en
// handgemaakt, nooit klinisch. Serif-display voor het ambacht, humanist sans voor de body.
...
// — Palet: warme crème grond + rieten/hennep vlechttinten —
const C = {
  bg: "#f4ece0",
  bgDeep: "#ece0cf",
  panel: "#fbf6ee",
  ...
  inkMute: "#8a755a",
  inkFaint: "#a99377",
  line: "rgba(138,106,60,0.24)",
  lineSoft: "rgba(138,106,60,0.12)",
  // vlecht-tinten
  reed: "#c99a5b",
  reedLo: "#a97e3f",
  reedHi: "#dcb377",
  reedWash: "rgba(201,154,91,0.16)",
  hemp: "#a8946f",
  hempHi: "#c2ad84",
  hempWash: "rgba(168,148,111,0.16)",
  bark: "#8a6a3c",
  barkHi: "#a37f4c",
  // status
  ok: "#5f8a4c",
  okInk: "#3f6a31",
  okWash: "rgba(95,138,76,0.16)",
  warn: "#c68a2e",
  warnInk: "#96631a",
  warnWash: "rgba(198,138,46,0.18)",
  info: "#6f8a7a",
  infoInk: "#4c6a5a",
  infoWash: "rgba(111,138,122,0.16)",
  bad: "#b8543f",
  badInk: "#8f3d2c",
  badWash: "rgba(184,84,63,0.16)",
};
...
// Weefsel-textuur: gekruiste rieten strengen die als een gevlochten mand over-onder lopen.
function weave(base: string): React.CSSProperties {
  return {
    backgroundColor: base,
    backgroundImage:
      "repeating-linear-gradient(45deg, rgba(138,106,60,0.06) 0px, rgba(138,106,60,0.06) 6px, transparent 6px, transparent 12px)," +
      "repeating-linear-gradient(-45deg, rgba(201,154,91,0.05) 0px, rgba(201,154,91,0.05) 6px, transparent 6px, transparent 12px)",
    backgroundSize: "17px 17px, 17px 17px",
  };
}

// — Vlecht-divider: twee strengen die over en onder elkaar heen wippen (over-onder braid) —
function BraidDivider({ tone = C.reed }: { tone?: string }) {
  return (
    <svg
      width="100%"
      height="14"
    ...
    >
      <path
        d="M0 7 C 25 1 45 1 70 7 S 115 13 140 7 S 185 1 210 7 S 255 13 280 7 S 325 1 350 7 S 385 10 400 7"
        fill="none"
        stroke={tone}
        strokeWidth="2.2"
        strokeLinecap="round"
      ...
      />
      <path
        d="M0 7 C 25 13 45 13 70 7 S 115 1 140 7 S 185 13 210 7 S 255 1 280 7 S 325 13 350 7 S 385 4 400 7"
        fill="none"
        stroke={C.hemp}
        strokeWidth="2.2"
        strokeLinecap="round"
...
// — Vlecht-medaillon: gekruiste rieten strengen in een cirkel (mandbodem-motief) —
function WeaveMark({ size = 40, tone = C.reed, id }: { size?: number; tone?: string; id: string }) {
  const c = size / 2;
  const strands = 4;
  return (
    <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} aria-hidden="true">
      <clipPath id={`wm-${id}`}>
        <circle cx={c} cy={c} r={size / 2 - 1} />
      </clipPath>
      <g clipPath={`url(#wm-${id})`}>
        {Array.from({ length: strands }).map((_, i) => {
          const off = ((i + 0.5) / strands) * size;
          return (
            <line
              key={`a-${id}-${i}`}
              x1={off}
      ...
      <circle cx={c} cy={c} r={size / 2 - 1} fill="none" stroke={tone} strokeWidth="1.2" />
    </svg>
  );
}
...
function Panel({
  ...
  className?: string;
  as?: "div" | "section" | "li";
  tint?: "panel" | "reed" | "hemp";
}) {
  const grond = { panel: C.panel, reed: "#f7ecd8", hemp: "#f1ece0" }[tint];
  const bd = { panel: C.line, reed: C.reed, hemp: C.hemp }[tint];
  return (
    <Tag
      ...
      style={{
        ...weave(grond),
        border: `1px solid ${bd}`,
        boxShadow: "0 1px 0 rgba(255,255,255,0.6) inset, 0 8px 22px rgba(138,106,60,0.10)",
        color: C.ink,
      }}
    >
      {children}
...
function Eyebrow({ children, tone = C.bark }: { children: React.ReactNode; tone?: string }) {
  return (
    <p
...
function PrimaryButton({
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className={`group inline-flex items-center justify-center gap-2 rounded-full px-5 py-2.5 text-[13px] font-semibold transition-all duration-200 hover:brightness-[1.05] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#8a6a3c] focus-visible:ring-offset-2 focus-visible:ring-offset-[#f4ece0] active:translate-y-px motion-reduce:transition-none ${className}`}
      style={{
        color: "#fbf6ee",
        backgroundImage: `linear-gradient(135deg, ${C.reedHi}, ${C.reed} 45%, ${C.reedLo})`,
        border: `1px solid ${C.reedLo}`,
        boxShadow: "0 1px 0 rgba(255,255,255,0.35) inset, 0 2px 6px rgba(138,106,60,0.25)",
        ...bodyFont,
      }}
    >
      {children}
...
function GhostButton({
  ...
  return (
    <button
      ...
      className={`inline-flex items-center justify-center gap-2 rounded-full px-4 py-2.5 text-[12.5px] font-semibold transition-all duration-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#8a6a3c] focus-visible:ring-offset-2 focus-visible:ring-offset-[#f4ece0] motion-reduce:transition-none ${className}`}
      style={{
        color: active ? "#fbf6ee" : C.inkSoft,
        background: active ? C.hemp : C.panelSoft,
        border: `1px solid ${active ? C.hemp : C.line}`,
        ...bodyFont,
      }}
    >
      {children}
...
// — Sparkline in rieten tint met warme vulling —
function WeaveLine({ data, tone, id }: { data: number[]; tone: string; id: string }) {
  ...
  return (
    ...
    >
      <defs>
        <linearGradient id={`vl-${id}`} x1="0" y1="0" x2="0" y2="1">
          <stop offset="0%" stopColor={tone} stopOpacity="0.26" />
          <stop offset="100%" stopColor={tone} stopOpacity="0" />
        </linearGradient>
      </defs>
      <polygon points={`0,${h} ${line} ${w},${h}`} fill={`url(#vl-${id})`} />
      <polyline
        points={line}
        fill="none"
        stroke={tone}
      ...
      />
      <circle cx={last[0]} cy={last[1]} r="2.6" fill={C.panel} stroke={tone} strokeWidth="1.6" />
    </svg>
  );
}

function Meter({ value, tone = C.reed }: { value: number; tone?: string }) {
  return (
    <span className="hidden items-center gap-2.5 sm:flex" aria-hidden="true">
      ...
      >
        <span
          ...
          style={{
            width: `${value}%`,
            backgroundImage: `linear-gradient(90deg, ${C.reedLo}, ${tone}, ${C.reedHi})`,
            transition: "width 0.6s cubic-bezier(0.22,1,0.36,1)",
          }}
        />
      </span>
      <span className="text-[12.5px] font-semibold" style={{ color: C.bark, ...num }}>
        {value}%
      </span>
    </span>
  );
...
export function Concept469() {
  ...
  return (
    <div
      ...
      style={{ ...bodyFont, color: C.ink, ...weave(C.bg) }}
    >
      <style>{`
        @keyframes weaveRise { from { opacity: 0; transform: translateY(8px); } to { opacity: 1; transform: translateY(0); } }
        .weave-rise { animation: weaveRise 0.5s cubic-bezier(0.22, 1, 0.36, 1) both; }
        @media (prefers-reduced-motion: reduce) { .weave-rise { animation: none !important; } }
      `}</style>

      <div className="relative mx-auto max-w-6xl px-4 pb-24 sm:px-6 md:px-8">
        <TopBar />
        <NavBar screen={screen} setScreen={setScreen} />
        <main key={screen} className="weave-rise pt-7">
          {screen === "dashboard" && (
    ...
    <header className="flex items-center justify-between gap-4 pt-8">
      <div className="flex items-center gap-3.5">
        <span
          ...
          style={{ background: "#f7ecd8", border: `1px solid ${C.reed}` }}
          aria-hidden="true"
        >
          <WeaveMark size={34} tone={C.reed} id="logo" />
        </span>
        <div>
          <p
          ...
          <p className="mt-1.5 text-[11px] leading-none" style={{ color: C.inkMute, ...bodyFont }}>
            {PROFIEL.plaats} · gevlochten ambacht
          </p>
        </div>
      </div>
      <div className="flex items-center gap-3">
        ...
        >
          ...
          {ongelezen > 0 && (
            <span
              ...
              style={{ background: C.reed, color: "#fbf6ee", ...num }}
              aria-hidden="true"
            >
              {ongelezen}
            </span>
        ...
        <span
          ...
          style={{
            background: C.reedWash,
            border: `1px solid ${C.reed}`,
            color: C.bark,
            ...bodyFont,
          }}
          aria-hidden="true"
        ...
        {SCREENS.map((s) => {
          ...
          return (
            <button
              ...
              className="flex shrink-0 items-center gap-2 rounded-full px-4 py-2 text-[12.5px] font-semibold transition-all duration-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#8a6a3c] focus-visible:ring-offset-2 focus-visible:ring-offset-[#f5ecdd] motion-reduce:transition-none"
              style={{
                color: on ? "#fbf6ee" : C.inkMute,
                backgroundImage: on
                  ? `linear-gradient(135deg, ${C.reedHi}, ${C.reed} 55%, ${C.reedLo})`
                  : "none",
                ...bodyFont,
              }}
            >
  ...
  return (
    <div className="space-y-7">
      <section className="grid grid-cols-1 gap-5 lg:grid-cols-[1.4fr_1fr]">
        <Panel className="p-7 md:p-9" tint="reed">
          <div className="relative flex items-start justify-between gap-4">
            <div>
              <Eyebrow>Vandaag · gevlochten</Eyebrow>
              <h1
            ...
            <WeaveMark size={56} tone={C.reed} id="d1" />
          </div>
          <p
            className="relative mt-3 max-w-md text-[14px] leading-relaxed"
            style={{ color: C.inkSoft }}
          >
            Je praktijk als gevlochten mand: elke streng verifieerbaar, warm en stevig verweven.
            Loop langs je acties — alles ligt strak in het vlechtwerk.
          </p>
          <BraidDivider />
      ...
      <section>
        <div className="mb-4">
          <Eyebrow tone={C.barkHi}>Strengen · deze maand</Eyebrow>
        </div>
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {KPIS.map((k, i) => (
            <Panel key={k.label} className="p-5">
              ...
              <div className="mt-3">
                <WeaveLine data={k.spark} tone={k.up ? C.reed : C.hemp} id={`k459-${i}`} />
              </div>
            </Panel>
          ))}
        </div>
      ...
      <section className="grid grid-cols-1 gap-5 lg:grid-cols-[1.45fr_1fr]">
        <div>
          <div className="mb-4 flex items-baseline justify-between">
            ...
            <button
              type="button"
              onClick={onOpen}
              className="rounded text-[11px] font-semibold uppercase tracking-[0.14em] transition-colors hover:brightness-110 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#8a6a3c] focus-visible:ring-offset-2 focus-visible:ring-offset-[#f4ece0]"
              style={{ color: C.bark, ...bodyFont }}
            >
              Alle →
            </button>
          </div>
              ...
              {OPDRACHTEN.map((o, i) => (
                <li key={o.id} style={{ borderTop: i === 0 ? "none" : `1px solid ${C.lineSoft}` }}>
                  <button
                    type="button"
                    onClick={onOpen}
                    className="group grid w-full grid-cols-[auto_1fr_auto] items-center gap-4 px-5 py-4 text-left transition-colors hover:bg-[#f5ecdd] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-[#8a6a3c] motion-reduce:transition-none"
                  >
                    <span
                      className="inline-flex h-11 w-11 items-center justify-center rounded-full"
                      style={{
                        background: i === 0 ? C.reedWash : C.raise,
                        border: `1px solid ${i === 0 ? C.reed : C.line}`,
                      }}
                    >
                      <span
                        className="text-[12px] font-bold leading-none"
                        style={{ color: i === 0 ? C.bark : C.inkMute, ...num }}
                      >
                        {o.match}
                      </span>
                    </span>
                    ...
                    <span className="flex items-center gap-3">
                      <Meter value={o.match} tone={o.match >= 90 ? C.reed : C.hemp} />
                      <ChevronRight
                        size={17}
                        aria-hidden="true"
                        className="transition-transform group-hover:translate-x-0.5 motion-reduce:transition-none"
        ...
        <div>
          <div className="mb-4">
            <Eyebrow tone={C.hempHi}>Certificaten</Eyebrow>
          </div>
          <Panel className="p-5">
  ...
  return (
    <div className="space-y-7">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
        ...
        >
          ...
          <input
            ...
            className="w-full bg-transparent text-[13px] outline-none placeholder:text-[#a99377]"
            style={{ color: C.ink, ...bodyFont }}
          />
        </div>
      ...
      {loading ? (
        ...
        <Panel className="p-6" tint="hemp">
          <div className="flex flex-col items-center py-14 text-center">
            <span
              className="inline-flex h-16 w-16 items-center justify-center rounded-full"
              style={{ background: C.panel, border: `1px solid ${C.hemp}`, color: C.hempHi }}
              aria-hidden="true"
            >
              <Sprout size={26} />
            </span>
...
function MarktKaart({
  ...
  const tone = strong ? C.reed : C.hemp;
  const toneHi = strong ? C.bark : C.hempHi;
  return (
    <Panel className="p-6" tint={strong ? "reed" : "panel"}>
      <div className="grid grid-cols-[1fr_auto] items-start gap-4">
        ...
        <div className="flex flex-col items-center gap-1.5">
          <span
            ...
            style={{ background: strong ? C.reedWash : C.hempWash, border: `1.5px solid ${tone}` }}
          >
            <span className="absolute inset-0 flex items-center justify-center opacity-40">
              <WeaveMark size={58} tone={tone} id={`m${index}`} />
            </span>
      ...
      <div className="mt-4 flex items-center gap-3">
        <button
          ...
          onClick={() => setOpen((v) => !v)}
          aria-expanded={open}
          className="inline-flex items-center gap-1.5 rounded-full px-3.5 py-1.5 text-[11.5px] font-semibold transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#8a6a3c] focus-visible:ring-offset-2 focus-visible:ring-offset-[#fbf6ee]"
  ...
  return (
    <div className="space-y-6">
      <button
        type="button"
        onClick={onBack}
        className="inline-flex items-center gap-2 rounded-full px-4 py-2 text-[12.5px] font-semibold transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#8a6a3c] focus-visible:ring-offset-2 focus-visible:ring-offset-[#f4ece0]"
        style={{
          color: C.inkSoft,
      ...
      <Panel className="p-7 md:p-9" tint="reed">
        <div className="relative flex flex-wrap items-center gap-2">
          ...
          <span
            ...
            style={{
              color: "#fbf6ee",
              backgroundImage: `linear-gradient(135deg, ${C.reedHi}, ${C.reed})`,
              ...bodyFont,
            }}
          >
...
function Verificatie() {
  ...
  return (
    <div className="space-y-6">
            ...
            return (
              ...
              >
                <button
                  ...
                  className="grid w-full grid-cols-[1fr_auto] items-center gap-4 px-6 py-4 text-left transition-colors hover:bg-[#f5ecdd] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-[#8a6a3c] motion-reduce:transition-none sm:grid-cols-[1fr_11rem_2.5rem]"
                >
      ...
      <div>
        <div className="mb-4">
          <Eyebrow tone={C.hempHi}>Documentenkast</Eyebrow>
        </div>
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-2 max-w-md text-[13.5px]" style={{ color: C.inkSoft }}>
          Rustig van boven naar beneden — zo blijf je verifieerbaar en betaald, strak verweven.
        </p>
      </div>
      ...
      <ol className="space-y-3">
        {ACTIES.map((a, i) => {
          ...
          const tone = warn ? C.warn : C.reed;
          const ink = warn ? C.warnInk : C.bark;
          const wash = warn ? C.warnWash : C.reedWash;
          return (
            <li key={a.titel}>
              <Panel className="p-6">
        ...
        <ul>
          {FACTUREN.map((f, i) => {
            ...
            return (
              <li
                key={f.nr}
                className="grid grid-cols-[1fr_auto] items-center gap-3 px-6 py-4 transition-colors hover:bg-[#f5ecdd] sm:grid-cols-[8rem_1fr_5rem_9rem_6rem] sm:gap-4"
                style={{ borderTop: i === 0 ? "none" : `1px solid ${C.lineSoft}` }}
              >
                <span
