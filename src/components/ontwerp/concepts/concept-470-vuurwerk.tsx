...
// Concept 470 — "Vuurwerk" · vuurwerk tegen de nachthemel, feestelijk en energiek.
// Diep nachtblauw-zwart (#070a18) met radiale kleur-bursts (goud #f5c542, magenta #ff4d9d,
// cyaan #46e0ff, koraal #ff7a59) als accenten en sprankelende highlight-punten. Premium-dark,
// vrolijk. Sans-display met strakke tracking, mono voor cijfers. Sprankel-beweging subtiel;
...
// — Palet: diepe nachthemel + feestelijke burst-kleuren —
const C = {
  bg: "#070a18",
  bgDeep: "#04060f",
  panel: "#0e1327",
  ...
  inkMute: "#8b95bf",
  inkFaint: "#5f6997",
  line: "rgba(139,149,191,0.24)",
  lineSoft: "rgba(139,149,191,0.12)",
  // burst-kleuren
  gold: "#f5c542",
  goldWash: "rgba(245,197,66,0.16)",
  magenta: "#ff4d9d",
  magentaWash: "rgba(255,77,157,0.16)",
  cyan: "#46e0ff",
  cyanWash: "rgba(70,224,255,0.16)",
  coral: "#ff7a59",
  coralWash: "rgba(255,122,89,0.16)",
  violet: "#b98cff",
  violetWash: "rgba(185,140,255,0.16)",
  // status
  ok: "#3ddc97",
  okInk: "#7cf0bf",
  okWash: "rgba(61,220,151,0.16)",
  warn: "#f5c542",
  warnInk: "#ffd970",
  warnWash: "rgba(245,197,66,0.16)",
  info: "#46e0ff",
  infoInk: "#8eecff",
  infoWash: "rgba(70,224,255,0.16)",
  bad: "#ff6b6b",
  badInk: "#ff9a9a",
  badWash: "rgba(255,107,107,0.16)",
};

const display = {
  fontFamily: "'Inter', 'Segoe UI', system-ui, -apple-system, sans-serif",
};
const bodyFont = display;
const num = {
  fontFamily: "ui-monospace, 'SF Mono', SFMono-Regular, Menlo, monospace",
  fontVariantNumeric: "tabular-nums" as const,
};

// Nachthemel-textuur: diepe radiale gloed onderaan + fijne sterrenkorrel.
function sky(base: string): React.CSSProperties {
  return {
    backgroundColor: base,
    backgroundImage:
      "radial-gradient(120% 80% at 50% 120%, rgba(70,224,255,0.05), transparent 60%)," +
      "radial-gradient(90% 60% at 15% -10%, rgba(255,77,157,0.05), transparent 55%)," +
      "radial-gradient(90% 60% at 85% -10%, rgba(245,197,66,0.05), transparent 55%)",
  };
}

// — Burst: een radiale vuurwerk-explosie met sprankels rond een kern —
function Burst({ size = 40, tone = C.gold, id }: { size?: number; tone?: string; id: string }) {
  const c = size / 2;
  const rays = 12;
  const r0 = size * 0.16;
  const r1 = size * 0.46;
  return (
    ...
    >
      <defs>
        <radialGradient id={`brc-${id}`}>
          <stop offset="0%" stopColor={tone} stopOpacity="0.9" />
          <stop offset="100%" stopColor={tone} stopOpacity="0" />
        </radialGradient>
      </defs>
      <circle cx={c} cy={c} r={r1} fill={`url(#brc-${id})`} opacity="0.35" />
        ...
        return (
          <g key={`${id}-${i}`}>
            <line
              ...
              opacity="0.85"
            />
            <circle cx={x1} cy={y1} r="1.4" fill={tone} />
          </g>
        );
      })}
      <circle cx={c} cy={c} r={size * 0.07} fill="#fff" opacity="0.9" />
    </svg>
  );
}
...
function Panel({
  ...
  return (
    <Tag
      ...
      style={{
        ...sky(C.panel),
        border: `1px solid ${glow ? glow : C.line}`,
        boxShadow: glow
          ? `0 0 0 1px ${glow}22, 0 10px 30px rgba(0,0,0,0.5)`
...
function PrimaryButton({
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className={`group inline-flex items-center justify-center gap-2 rounded-full px-5 py-2.5 text-[13px] font-semibold transition-all duration-200 hover:brightness-[1.08] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#46e0ff] focus-visible:ring-offset-2 focus-visible:ring-offset-[#070a18] active:translate-y-px motion-reduce:transition-none ${className}`}
      style={{
        color: "#070a18",
        backgroundImage: `linear-gradient(120deg, ${C.cyan}, ${C.violet} 55%, ${C.magenta})`,
        boxShadow: "0 0 18px rgba(70,224,255,0.35), 0 2px 6px rgba(0,0,0,0.4)",
        ...bodyFont,
      }}
    >
      {children}
...
function GhostButton({
  ...
  return (
    <button
      ...
      className={`inline-flex items-center justify-center gap-2 rounded-full px-4 py-2.5 text-[12.5px] font-semibold transition-all duration-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#46e0ff] focus-visible:ring-offset-2 focus-visible:ring-offset-[#070a18] motion-reduce:transition-none ${className}`}
      style={{
        color: active ? "#070a18" : C.inkSoft,
        background: active ? C.cyan : C.panelSoft,
...
// — Sparkline als een lichtspoor met gloeiende kop —
function TrailLine({ data, tone, id }: { data: number[]; tone: string; id: string }) {
  ...
  return (
    ...
    >
      ...
      />
      <circle cx={last[0]} cy={last[1]} r="3.4" fill={tone} opacity="0.35" />
    ...
    <span className="hidden items-center gap-2.5 sm:flex" aria-hidden="true">
      ...
      >
        <span
          ...
          style={{
            width: `${value}%`,
            backgroundImage: `linear-gradient(90deg, ${C.violet}, ${tone})`,
...
export function Concept470() {
  ...
  return (
    <div
      ...
      style={{ ...bodyFont, color: C.ink, ...sky(C.bg) }}
    >
      <style>{`
        @keyframes fwRise { from { opacity: 0; transform: translateY(8px); } to { opacity: 1; transform: translateY(0); } }
        .fw-rise { animation: fwRise 0.5s cubic-bezier(0.22, 1, 0.36, 1) both; }
        @keyframes fwTwinkle { 0%, 100% { opacity: 0.25; transform: scale(0.85); } 50% { opacity: 1; transform: scale(1.15); } }
        .fw-spark { animation: fwTwinkle 3.6s ease-in-out infinite; }
        @media (prefers-reduced-motion: reduce) {
          .fw-rise { animation: none !important; }
          .fw-spark { animation: none !important; opacity: 0.7 !important; }
        }
      `}</style>

      {/* Decoratieve sprankels tegen de nachthemel */}
      <div className="pointer-events-none absolute inset-0" aria-hidden="true">
        {[
          { l: "12%", t: "14%", tone: C.gold, d: "0s" },
          { l: "82%", t: "10%", tone: C.magenta, d: "1.1s" },
          { l: "68%", t: "26%", tone: C.cyan, d: "2s" },
          { l: "30%", t: "8%", tone: C.violet, d: "0.6s" },
          { l: "92%", t: "32%", tone: C.coral, d: "1.6s" },
        ].map((s, i) => (
          <span
            key={i}
            className="fw-spark absolute"
            style={{ left: s.l, top: s.t, animationDelay: s.d }}
          >
            <Sparkle size={14} style={{ color: s.tone }} />
          </span>
        ))}
      </div>
      ...
      <div className="relative mx-auto max-w-6xl px-4 pb-24 sm:px-6 md:px-8">
        ...
        <main key={screen} className="fw-rise pt-7">
          {screen === "dashboard" && (
    ...
    <header className="flex items-center justify-between gap-4 pt-8">
      <div className="flex items-center gap-3.5">
        <span
          ...
          style={{ background: C.bgDeep, border: `1px solid ${C.cyan}` }}
          aria-hidden="true"
        >
          <Burst size={38} tone={C.cyan} id="logo" />
        </span>
        <div>
          <p
        ...
        >
          ...
          {ongelezen > 0 && (
            <span
              ...
              style={{ background: C.magenta, color: "#fff", ...num }}
              aria-hidden="true"
            >
              {ongelezen}
            </span>
        ...
        {SCREENS.map((s) => {
          ...
          return (
            <button
              ...
              className="flex shrink-0 items-center gap-2 rounded-full px-4 py-2 text-[12.5px] font-semibold transition-all duration-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#46e0ff] focus-visible:ring-offset-2 focus-visible:ring-offset-[#131a34] motion-reduce:transition-none"
              style={{
                color: on ? "#070a18" : C.inkMute,
                backgroundImage: on
                  ? `linear-gradient(120deg, ${C.cyan}, ${C.violet} 60%, ${C.magenta})`
                  : "none",
                boxShadow: on ? "0 0 14px rgba(70,224,255,0.3)" : "none",
                ...bodyFont,
              }}
            >
              {s.label}
...
function Dashboard({ onOpen, onActies }: { onOpen: () => void; onActies: () => void }) {
  ...
  const bursts = [C.gold, C.magenta, C.cyan, C.coral];
  return (
    <div className="space-y-7">
      <section className="grid grid-cols-1 gap-5 lg:grid-cols-[1.4fr_1fr]">
        <Panel className="p-7 md:p-9" glow={C.cyan}>
          <div className="relative flex items-start justify-between gap-4">
            <div>
              <Eyebrow>Vandaag · ontstoken</Eyebrow>
              <h1
            ...
            <Burst size={58} tone={C.gold} id="d1" />
          </div>
          <p
            className="relative mt-3 max-w-md text-[14px] leading-relaxed"
            style={{ color: C.inkSoft }}
          >
            Je praktijk als vuurwerk tegen de nachthemel: elke prestatie een lichtpunt, elk
            certificaat geverifieerd. Loop langs je acties — vandaag knalt het.
          </p>
      ...
      <section>
        <div className="mb-4">
          <Eyebrow tone={C.violet}>Bursts · deze maand</Eyebrow>
        </div>
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {KPIS.map((k, i) => {
            const tone = bursts[i % bursts.length]!;
            return (
              <Panel key={k.label} className="p-5" glow={tone}>
                ...
                <p
                  className="mt-3 text-[27px] font-semibold leading-none tracking-[-0.02em]"
                  style={{ color: C.ink, ...num }}
                >
                  {k.value}
                </p>
                <div className="mt-3">
                  <TrailLine data={k.spark} tone={tone} id={`k460-${i}`} />
                </div>
              </Panel>
            );
          })}
      ...
      <section className="grid grid-cols-1 gap-5 lg:grid-cols-[1.45fr_1fr]">
        <div>
          <div className="mb-4 flex items-baseline justify-between">
            ...
            <button
              type="button"
              onClick={onOpen}
              className="rounded text-[11px] font-semibold uppercase tracking-[0.14em] transition-colors hover:brightness-110 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#46e0ff] focus-visible:ring-offset-2 focus-visible:ring-offset-[#070a18]"
              ...
              {OPDRACHTEN.map((o, i) => (
                <li key={o.id} style={{ borderTop: i === 0 ? "none" : `1px solid ${C.lineSoft}` }}>
                  <button
                    type="button"
                    onClick={onOpen}
                    className="group grid w-full grid-cols-[auto_1fr_auto] items-center gap-4 px-5 py-4 text-left transition-colors hover:bg-[#131a34] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-[#46e0ff] motion-reduce:transition-none"
                  >
                    <span
                    ...
                    >
                      <span
                        ...
                        style={{ color: i === 0 ? C.cyan : C.inkMute, ...num }}
                      >
                        {o.match}
                      </span>
                    </span>
                    ...
                    <span className="flex items-center gap-3">
                      <Meter value={o.match} tone={o.match >= 90 ? C.cyan : C.violet} />
                      <ChevronRight
                        size={17}
                        aria-hidden="true"
                        className="transition-transform group-hover:translate-x-0.5 motion-reduce:transition-none"
        ...
        <div>
          <div className="mb-4">
            <Eyebrow tone={C.magenta}>Certificaten</Eyebrow>
          </div>
          <Panel className="p-5">
  ...
  return (
    <div className="space-y-7">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
        ...
        >
          ...
          <input
            ...
            className="w-full bg-transparent text-[13px] outline-none placeholder:text-[#5f6997]"
            style={{ color: C.ink, ...bodyFont }}
          />
        </div>
      ...
      {loading ? (
        ...
        <Panel className="p-6" glow={C.violet}>
          <div className="flex flex-col items-center py-14 text-center">
            <span
              className="inline-flex h-16 w-16 items-center justify-center rounded-full"
              style={{ background: C.bgDeep, border: `1px solid ${C.violet}`, color: C.violet }}
              aria-hidden="true"
            >
              <Sparkles size={26} />
            </span>
    ...
    <Panel className="p-6" glow={strong ? C.cyan : undefined}>
      <div className="grid grid-cols-[1fr_auto] items-start gap-4">
        ...
        <div className="flex flex-col items-center gap-1.5">
          <span
            ...
            style={{
              background: strong ? C.cyanWash : C.violetWash,
              border: `1.5px solid ${tone}`,
            }}
          >
            <span className="absolute inset-0 flex items-center justify-center opacity-45">
              <Burst size={62} tone={tone} id={`m${index}`} />
            </span>
      ...
      <div className="mt-4 flex items-center gap-3">
        <button
          ...
          onClick={() => setOpen((v) => !v)}
          aria-expanded={open}
          className="inline-flex items-center gap-1.5 rounded-full px-3.5 py-1.5 text-[11.5px] font-semibold transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#46e0ff] focus-visible:ring-offset-2 focus-visible:ring-offset-[#0e1327]"
          style={{ color: tone, border: `1px solid ${C.line}`, ...bodyFont }}
        >
  ...
  return (
    <div className="space-y-6">
      <button
        type="button"
        onClick={onBack}
        className="inline-flex items-center gap-2 rounded-full px-4 py-2 text-[12.5px] font-semibold transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#46e0ff] focus-visible:ring-offset-2 focus-visible:ring-offset-[#070a18]"
        style={{
          color: C.inkSoft,
      ...
      <Panel className="p-7 md:p-9" glow={C.cyan}>
        <div className="relative flex flex-wrap items-center gap-2">
          ...
          <span
            ...
            style={{
              color: "#070a18",
              backgroundImage: `linear-gradient(120deg, ${C.cyan}, ${C.violet})`,
              ...bodyFont,
            }}
          >
...
function Verificatie() {
  ...
  return (
    <div className="space-y-6">
      <Panel className="p-7 md:p-9" glow={C.cyan}>
        <div className="relative flex flex-wrap items-center justify-between gap-6">
          ...
          <span
            ...
            style={{ background: C.bgDeep, border: `1.5px solid ${C.cyan}` }}
          >
            <span className="absolute inset-0 flex items-center justify-center opacity-55">
              <Burst size={92} tone={C.cyan} id="vratio" />
            </span>
            <span className="relative flex flex-col items-center">
              <span
            ...
            return (
              ...
              >
                <button
                  ...
                  className="grid w-full grid-cols-[1fr_auto] items-center gap-4 px-6 py-4 text-left transition-colors hover:bg-[#131a34] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-[#46e0ff] motion-reduce:transition-none sm:grid-cols-[1fr_11rem_2.5rem]"
                >
      ...
      <div>
        <div className="mb-4">
          <Eyebrow tone={C.magenta}>Documentenkast</Eyebrow>
        </div>
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-2 max-w-md text-[13.5px]" style={{ color: C.inkSoft }}>
          Rustig van boven naar beneden — zo blijf je verifieerbaar en betaald, elk lichtpunt op
          orde.
        </p>
      </div>
      ...
      <ol className="space-y-3">
        {ACTIES.map((a, i) => {
          ...
          const ink = warn ? C.warnInk : C.cyan;
    ...
    <div className="space-y-6">
      ...
      <section className="grid grid-cols-1 gap-4 sm:grid-cols-3">
        {[
          ...
          { l: "Concept", v: "€ 880", sub: "klaar om te versturen", alarm: false, glow: C.violet },
        ].map((s) => (
        ...
        <ul>
          {FACTUREN.map((f, i) => {
            ...
            return (
              <li
                key={f.nr}
                className="grid grid-cols-[1fr_auto] items-center gap-3 px-6 py-4 transition-colors hover:bg-[#131a34] sm:grid-cols-[8rem_1fr_5rem_9rem_6rem] sm:gap-4"
                style={{ borderTop: i === 0 ? "none" : `1px solid ${C.lineSoft}` }}
              >
                <span
