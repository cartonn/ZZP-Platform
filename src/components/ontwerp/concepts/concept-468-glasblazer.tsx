...
// Concept 468 — "Glasblazer" · glasblazen bij de oven.
// Donker-warme grond (#161110) met gloeiend smelt-licht: amber #ffab45, ember-oranje #ff7a2e,
// hete kern #ffe7bf. Translucente glas-panelen met zachte gloed, refractie-streep en een lichte
// glas-rand (top-highlight). Signatuur = de gloeiende "gob" (smelt-orb) als hero-accent en
// match-/vertrouwensmeter. Premium ambachtelijk-dark; alle beweging respecteert reduced-motion.
...
// — Palet: donker-warme oven-grond, gloeiend smelt-licht, translucent glas —
const C = {
  bg: "#161110",
  bgDeep: "#0d0a09",
  glass: "rgba(40,29,24,0.62)",
  glassSoft: "rgba(54,39,31,0.55)",
  glassRaise: "rgba(66,47,37,0.6)",
  rim: "rgba(255,206,150,0.24)",
  ink: "#f7ede1",
  inkSoft: "#dcc7b3",
  inkMute: "#b0967f",
  inkFaint: "#846d5b",
  line: "rgba(255,178,110,0.16)",
  lineSoft: "rgba(255,178,110,0.09)",
  // smelt
  amber: "#ffab45",
  amberHi: "#ffc978",
  ember: "#ff7a2e",
  emberDeep: "#e0561c",
  core: "#ffe7bf",
  amberWash: "rgba(255,171,69,0.16)",
  // status
  ok: "#67c6a0",
  okInk: "#8fdcbb",
  okWash: "rgba(103,198,160,0.15)",
  warn: "#ff9d3c",
  warnInk: "#ffbb6e",
  warnWash: "rgba(255,157,60,0.16)",
  info: "#79aecf",
  infoInk: "#a3cbe4",
  infoWash: "rgba(121,174,207,0.15)",
  bad: "#ef6a52",
  badInk: "#ff937d",
  badWash: "rgba(239,106,82,0.16)",
};

const display = { fontFamily: "'Georgia', 'Iowan Old Style', 'Times New Roman', serif" };
...
// — Gloeiende "gob" — smelt-orb met hete kern, gebruikt als accent en als match-meter —
function Gob({ size = 60, value = 100, id }: { size?: number; value?: number; id: string }) {
  const c = size / 2;
  const heat = 0.55 + (value / 100) * 0.45;
  return (
    <svg
      width={size}
      height={size}
    ...
    >
      <defs>
        <radialGradient id={`gob-${id}`} cx="42%" cy="38%" r="65%">
          <stop offset="0%" stopColor={C.core} stopOpacity={heat} />
          <stop offset="42%" stopColor={C.amber} stopOpacity={heat * 0.95} />
          <stop offset="78%" stopColor={C.ember} stopOpacity={heat * 0.85} />
          <stop offset="100%" stopColor={C.emberDeep} stopOpacity="0.9" />
        </radialGradient>
...
// — Glas-sparkline: gloeiende ember-lijn met zachte diffuse gloed —
function GlowLine({ data, tone, id }: { data: number[]; tone: string; id: string }) {
  ...
  return (
    ...
    >
      ...
      <polyline
        ...
        strokeLinejoin="round"
      />
      <circle cx={last[0]} cy={last[1]} r="3" fill={C.core} stroke={tone} strokeWidth="1.4" />
    </svg>
  );
}

function Meter({ value, tone = C.amber }: { value: number; tone?: string }) {
  return (
    <span className="hidden items-center gap-2.5 sm:flex" aria-hidden="true">
      ...
      >
        <span
          ...
          style={{
            width: `${value}%`,
            backgroundImage: `linear-gradient(90deg, ${C.emberDeep}, ${tone}, ${C.core})`,
            boxShadow: `0 0 8px ${tone}`,
            transition: "width 0.6s cubic-bezier(0.22,1,0.36,1)",
          }}
        />
      </span>
      <span className="text-[12.5px] font-semibold" style={{ color: C.amberHi, ...num }}>
        {value}%
      </span>
    </span>
  );
...
// — Glas-paneel: translucent, glas-rand-highlight boven, refractie-streep, zachte gloed —
function Panel({
  children,
  className = "",
  as: Tag = "div",
  ...
  as?: "div" | "section" | "li";
  glow?: boolean;
  fill?: "glass" | "soft" | "raise";
}) {
  const bg = { glass: C.glass, soft: C.glassSoft, raise: C.glassRaise }[fill];
  return (
    <Tag
      className={`relative overflow-hidden rounded-[16px] backdrop-blur-md ${className}`}
      style={{
        background: bg,
        border: `1px solid ${C.line}`,
        boxShadow: glow
          ? `0 1px 0 ${C.rim} inset, 0 0 42px rgba(255,122,46,0.14), 0 14px 40px rgba(0,0,0,0.5)`
          : `0 1px 0 ${C.rim} inset, 0 12px 34px rgba(0,0,0,0.42)`,
        color: C.ink,
      }}
    >
      {/* glas-rand highlight */}
      <span
        ...
        style={{ background: `linear-gradient(90deg, transparent, ${C.rim}, transparent)` }}
      />
      {/* refractie-streep */}
      <span
        aria-hidden="true"
        className="pointer-events-none absolute -top-8 right-6 h-40 w-16 -rotate-45"
        style={{ background: "linear-gradient(180deg, rgba(255,231,191,0.10), transparent)" }}
      />
      {children}
    </Tag>
  );
...
function PrimaryButton({
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className={`group inline-flex items-center justify-center gap-2 rounded-full px-5 py-2.5 text-[13px] font-semibold transition-all duration-200 hover:brightness-[1.08] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#ffab45] focus-visible:ring-offset-2 focus-visible:ring-offset-[#161110] active:translate-y-px motion-reduce:transition-none ${className}`}
      style={{
        color: "#2a1608",
        backgroundImage: `linear-gradient(135deg, ${C.amberHi}, ${C.amber} 48%, ${C.ember})`,
        border: `1px solid ${C.emberDeep}`,
        boxShadow: `0 0 16px rgba(255,122,46,0.4), 0 1px 0 rgba(255,231,191,0.55) inset`,
        ...bodyFont,
      }}
    >
      {children}
...
function GhostButton({
  ...
  return (
    <button
      ...
      className={`inline-flex items-center justify-center gap-2 rounded-full px-4 py-2.5 text-[12.5px] font-semibold transition-all duration-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#ffab45] focus-visible:ring-offset-2 focus-visible:ring-offset-[#161110] motion-reduce:transition-none ${className}`}
      style={{
        color: active ? "#2a1608" : C.inkSoft,
        background: active ? C.amber : C.glassRaise,
...
export function Concept468() {
  ...
  return (
    ...
    >
      {/* smelt-gloed op de achtergrond */}
      <div
        aria-hidden="true"
        className="pointer-events-none absolute inset-0"
        style={{
          backgroundImage:
            `radial-gradient(60% 40% at 78% 8%, rgba(255,122,46,0.20), transparent 70%),` +
            `radial-gradient(50% 45% at 12% 22%, rgba(255,171,69,0.12), transparent 70%),` +
            `radial-gradient(70% 50% at 50% 108%, rgba(224,86,28,0.16), transparent 70%)`,
        }}
      />
      <style>{`
        @keyframes gbRise { from { opacity: 0; transform: translateY(8px); } to { opacity: 1; transform: translateY(0); } }
        .gb-rise { animation: gbRise 0.5s cubic-bezier(0.22,1,0.36,1) both; }
        @keyframes gbGlow { 0%,100% { opacity: 0.82; } 50% { opacity: 1; } }
        .gb-glow { animation: gbGlow 4.5s ease-in-out infinite; }
        @media (prefers-reduced-motion: reduce) { .gb-rise, .gb-glow { animation: none !important; } }
      `}</style>

      <div className="relative mx-auto max-w-6xl px-4 pb-24 sm:px-6 md:px-8">
        <TopBar />
        <NavBar screen={screen} setScreen={setScreen} />
        <main key={screen} className="gb-rise pt-7">
          {screen === "dashboard" && (
  ...
  return (
    <header className="flex items-center justify-between gap-4 pt-8">
      <div className="flex items-center gap-3.5">
        <span
          className="gb-glow relative inline-flex h-12 w-12 items-center justify-center rounded-2xl"
          style={{ background: C.bgDeep, border: `1px solid ${C.line}` }}
          aria-hidden="true"
        >
          <Gob size={38} value={100} id="logo" />
        </span>
        <div>
          <p
          ...
          <p className="mt-1.5 text-[11px] leading-none" style={{ color: C.inkMute }}>
            {PROFIEL.plaats} · gloeiend gevormd
          </p>
        </div>
      </div>
      <div className="flex items-center gap-3">
        ...
        <span
          ...
          style={{ background: C.glassRaise, border: `1px solid ${C.line}`, color: C.inkMute }}
        ...
        >
          ...
          {ongelezen > 0 && (
            <span
              ...
              style={{ background: C.ember, color: C.core, ...num }}
              aria-hidden="true"
            >
              {ongelezen}
            </span>
...
function NavBar({ screen, setScreen }: { screen: ScreenKey; setScreen: (s: ScreenKey) => void }) {
  return (
    <nav aria-label="Hoofdnavigatie" className="mt-7">
      <div
        className="flex items-center gap-1 overflow-x-auto rounded-full p-1.5 backdrop-blur-md"
        style={{ background: C.glassSoft, border: `1px solid ${C.line}` }}
      >
        {SCREENS.map((s) => {
          ...
          return (
            <button
              ...
              className="flex shrink-0 items-center gap-2 rounded-full px-4 py-2 text-[12.5px] font-semibold transition-all duration-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#ffab45] focus-visible:ring-offset-2 focus-visible:ring-offset-[#241b16] motion-reduce:transition-none"
              style={{
                color: on ? "#2a1608" : C.inkMute,
                backgroundImage: on
                  ? `linear-gradient(135deg, ${C.amberHi}, ${C.amber} 55%, ${C.ember})`
                  : "none",
                boxShadow: on ? "0 0 14px rgba(255,122,46,0.4)" : "none",
                ...bodyFont,
              }}
            >
              {s.label}
...
function Dashboard({ onOpen, onActies }: { onOpen: () => void; onActies: () => void }) {
  ...
  return (
    <div className="space-y-7">
      <section className="grid grid-cols-1 gap-5 lg:grid-cols-[1.4fr_1fr]">
        <Panel className="p-7 md:p-9" glow>
          <div className="relative flex items-start justify-between gap-4">
            <div>
              <Eyebrow>Vandaag · uit de oven</Eyebrow>
              <h1
            ...
            <span className="gb-glow shrink-0">
              <Gob size={64} value={92} id="d1" />
            </span>
          </div>
          <p
          ...
          >
            Je praktijk als gloeiend glas: warm gevormd, helder en verifieerbaar. Loop langs je
            acties — alles blijft op temperatuur.
          </p>
      ...
      <section>
        <div className="mb-4">
          <Eyebrow>Op temperatuur · deze maand</Eyebrow>
        </div>
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {KPIS.map((k, i) => (
            <Panel key={k.label} className="p-5">
              ...
              <div className="mt-3">
                <GlowLine data={k.spark} tone={k.up ? C.amber : C.warn} id={`k458-${i}`} />
              </div>
            </Panel>
          ))}
        </div>
        ...
        <div>
          <div className="mb-4 flex items-baseline justify-between">
            ...
            <button
              type="button"
              onClick={onOpen}
              className="rounded text-[11px] font-semibold uppercase tracking-[0.14em] transition-colors hover:brightness-110 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#ffab45]"
              style={{ color: C.amberHi, ...bodyFont }}
            >
              Alle →
            </button>
          </div>
              ...
              {OPDRACHTEN.map((o, i) => (
                <li key={o.id} style={{ borderTop: i === 0 ? "none" : `1px solid ${C.lineSoft}` }}>
                  <button
                    type="button"
                    onClick={onOpen}
                    className="group grid w-full grid-cols-[auto_1fr_auto] items-center gap-4 px-5 py-4 text-left transition-colors hover:bg-[rgba(66,47,37,0.5)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-[#ffab45] motion-reduce:transition-none"
                  >
                    <span className="shrink-0">
                      <Gob size={40} value={o.match} id={`dl-${i}`} />
                    </span>
                    ...
                    <span className="flex items-center gap-3">
                      <Meter value={o.match} tone={o.match >= 90 ? C.amber : C.ember} />
                      <ChevronRight
                        size={17}
                        aria-hidden="true"
                        className="transition-transform group-hover:translate-x-0.5 motion-reduce:transition-none"
  ...
  return (
    <div className="space-y-7">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
        <div
          className="flex flex-1 items-center gap-2.5 rounded-full px-5 py-3 backdrop-blur-md"
          style={{ background: C.glassSoft, border: `1px solid ${C.line}` }}
        >
          ...
          <input
            ...
            className="w-full bg-transparent text-[13px] outline-none placeholder:text-[#846d5b]"
            ...
            <li key={i}>
              <Panel className="p-6">
                <div className="animate-pulse space-y-3 motion-reduce:animate-none">
                  <div className="h-3 w-24 rounded-full" style={{ background: C.glassRaise }} />
                  <div className="h-5 w-2/3 rounded-full" style={{ background: C.glassSoft }} />
                  <div className="h-3 w-1/2 rounded-full" style={{ background: C.glassRaise }} />
                  <div className="h-2 w-full rounded-full" style={{ background: C.glassRaise }} />
                </div>
              </Panel>
            </li>
          ))}
        ...
        <Panel className="p-6" glow>
          <div className="flex flex-col items-center py-14 text-center">
            <span
              ...
              style={{ background: C.bgDeep, border: `1px solid ${C.line}`, color: C.amberHi }}
              aria-hidden="true"
            >
              <Search size={26} />
            </span>
...
function MarktKaart({
  ...
  return (
    <Panel className="p-6" glow={strong}>
      <div className="grid grid-cols-[1fr_auto] items-start gap-4">
        ...
        <div className="flex flex-col items-center gap-1.5">
          <span className="relative inline-flex items-center justify-center">
            <Gob size={64} value={opdracht.match} id={`m${index}`} />
            <span className="absolute flex flex-col items-center">
              <span
                className="text-[15px] font-bold leading-none"
                style={{ color: "#2a1608", ...num }}
              >
                {opdracht.match}
              </span>
              <span
                className="mt-0.5 text-[7.5px] uppercase tracking-[0.12em]"
                style={{ color: "rgba(42,22,8,0.7)", ...bodyFont }}
              >
                match
              </span>
            </span>
          </span>
          <span className="text-[13px] font-bold" style={{ color: C.amberHi, ...num }}>
            {opdracht.tarief}
          </span>
        </div>
      </div>
      <div className="mt-4 flex items-center gap-3">
        <button
          ...
          onClick={() => setOpen((v) => !v)}
          aria-expanded={open}
          className="inline-flex items-center gap-1.5 rounded-full px-3.5 py-1.5 text-[11.5px] font-semibold transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#ffab45] focus-visible:ring-offset-2 focus-visible:ring-offset-[#241b16]"
          style={{ color: C.amberHi, border: `1px solid ${C.line}` }}
        >
...
function RedenBlok({
  ...
  return (
    <div
      ...
      style={{ background: C.glassRaise, border: `1px solid ${C.lineSoft}` }}
    >
      <p
  ...
  return (
    <div className="space-y-6">
      <button
        type="button"
        onClick={onBack}
        className="inline-flex items-center gap-2 rounded-full px-4 py-2 text-[12.5px] font-semibold transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#ffab45] focus-visible:ring-offset-2 focus-visible:ring-offset-[#161110]"
        style={{ color: C.inkSoft, border: `1px solid ${C.line}`, background: C.glassRaise }}
      >
      ...
      <Panel className="p-7 md:p-9" glow>
        <div className="relative flex items-start justify-between gap-4">
          ...
          <span className="gb-glow hidden shrink-0 sm:block">
            <Gob size={72} value={opdracht.match} id="detail" />
          </span>
        </div>
        ...
        <div className="relative flex flex-wrap items-center justify-between gap-6">
          <div className="max-w-md">
            ...
            <div className="mt-4 max-w-xs">
              <Meter value={ratio} tone={C.amber} />
            </div>
          </div>
          <span className="relative inline-flex items-center justify-center">
            <Gob size={104} value={ratio} id="vratio" />
            <span className="absolute flex flex-col items-center">
              <span
              ...
              <span
                ...
                style={{ color: "rgba(42,22,8,0.72)", ...bodyFont }}
              >
                % op orde
              </span>
            </span>
            ...
            return (
              ...
              >
                <button
                  ...
                  className="grid w-full grid-cols-[1fr_auto] items-center gap-4 px-6 py-4 text-left transition-colors hover:bg-[rgba(66,47,37,0.5)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-[#ffab45] motion-reduce:transition-none sm:grid-cols-[1fr_11rem_2.5rem]"
                >
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-2 max-w-md text-[13.5px]" style={{ color: C.inkSoft }}>
          Rustig van boven naar beneden — zo blijf je verifieerbaar en betaald, op temperatuur.
        </p>
      </div>
...
function factuurTone(status: string): {
  ...
  return { ink: C.inkMute, wash: C.glassRaise, tone: C.line, Icon: FileText };
}
    ...
    <div className="space-y-6">
      ...
      <section className="grid grid-cols-1 gap-4 sm:grid-cols-3">
        {[
          ...
          <Panel key={s.l} className="p-6" glow={s.alarm}>
            <div className="flex items-center justify-between">
              <p
                className="text-[10.5px] font-semibold uppercase tracking-[0.1em]"
                style={{ color: C.inkMute, ...bodyFont }}
        ...
        <ul>
          {FACTUREN.map((f, i) => {
            ...
            return (
              <li
                key={f.nr}
                className="grid grid-cols-[1fr_auto] items-center gap-3 px-6 py-4 transition-colors hover:bg-[rgba(66,47,37,0.5)] sm:grid-cols-[8rem_1fr_5rem_9rem_6rem] sm:gap-4"
                style={{ borderTop: i === 0 ? "none" : `1px solid ${C.lineSoft}` }}
              >
                <span
