...
// Concept 436 — "Salie" · Salie-groen, kruidig-kalm / calm interface.
// Een laag-prikkelende interface in de 2026 calm-taal: cognitieve helderheid boven sensorische
// drukte. Gedempt saliegroen (#5f7a5a / #4a6147) met botanische rust op warm wit (#f2f4ee),
// tekst #262a22, een klei-secundair voor warmte. Veel lucht, zachte radii, geen decoratieve
// drukte — rustgevend rondom gevoelige documenten. Micro-interacties zijn traag en zacht en
// respecteren reduced-motion volledig.
...
// — Palet: gedempt saliegroen, warm wit, klei-secundair, diepe kruid-ink —
const C = {
  bg: "#f2f4ee",
  bgDeep: "#eaeee3",
  paper: "#f8faf4",
  ...
  inkMute: "#727c65",
  inkFaint: "#9aa38a",
  line: "rgba(38,42,34,0.11)",
  lineSoft: "rgba(38,42,34,0.06)",
  sage: "#5f7a5a",
  sageDeep: "#4a6147",
  sageHi: "#7d966f",
  sageWash: "rgba(95,122,90,0.13)",
  sageMist: "rgba(95,122,90,0.07)",
  clay: "#b98b62",
  clayInk: "#8a6039",
  clayWash: "rgba(185,139,98,0.15)",
  ok: "#5f7a5a",
  okInk: "#43613e",
  okWash: "rgba(95,122,90,0.15)",
  warn: "#b98b62",
  warnInk: "#8a6039",
  warnWash: "rgba(185,139,98,0.16)",
  info: "#5f7a5a",
  infoInk: "#48613f",
  infoWash: "rgba(95,122,90,0.12)",
  bad: "#b06a58",
  badInk: "#8a4635",
  badWash: "rgba(176,106,88,0.15)",
};
...
// — Blad-merk: een rustig botanisch rondje voor certificaten/insignia —
function Blad({ Icon, tone, size = 40 }: { Icon: LucideIcon; tone: string; size?: number }) {
  return (
    <span
      className="relative inline-flex items-center justify-center rounded-[14px]"
      style={{
        width: size,
        height: size,
...
function Panel({
  ...
  const bg = tone === "sage" ? C.paperSoft : C.paper;
  return (
    <Tag
      ...
      style={{
        background: bg,
        border: `1px solid ${C.line}`,
        boxShadow: "0 1px 2px rgba(38,42,34,0.03), 0 10px 30px rgba(74,97,71,0.05)",
        color: C.ink,
      }}
    >
      {children}
...
function Eyebrow({ children, tone = C.sageDeep }: { children: React.ReactNode; tone?: string }) {
  return (
    <p
...
function PrimaryButton({
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className={`group inline-flex items-center justify-center gap-2 rounded-full px-5 py-2.5 text-[13px] font-semibold transition-all duration-500 hover:brightness-[1.04] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#5f7a5a] focus-visible:ring-offset-2 focus-visible:ring-offset-[#f2f4ee] active:translate-y-px motion-reduce:transition-none ${className}`}
      style={{
        color: "#f8faf4",
        background: C.sage,
        boxShadow: "0 4px 14px rgba(74,97,71,0.2)",
        ...body,
      }}
    >
      {children}
...
function GhostButton({
  ...
  return (
    <button
      ...
      className={`inline-flex items-center justify-center gap-2 rounded-full px-4 py-2.5 text-[12.5px] font-semibold transition-all duration-500 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#5f7a5a] focus-visible:ring-offset-2 focus-visible:ring-offset-[#f2f4ee] motion-reduce:transition-none ${className}`}
      style={{
        color: active ? "#f8faf4" : C.inkSoft,
...
// — Kruid-lijn: zachte gedempte sparkline zonder felle accenten —
function KruidLijn({ data, tone, id }: { data: number[]; tone: string; id: string }) {
  ...
  return (
    ...
    >
      <defs>
        <linearGradient id={`salie-${id}`} x1="0" y1="0" x2="0" y2="1">
          <stop offset="0%" stopColor={tone} stopOpacity="0.22" />
          <stop offset="100%" stopColor={tone} stopOpacity="0" />
        </linearGradient>
      </defs>
      <polygon points={`0,${h} ${line} ${w},${h}`} fill={`url(#salie-${id})`} />
      <polyline
        points={line}
        fill="none"
        stroke={tone}
      ...
      />
      <circle cx={last[0]} cy={last[1]} r="2.4" fill={C.paper} stroke={tone} strokeWidth="1.5" />
    </svg>
  );
}

// — Tuinmeter: rustige voortgangsbalk in saliegroen —
function TuinMeter({ value, tone = C.sage }: { value: number; tone?: string }) {
  return (
...
export function Concept436() {
  ...
  return (
    ...
    >
      <style>{`
        @keyframes salieRise { from { opacity: 0; transform: translateY(6px); } to { opacity: 1; transform: translateY(0); } }
        .salie-rise { animation: salieRise 0.6s cubic-bezier(0.22, 1, 0.36, 1) both; }
        @media (prefers-reduced-motion: reduce) { .salie-rise { animation: none !important; } }
      `}</style>

      <div className="relative mx-auto max-w-6xl px-4 pb-24 sm:px-6 md:px-8">
        <TopBar />
        <NavBar screen={screen} setScreen={setScreen} />
        <main key={screen} className="salie-rise pt-8">
          {screen === "dashboard" && (
    ...
    <header className="flex items-center justify-between gap-4 pt-8">
      <div className="flex items-center gap-3.5">
        <span
          ...
          style={{ background: C.sage, color: "#f8faf4" }}
          aria-hidden="true"
        >
          <Leaf size={18} />
        </span>
        <div>
          ...
          >
            <Sprout size={11} aria-hidden="true" style={{ color: C.sage }} />
            {PROFIEL.plaats} · rustig op orde
          </p>
        </div>
      </div>
      <div className="flex items-center gap-3">
        ...
        >
          ...
          {ongelezen > 0 && (
            <span
              ...
              style={{ background: C.clay, color: "#f8faf4", ...num }}
              aria-hidden="true"
            >
              {ongelezen}
            </span>
        ...
        <span
          className="inline-flex h-11 w-11 items-center justify-center rounded-[16px] text-[13px] font-semibold"
          style={{
        ...
        {SCREENS.map((s) => {
          ...
          return (
            <button
              ...
              className="flex shrink-0 items-center gap-2 rounded-full px-4 py-2 text-[12.5px] font-semibold transition-all duration-500 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#5f7a5a] focus-visible:ring-offset-2 focus-visible:ring-offset-[#f8faf4] motion-reduce:transition-none"
              style={{
                color: on ? "#f8faf4" : C.inkMute,
  ...
  return (
    <div className="space-y-8">
      <section className="grid grid-cols-1 gap-5 lg:grid-cols-[1.4fr_1fr]">
        <Panel className="p-7 md:p-10" tone="sage">
          <div className="relative">
            <Eyebrow>Vandaag · rustig licht</Eyebrow>
            <div className="mt-5 flex items-start justify-between gap-4">
              <h1
                className="text-[32px] font-semibold leading-[1.08] tracking-[-0.01em] md:text-[44px]"
                style={{ color: C.ink, ...display }}
              >
                Goedemorgen,
                <br />
            ...
            <p className="mt-4 max-w-md text-[14px] leading-relaxed" style={{ color: C.inkSoft }}>
              Een kalme werkdag begint met overzicht. Loop rustig langs je acties, houd je praktijk
              verifieerbaar en betaald — laag-prikkelend en helder, zonder ruis.
            </p>
        ...
        <Panel className="p-7">
          ...
          <h2
            className="mt-5 text-[20px] font-semibold leading-snug"
            style={{ color: C.ink, ...display }}
          >
            {primair.titel}
          </h2>
          ...
          <div className="mt-5 pt-4" style={{ borderTop: `1px solid ${C.lineSoft}` }}>
            <p className="flex items-center gap-2 text-[12px]" style={{ color: C.inkMute, ...num }}>
              <Leaf size={13} aria-hidden="true" style={{ color: C.sage }} />
      ...
      <section>
        <div className="mb-4">
          <Eyebrow>Kruidentuin · deze maand</Eyebrow>
        </div>
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {KPIS.map((k, i) => (
            <Panel key={k.label} className="p-5">
              ...
              <div className="mt-3">
                <KruidLijn data={k.spark} tone={k.up ? C.sage : C.clay} id={`kpi-${i}`} />
              </div>
            </Panel>
          ))}
        </div>
        ...
        <div>
          <div className="mb-4 flex items-baseline justify-between">
            ...
            <button
              type="button"
              onClick={onOpen}
              className="rounded text-[11px] font-semibold uppercase tracking-[0.14em] transition-colors hover:brightness-90 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#5f7a5a] focus-visible:ring-offset-2 focus-visible:ring-offset-[#f2f4ee]"
              style={{ color: C.sageDeep, ...body }}
            >
              Alle →
            </button>
          </div>
              ...
              {OPDRACHTEN.map((o, i) => (
                <li key={o.id} style={{ borderTop: i === 0 ? "none" : `1px solid ${C.lineSoft}` }}>
                  <button
                    type="button"
                    onClick={onOpen}
                    className="group grid w-full grid-cols-[auto_1fr_auto] items-center gap-4 px-5 py-4 text-left transition-colors hover:bg-[#eef2e7] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-[#5f7a5a] motion-reduce:transition-none"
                  >
                    <span
                      className="inline-flex h-10 w-10 items-center justify-center rounded-[13px]"
                      style={{
                        background: i === 0 ? C.sageWash : C.raise,
                        border: `1px solid ${i === 0 ? C.sage : C.line}`,
                      }}
                    >
                      <span
                        className="text-[12px] font-bold leading-none"
                        style={{ color: i === 0 ? C.sageDeep : C.inkMute, ...num }}
                      >
                        {o.match}
                      </span>
                    </span>
                    ...
                    <span className="flex items-center gap-3">
                      <TuinMeter value={o.match} tone={o.match >= 90 ? C.sage : C.clay} />
                      <ChevronRight
                        size={17}
                        aria-hidden="true"
                        className="transition-transform group-hover:translate-x-0.5 motion-reduce:transition-none"
            ...
            <ul className="space-y-1">
              {CREDENTIALS.map((c, i) => {
                ...
                return (
                  ...
                  >
                    <Blad Icon={st.Icon} tone={st.ink} size={34} />
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
        ...
        >
          ...
          <input
            ...
            className="w-full bg-transparent text-[13px] outline-none placeholder:text-[#9aa38a]"
            style={{ color: C.ink, ...body }}
          />
        </div>
      ...
      {loading ? (
        ...
        <Panel className="p-6" tone="sage">
          <div className="relative flex flex-col items-center py-14 text-center">
            <Blad Icon={Search} tone={C.sageDeep} size={60} />
            ...
            <p className="mx-auto mt-2 max-w-xs text-[13.5px]" style={{ color: C.inkSoft }}>
              Geen opdracht bij {q ? `“${q}”` : "je zoekterm"}. Verruim je zoekterm en probeer het
              rustig opnieuw.
            </p>
            <div className="mt-6">
...
function MarktKaart({
  ...
  const tone = strong ? C.sage : C.clay;
  const toneInk = strong ? C.sageDeep : C.clayInk;
  return (
    <Panel className="p-6">
      <div className="grid grid-cols-[1fr_auto] items-start gap-4">
        ...
        <div className="flex flex-col items-center gap-1.5">
          <span
            className="relative inline-flex h-14 w-14 flex-col items-center justify-center rounded-[16px]"
            style={{ background: strong ? C.sageWash : C.clayWash, border: `1.5px solid ${tone}` }}
          >
      ...
      <div className="mt-4 flex items-center gap-3">
        <button
          ...
          onClick={() => setOpen((v) => !v)}
          aria-expanded={open}
          className="inline-flex items-center gap-1.5 rounded-full px-3.5 py-1.5 text-[11.5px] font-semibold transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#5f7a5a] focus-visible:ring-offset-2 focus-visible:ring-offset-[#f8faf4]"
          style={{ color: C.sageDeep, border: `1px solid ${C.line}`, ...body }}
        >
  ...
  return (
    <div className="space-y-6">
      <button
        type="button"
        onClick={onBack}
        className="inline-flex items-center gap-2 rounded-full px-4 py-2 text-[12.5px] font-semibold transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#5f7a5a] focus-visible:ring-offset-2 focus-visible:ring-offset-[#f2f4ee]"
      ...
      <Panel className="p-7 md:p-10" tone="sage">
        <div className="relative flex flex-wrap items-center gap-2">
          ...
          <span
            ...
            style={{ color: "#f8faf4", background: tone, ...body }}
          >
            <Sprout size={11} aria-hidden="true" /> {strong ? "Sterke match" : "Goede match"} ·{" "}
            {opdracht.match}%
          </span>
        </div>
        <h1
          className="relative mt-4 max-w-2xl text-[30px] font-semibold leading-[1.1] tracking-[-0.01em] md:text-[42px]"
          style={{ color: C.ink, ...display }}
        >
          {opdracht.titel}
        </h1>
      ...
      <section>
        ...
        <div className="mt-5 grid grid-cols-1 gap-4 md:grid-cols-2">
          <Panel className="p-6">
            <div className="flex items-center gap-2">
              <span
                className="inline-flex h-8 w-8 items-center justify-center rounded-[11px]"
        ...
        <div className="mt-4">
          <span className="text-[12px]" style={{ color: toneInk, ...body }}>
            ...
            {strong
              ? "rustig en sterk afgestemd op jouw profiel."
...
function Verificatie() {
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="relative flex flex-wrap items-center justify-between gap-6">
          <div className="max-w-md">
            ...
            <div className="mt-4 max-w-xs">
              <TuinMeter value={ratio} tone={C.sage} />
            </div>
          </div>
          <span
            className="relative inline-flex h-24 w-24 flex-col items-center justify-center rounded-[26px]"
            style={{ background: C.sageWash, border: `1.5px solid ${C.sage}` }}
          >
            <span
              className="text-[26px] font-semibold leading-none"
              style={{ color: C.sageDeep, ...num }}
            >
              {ratio}
            </span>
            <span
            ...
            return (
              ...
              >
                <button
                  ...
                  className="grid w-full grid-cols-[1fr_auto] items-center gap-4 px-6 py-4 text-left transition-colors hover:bg-[#eef2e7] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-[#5f7a5a] motion-reduce:transition-none sm:grid-cols-[1fr_11rem_2.5rem]"
                >
                  <span className="flex min-w-0 items-center gap-3">
                    <Blad Icon={st.Icon} tone={st.ink} size={38} />
                    <span className="min-w-0">
                      <span
                        className="block truncate text-[15px] font-semibold"
                        style={{ color: C.ink, ...body }}
      ...
      <div>
        <div className="mb-4">
          <Eyebrow tone={C.sageDeep}>Documentenkast</Eyebrow>
        </div>
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-2 max-w-md text-[13.5px]" style={{ color: C.inkSoft }}>
          Rustig van boven naar beneden — zo blijf je verifieerbaar en betaald, kalm en op orde.
        </p>
      </div>
      ...
      <ol className="space-y-3">
        {ACTIES.map((a, i) => {
          ...
          const tone = warn ? C.warn : C.sage;
          const ink = warn ? C.warnInk : C.sageDeep;
          const wash = warn ? C.warnWash : C.sageWash;
          return (
            <li key={a.titel}>
              <Panel className="p-6">
                <div className="grid grid-cols-[auto_1fr] items-start gap-4 sm:grid-cols-[auto_1fr_auto]">
                  <span
                    className="flex h-11 w-11 items-center justify-center rounded-[16px] text-[15px] font-bold"
      ...
      <Panel>
        ...
        <ul>
          {FACTUREN.map((f, i) => {
            ...
            return (
              <li
                key={f.nr}
                className="grid grid-cols-[1fr_auto] items-center gap-3 px-6 py-4 transition-colors hover:bg-[#eef2e7] sm:grid-cols-[8rem_1fr_5rem_9rem_6rem] sm:gap-4"
                style={{ borderTop: i === 0 ? "none" : `1px solid ${C.lineSoft}` }}
              >
                <span
        ...
        >
          <span
            ...
            style={{ color: C.inkMute, ...body }}
          >
            <Leaf size={12} aria-hidden="true" style={{ color: C.sage }} /> Totaal betaald
          </span>
