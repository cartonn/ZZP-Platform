...
// Concept 438 — "Aquarium" · Zwevend glas in water-licht / spatial depth.
// Een lichte, luchtige interface in de 2026 spatial-layering-taal: translucente glaspanelen die als
// glas in helder water zweven, met zachte diepte-lagen en subtiele stroming. Helder aqua
// (#0891b2 / #0e7490) op een water-lichte achtergrond (#eef6f8), tekst #10262c. Light
// glassmorphism (nooit donker): backdrop-blur, zachte randen en drijvende schaduwen. Stroming-
// motion is traag en decoratief en respecteert reduced-motion volledig.
...
// — Palet: helder aqua, water-licht, diepe zeegroen-ink, glasachtige translucentie —
const C = {
  bg: "#eef6f8",
  bgDeep: "#e2eff2",
  ...
  raise: "rgba(8,145,178,0.08)",
  ink: "#10262c",
  inkSoft: "#2f4a51",
  inkMute: "#5c757b",
  inkFaint: "#8ba5aa",
  line: "rgba(16,38,44,0.1)",
  lineSoft: "rgba(16,38,44,0.05)",
  glassLine: "rgba(255,255,255,0.7)",
  aqua: "#0891b2",
  aquaDeep: "#0e7490",
  aquaHi: "#22b8d8",
  aquaWash: "rgba(8,145,178,0.12)",
  aquaMist: "rgba(8,145,178,0.06)",
  coral: "#e07a5f",
  coralInk: "#b1543b",
  coralWash: "rgba(224,122,95,0.14)",
  ok: "#0e9488",
  okInk: "#0a7268",
  okWash: "rgba(14,148,136,0.13)",
  warn: "#d98a3d",
  warnInk: "#a5641f",
  warnWash: "rgba(217,138,61,0.15)",
  info: "#0891b2",
  infoInk: "#0e7490",
  infoWash: "rgba(8,145,178,0.12)",
  bad: "#dc5b57",
  badInk: "#ab3936",
  badWash: "rgba(220,91,87,0.14)",
};
...
// — Glas-bel: een klein translucent rondje voor certificaten/insignia —
function Bel({ Icon, tone, size = 40 }: { Icon: LucideIcon; tone: string; size?: number }) {
  return (
    <span
      ...
      style={{
        ...
        boxShadow: "0 2px 8px rgba(8,145,178,0.14), inset 0 1px 2px rgba(255,255,255,0.9)",
        color: tone,
      }}
      aria-hidden="true"
    >
...
function Panel({
  ...
  return (
    <Tag
      ...
      style={{
        background: tone === "strong" ? C.glassStrong : C.glass,
        border: `1px solid ${C.glassLine}`,
        boxShadow:
          "0 1px 2px rgba(16,38,44,0.03), 0 14px 40px rgba(14,116,144,0.1), inset 0 1px 0 rgba(255,255,255,0.7)",
        color: C.ink,
      }}
    >
      {children}
...
function PrimaryButton({
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className={`group inline-flex items-center justify-center gap-2 rounded-full px-5 py-2.5 text-[13px] font-semibold transition-all duration-300 hover:brightness-[1.06] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#0891b2] focus-visible:ring-offset-2 focus-visible:ring-offset-[#eef6f8] active:translate-y-px motion-reduce:transition-none ${className}`}
      style={{
        color: "#ffffff",
        background: `linear-gradient(135deg, ${C.aquaHi} 0%, ${C.aqua} 55%, ${C.aquaDeep} 100%)`,
        boxShadow: "0 6px 18px rgba(8,145,178,0.28)",
        ...body,
      }}
    >
      {children}
...
function GhostButton({
  ...
  return (
    <button
      ...
      className={`inline-flex items-center justify-center gap-2 rounded-full px-4 py-2.5 text-[12.5px] font-semibold backdrop-blur-md transition-all duration-300 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#0891b2] focus-visible:ring-offset-2 focus-visible:ring-offset-[#eef6f8] motion-reduce:transition-none ${className}`}
      style={{
        color: active ? "#ffffff" : C.inkSoft,
        background: active ? C.aqua : C.glassSoft,
        border: `1px solid ${active ? C.aqua : C.glassLine}`,
        ...body,
      }}
    >
      {children}
...
// — Stroom-lijn: zachte aqua sparkline met water-vulling —
function StroomLijn({ data, tone, id }: { data: number[]; tone: string; id: string }) {
  ...
  return (
    ...
    >
      <defs>
        <linearGradient id={`aqua-${id}`} x1="0" y1="0" x2="0" y2="1">
          <stop offset="0%" stopColor={tone} stopOpacity="0.28" />
          <stop offset="100%" stopColor={tone} stopOpacity="0" />
        </linearGradient>
      </defs>
      <polygon points={`0,${h} ${line} ${w},${h}`} fill={`url(#aqua-${id})`} />
      <polyline
        points={line}
        fill="none"
        stroke={tone}
      ...
      />
      <circle cx={last[0]} cy={last[1]} r="3.2" fill={tone} opacity="0.25" />
      <circle cx={last[0]} cy={last[1]} r="2.2" fill="#ffffff" stroke={tone} strokeWidth="1.5" />
    </svg>
  );
}

// — Waterpeil-meter: translucent voortgangsbalk als vloeistof —
function PeilMeter({ value, tone = C.aqua }: { value: number; tone?: string }) {
  return (
    <span className="hidden items-center gap-2.5 sm:flex" aria-hidden="true">
      ...
      >
        <span
          ...
          style={{
            width: `${value}%`,
            background: `linear-gradient(90deg, ${C.aquaHi}, ${tone})`,
  ...
  return (
    <div
      ...
      style={{
        ...body,
        color: C.ink,
        background: `radial-gradient(120% 90% at 15% -10%, #f4fbfc 0%, ${C.bg} 55%, ${C.bgDeep} 100%)`,
      }}
    >
      <style>{`
        @keyframes aquaRise { from { opacity: 0; transform: translateY(8px); } to { opacity: 1; transform: translateY(0); } }
        .aqua-rise { animation: aquaRise 0.5s cubic-bezier(0.22, 1, 0.36, 1) both; }
        @keyframes aquaFloat { 0%,100% { transform: translateY(0); } 50% { transform: translateY(-16px); } }
        .aqua-float { animation: aquaFloat 9s ease-in-out infinite; }
        @media (prefers-reduced-motion: reduce) {
          .aqua-rise { animation: none !important; }
          .aqua-float { animation: none !important; }
        }
      `}</style>

      {/* Zwevende water-licht lagen — puur decoratief */}
      <span
        aria-hidden="true"
        className="aqua-float pointer-events-none absolute -left-24 top-10 h-80 w-80 rounded-full"
        style={{
          background: `radial-gradient(circle at center, ${C.aquaHi} 0%, transparent 70%)`,
          opacity: 0.16,
          filter: "blur(34px)",
        }}
      />
      <span
        aria-hidden="true"
        className="aqua-float pointer-events-none absolute right-0 top-52 h-72 w-72 rounded-full"
        style={{
          background: `radial-gradient(circle at center, ${C.aqua} 0%, transparent 70%)`,
          opacity: 0.12,
          filter: "blur(38px)",
          animationDelay: "-4s",
        }}
      ...
      <div className="relative mx-auto max-w-6xl px-4 pb-24 sm:px-6 md:px-8">
        ...
        <main key={screen} className="aqua-rise pt-7">
          {screen === "dashboard" && (
    ...
    <header className="flex items-center justify-between gap-4 pt-8">
      <div className="flex items-center gap-3.5">
        <span
          ...
          style={{
            background: `linear-gradient(135deg, ${C.aquaHi} 0%, ${C.aqua} 60%, ${C.aquaDeep} 100%)`,
            color: "#ffffff",
            boxShadow: "0 4px 12px rgba(8,145,178,0.3), inset 0 1px 2px rgba(255,255,255,0.5)",
          }}
          aria-hidden="true"
        >
          <Waves size={18} />
        ...
        <div>
          <p
            className="text-[19px] font-semibold leading-none tracking-[-0.005em]"
          ...
          >
            <Droplet size={11} aria-hidden="true" style={{ color: C.aqua }} />
            {PROFIEL.plaats} · helder bewaard
          </p>
        </div>
      </div>
      <div className="flex items-center gap-3">
        ...
        <span
          className="relative inline-flex h-10 w-10 items-center justify-center rounded-full backdrop-blur-md"
          style={{ background: C.glassSoft, border: `1px solid ${C.glassLine}`, color: C.inkMute }}
        ...
        >
          ...
          {ongelezen > 0 && (
            <span
              ...
              style={{ background: C.coral, color: "#ffffff", ...num }}
              aria-hidden="true"
            >
              {ongelezen}
            </span>
...
function NavBar({ screen, setScreen }: { screen: ScreenKey; setScreen: (s: ScreenKey) => void }) {
  return (
    <nav aria-label="Hoofdnavigatie" className="mt-7">
      <div
        className="flex items-center gap-1 overflow-x-auto rounded-full p-1.5 backdrop-blur-xl"
        ...
        {SCREENS.map((s) => {
          ...
          return (
            <button
              ...
              className="flex shrink-0 items-center gap-2 rounded-full px-4 py-2 text-[12.5px] font-semibold transition-all duration-300 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#0891b2] focus-visible:ring-offset-2 focus-visible:ring-offset-[#eef6f8] motion-reduce:transition-none"
              style={{
                color: on ? "#ffffff" : C.inkMute,
                background: on ? C.aqua : "transparent",
                ...body,
              }}
            >
              {s.label}
  ...
  return (
    <div className="space-y-7">
      <section className="grid grid-cols-1 gap-5 lg:grid-cols-[1.4fr_1fr]">
        <Panel className="p-7 md:p-9" tone="strong">
          <div className="relative">
            <Eyebrow>Vandaag · helder water</Eyebrow>
            <div className="mt-4 flex items-start justify-between gap-4">
              <h1
                className="text-[32px] font-semibold leading-[1.06] tracking-[-0.015em] md:text-[44px]"
                style={{ color: C.ink, ...display }}
              >
                Goedemorgen,
                <br />
            ...
            <p className="mt-3 max-w-md text-[14px] leading-relaxed" style={{ color: C.inkSoft }}>
              Je praktijk zweeft helder en overzichtelijk als glas in water. Loop langs je acties en
              houd je administratie verifieerbaar en betaald — luchtig, met diepte waar het telt.
            </p>
        ...
        <Panel className="p-7">
          ...
          <div className="mt-5 pt-4" style={{ borderTop: `1px solid ${C.lineSoft}` }}>
            <p className="flex items-center gap-2 text-[12px]" style={{ color: C.inkMute, ...num }}>
              <Droplet size={13} aria-hidden="true" style={{ color: C.aqua }} />
      ...
      <section>
        <div className="mb-4">
          <Eyebrow>Peilwaarden · deze maand</Eyebrow>
        </div>
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {KPIS.map((k, i) => (
            <Panel key={k.label} className="p-5">
              ...
              <div className="mt-3">
                <StroomLijn data={k.spark} tone={k.up ? C.aqua : C.coral} id={`kpi-${i}`} />
              </div>
            </Panel>
          ))}
        </div>
        ...
        <div>
          <div className="mb-4 flex items-baseline justify-between">
            ...
            <button
              type="button"
              onClick={onOpen}
              className="rounded text-[11px] font-semibold uppercase tracking-[0.14em] transition-colors hover:brightness-90 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#0891b2] focus-visible:ring-offset-2 focus-visible:ring-offset-[#eef6f8]"
              style={{ color: C.aquaDeep, ...body }}
            >
              Alle →
            </button>
          </div>
              ...
              {OPDRACHTEN.map((o, i) => (
                <li key={o.id} style={{ borderTop: i === 0 ? "none" : `1px solid ${C.lineSoft}` }}>
                  <button
                    type="button"
                    onClick={onOpen}
                    className="group grid w-full grid-cols-[auto_1fr_auto] items-center gap-4 px-5 py-4 text-left transition-colors hover:bg-[rgba(8,145,178,0.06)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-[#0891b2] motion-reduce:transition-none"
                  >
                    <span
                      className="inline-flex h-10 w-10 items-center justify-center rounded-full"
                      style={{
                        background: i === 0 ? C.aquaWash : C.glassSoft,
                        border: `1px solid ${i === 0 ? C.aqua : C.glassLine}`,
                      }}
                    >
                      <span
                    ...
                    <span className="flex items-center gap-3">
                      <PeilMeter value={o.match} tone={o.match >= 90 ? C.aqua : C.coral} />
                      <ChevronRight
                        size={17}
                        aria-hidden="true"
                        className="transition-transform group-hover:translate-x-0.5 motion-reduce:transition-none"
            ...
            <ul className="space-y-1">
              {CREDENTIALS.map((c, i) => {
                ...
                return (
                  ...
                  >
                    <Bel Icon={st.Icon} tone={st.ink} size={34} />
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
        ...
        >
          ...
          <input
            ...
            className="w-full bg-transparent text-[13px] outline-none placeholder:text-[#8ba5aa]"
            style={{ color: C.ink, ...body }}
          />
        </div>
      ...
      {loading ? (
            ...
            <li key={i}>
              <Panel className="p-6">
                <div className="animate-pulse space-y-3 motion-reduce:animate-none">
                  ...
                  <div className="h-5 w-2/3 rounded-full" style={{ background: C.aquaMist }} />
        ...
        <Panel className="p-6" tone="strong">
          <div className="relative flex flex-col items-center py-14 text-center">
            <Bel Icon={Search} tone={C.aquaDeep} size={60} />
            ...
            <p className="mx-auto mt-2 max-w-xs text-[13.5px]" style={{ color: C.inkSoft }}>
              Geen opdracht bij {q ? `“${q}”` : "je zoekterm"} kwam bovendrijven. Verruim je
              zoekterm en probeer opnieuw.
            </p>
            <div className="mt-6">
...
function MarktKaart({
  ...
  const tone = strong ? C.aqua : C.coral;
  const toneInk = strong ? C.aquaDeep : C.coralInk;
  const wash = strong ? C.aquaWash : C.coralWash;
  return (
    <Panel className="p-6">
      ...
      <div className="mt-4 flex items-center gap-3">
        <button
          ...
          onClick={() => setOpen((v) => !v)}
          aria-expanded={open}
          className="inline-flex items-center gap-1.5 rounded-full px-3.5 py-1.5 text-[11.5px] font-semibold transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#0891b2] focus-visible:ring-offset-2 focus-visible:ring-offset-[#eef6f8]"
          style={{ color: C.aquaDeep, border: `1px solid ${C.line}`, ...body }}
        >
...
function RedenBlok({
  ...
  return (
    <div
      ...
      style={{ background: C.glassSoft, border: `1px solid ${C.glassLine}` }}
    >
      <p
  ...
  return (
    <div className="space-y-6">
      <button
        type="button"
        onClick={onBack}
        className="inline-flex items-center gap-2 rounded-full px-4 py-2 text-[12.5px] font-semibold backdrop-blur-md transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#0891b2] focus-visible:ring-offset-2 focus-visible:ring-offset-[#eef6f8]"
        style={{
          color: C.inkSoft,
          border: `1px solid ${C.glassLine}`,
          background: C.glass,
      ...
      <Panel className="p-7 md:p-9" tone="strong">
        <div className="relative flex flex-wrap items-center gap-2">
          ...
          <span
            ...
            style={{ color: "#ffffff", background: tone, ...body }}
          >
        ...
        <h1
          className="relative mt-4 max-w-2xl text-[30px] font-semibold leading-[1.1] tracking-[-0.015em] md:text-[42px]"
          style={{ color: C.ink, ...display }}
        >
          {opdracht.titel}
        </h1>
      ...
      <section>
        ...
        <div className="mt-4">
          <span className="text-[12px]" style={{ color: toneInk, ...body }}>
            ...
            {strong
              ? "helder en sterk afgestemd op jouw profiel."
...
function Verificatie() {
  ...
  return (
    <div className="space-y-6">
      <Panel className="p-7 md:p-9" tone="strong">
        <div className="relative flex flex-wrap items-center justify-between gap-6">
          <div className="max-w-md">
            <Eyebrow>Verificatie · helder bewaard</Eyebrow>
            <h1
              className="mt-3 text-[28px] font-semibold leading-tight tracking-[-0.015em]"
              style={{ color: C.ink, ...display }}
            >
              Jouw certificaten
            </h1>
            ...
            <div className="mt-4 max-w-xs">
              <PeilMeter value={ratio} tone={C.aqua} />
            </div>
          </div>
          <span
            ...
            return (
              ...
              >
                <button
                  ...
                  className="grid w-full grid-cols-[1fr_auto] items-center gap-4 px-6 py-4 text-left transition-colors hover:bg-[rgba(8,145,178,0.06)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-[#0891b2] motion-reduce:transition-none sm:grid-cols-[1fr_11rem_2.5rem]"
                >
                  <span className="flex min-w-0 items-center gap-3">
                    <Bel Icon={st.Icon} tone={st.ink} size={38} />
                    <span className="min-w-0">
                      <span
                        className="block truncate text-[15px] font-semibold"
                        style={{ color: C.ink, ...body }}
      ...
      <div>
        <div className="mb-4">
          <Eyebrow tone={C.aquaDeep}>Documentenkast</Eyebrow>
        </div>
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-2 max-w-md text-[13.5px]" style={{ color: C.inkSoft }}>
          Rustig van boven naar beneden — zo blijf je verifieerbaar en betaald, helder en op orde.
        </p>
      </div>
...
function factuurTone(status: string): {
  ...
  return { ink: C.inkMute, wash: C.glassSoft, tone: C.line, Icon: FileText };
}
      ...
      <Panel>
        ...
        <ul>
          {FACTUREN.map((f, i) => {
            ...
            return (
              <li
                key={f.nr}
                className="grid grid-cols-[1fr_auto] items-center gap-3 px-6 py-4 transition-colors hover:bg-[rgba(8,145,178,0.06)] sm:grid-cols-[8rem_1fr_5rem_9rem_6rem] sm:gap-4"
                style={{ borderTop: i === 0 ? "none" : `1px solid ${C.lineSoft}` }}
              >
                <span
        ...
        >
          <span
            ...
            style={{ color: C.inkMute, ...body }}
          >
            <Droplet size={12} aria-hidden="true" style={{ color: C.aqua }} /> Totaal betaald
          </span>
