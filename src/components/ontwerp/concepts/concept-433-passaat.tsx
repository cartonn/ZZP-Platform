...
// Concept 433 — "Passaat" · Luchtige passaatwind / richtingsbeweging.
// Licht en luchtig: een richtings-gradient-wash 'waait' van links naar rechts over het scherm.
// Veren-lichte kaarten met zachte drift op hover; een subtiele kompas-/windroos-toon zonder cliché.
// Hemelblauw (#0284c7), warm zand (#e7cfa6) secundair, achtergrond #f2f7fb, tekst #0f2233.
// Alle beweging respecteert reduced-motion.
...
// — Palet: hemelblauw, warm zand, luchtig lichtblauw papier —
const C = {
  bg: "#f2f7fb",
  bgDeep: "#e8f1f8",
  card: "#ffffff",
  ...
  inkMute: "#657988",
  inkFaint: "#9fb2c0",
  line: "rgba(15,34,51,0.10)",
  lineSoft: "rgba(15,34,51,0.06)",
  sky: "#0284c7",
  skyHi: "#38bdf8",
  skyDeep: "#03628f",
  skyWash: "rgba(2,132,199,0.12)",
  skyMist: "rgba(2,132,199,0.06)",
  sand: "#e7cfa6",
  sandTone: "#c79a52",
  sandDeep: "#a37b38",
  sandWash: "rgba(199,154,82,0.16)",
  ok: "#0f9d6e",
  okInk: "#0a6d4c",
  okWash: "rgba(15,157,110,0.14)",
  warn: "#c79a52",
  warnInk: "#a37b38",
  warnWash: "rgba(199,154,82,0.18)",
  info: "#0284c7",
  infoInk: "#03628f",
  infoWash: "rgba(2,132,199,0.13)",
  bad: "#d1685a",
  badInk: "#a84537",
  badWash: "rgba(209,104,90,0.16)",
};
...
const num = {
  fontFamily: "'Plus Jakarta Sans', ui-monospace, SFMono-Regular, Menlo, monospace",
...
function Panel({
  ...
  const bg =
    tone === "sky"
      ? `linear-gradient(105deg, ${C.card} 0%, ${C.card} 48%, ${C.skyMist} 100%)`
      : C.card;
  return (
    <Tag
      className={`pas-panel relative overflow-hidden rounded-[20px] ${drift ? "pas-drift" : ""} ${className}`}
      style={{
        background: bg,
        border: `1px solid ${C.line}`,
        boxShadow: "0 1px 2px rgba(15,34,51,0.03), 0 16px 36px rgba(2,132,199,0.06)",
        color: C.ink,
      }}
    >
      {children}
...
function Eyebrow({ children, tone = C.skyDeep }: { children: React.ReactNode; tone?: string }) {
  return (
    <p
...
function PrimaryButton({
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className={`group inline-flex items-center justify-center gap-2 rounded-full px-5 py-2.5 text-[13px] font-semibold transition-all duration-300 hover:brightness-[1.06] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#0284c7] focus-visible:ring-offset-2 focus-visible:ring-offset-[#f2f7fb] active:translate-y-px motion-reduce:transition-none ${className}`}
      style={{
        color: "#ffffff",
        background: `linear-gradient(105deg, ${C.skyHi} 0%, ${C.sky} 55%, ${C.skyDeep} 100%)`,
...
function GhostButton({
  ...
  return (
    <button
      ...
      className={`inline-flex items-center justify-center gap-2 rounded-full px-4 py-2.5 text-[12.5px] font-semibold transition-all duration-300 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#0284c7] focus-visible:ring-offset-2 focus-visible:ring-offset-[#f2f7fb] motion-reduce:transition-none ${className}`}
      style={{
        color: active ? "#ffffff" : C.inkSoft,
        background: active ? C.sky : C.card,
        border: `1px solid ${active ? C.sky : C.line}`,
        ...body,
      }}
    >
      {children}
...
// — Wind-sparkline: luchtige curve die naar rechts uitwaaiert —
function WindLine({ data, tone, id }: { data: number[]; tone: string; id: string }) {
  ...
  return (
    ...
    >
      <defs>
        <linearGradient id={`wind-${id}`} x1="0" y1="0" x2="1" y2="0">
          <stop offset="0%" stopColor={tone} stopOpacity="0.3" />
          <stop offset="100%" stopColor={tone} stopOpacity="1" />
        </linearGradient>
        <linearGradient id={`windfill-${id}`} x1="0" y1="0" x2="0" y2="1">
          <stop offset="0%" stopColor={tone} stopOpacity="0.24" />
          <stop offset="100%" stopColor={tone} stopOpacity="0" />
        </linearGradient>
      </defs>
      <polygon points={`0,${h} ${line} ${w},${h}`} fill={`url(#windfill-${id})`} />
      <polyline
        points={line}
        fill="none"
        stroke={`url(#wind-${id})`}
...
// — Windmeter: dunne wash-lijn met richtingspunt —
function WindMeter({ value, tone = C.sky }: { value: number; tone?: string }) {
  return (
    <span className="hidden items-center gap-2.5 sm:flex" aria-hidden="true">
      ...
      >
        <span
          ...
          style={{
            width: `${value}%`,
            background: `linear-gradient(90deg, ${C.skyHi}, ${tone})`,
  ...
  return (
    <div
      ...
      style={{
        ...body,
        color: C.ink,
        background: `linear-gradient(105deg, ${C.bg} 0%, ${C.bgDeep} 55%, ${C.soft} 100%)`,
      }}
    >
      <style>{`
        @keyframes pasWaft { from { opacity: 0; transform: translateX(-14px); } to { opacity: 1; transform: translateX(0); } }
        .pas-waft > * { animation: pasWaft 0.55s cubic-bezier(0.22, 1, 0.36, 1) both; }
        .pas-waft > *:nth-child(2) { animation-delay: 0.07s; }
        .pas-waft > *:nth-child(3) { animation-delay: 0.14s; }
        .pas-waft > *:nth-child(4) { animation-delay: 0.21s; }
        .pas-drift { transition: transform 0.4s cubic-bezier(0.22, 1, 0.36, 1), box-shadow 0.4s ease; }
        .pas-drift:hover { transform: translateX(3px) translateY(-2px); box-shadow: 0 20px 44px rgba(2,132,199,0.12); }
        @keyframes pasBand { from { transform: translateX(-4%); } to { transform: translateX(4%); } }
        .pas-band { animation: pasBand 14s ease-in-out infinite alternate; }
        @media (prefers-reduced-motion: reduce) {
          .pas-waft > * { animation: none !important; }
          .pas-drift { transition: none !important; }
          .pas-drift:hover { transform: none !important; }
          .pas-band { animation: none !important; }
        }
      `}</style>

      {/* passaat-wash: een lichtband die zacht van links naar rechts waait */}
      <div
        aria-hidden="true"
        className="pas-band pointer-events-none absolute inset-0"
        style={{
          background: `radial-gradient(60% 80% at 10% 20%, ${C.skyMist} 0%, transparent 60%), radial-gradient(50% 70% at 95% 70%, ${C.sandWash} 0%, transparent 60%)`,
        }}
      />
      ...
      <div className="relative mx-auto max-w-6xl px-4 pb-24 sm:px-6 md:px-8">
        ...
        <main key={screen} className="pas-waft pt-7">
          {screen === "dashboard" && (
    ...
    <header className="flex items-center justify-between gap-4 pt-8">
      <div className="flex items-center gap-3.5">
        <span
          ...
          style={{
            background: `linear-gradient(135deg, ${C.skyHi} 0%, ${C.sky} 60%, ${C.skyDeep} 100%)`,
            color: "#ffffff",
        ...
        <div>
          ...
          >
            <Compass size={11} aria-hidden="true" style={{ color: C.sky }} />
            {PROFIEL.plaats} · met de wind mee
          </p>
        </div>
      </div>
      <div className="flex items-center gap-3">
        ...
        >
          ...
          {ongelezen > 0 && (
            <span
              ...
              style={{ background: C.sky, color: "#ffffff", ...num }}
              aria-hidden="true"
            >
              {ongelezen}
            </span>
        ...
        <span
          ...
          style={{ background: C.skyWash, color: C.skyDeep, ...body }}
          aria-hidden="true"
        >
          {PROFIEL.initialen}
        </span>
        ...
        {SCREENS.map((s) => {
          ...
          return (
            <button
              ...
              className="flex shrink-0 items-center gap-2 rounded-full px-4 py-2 text-[12.5px] font-semibold transition-all duration-300 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#0284c7] focus-visible:ring-offset-2 focus-visible:ring-offset-[#ffffff] motion-reduce:transition-none"
              style={{
  ...
  return (
    <div className="space-y-7">
      <section className="grid grid-cols-1 gap-5 lg:grid-cols-[1.4fr_1fr]">
        <Panel className="p-7 md:p-9" tone="sky" drift>
          <div className="relative">
            <Eyebrow>Vandaag · wind mee</Eyebrow>
            ...
            <p className="mt-3 max-w-md text-[14px] leading-relaxed" style={{ color: C.inkSoft }}>
              De wind staat gunstig. Zet vandaag koers naar je eerstvolgende actie en de opdrachten
              die het best bij je passen — licht, luchtig en verifieerbaar onderweg.
            </p>
        ...
        <Panel className="p-7" drift>
          ...
          <div className="mt-5 pt-4" style={{ borderTop: `1px solid ${C.lineSoft}` }}>
            <p className="flex items-center gap-2 text-[12px]" style={{ color: C.inkMute, ...num }}>
              <ShieldCheck size={13} aria-hidden="true" style={{ color: C.sky }} />
      ...
      <section>
        <div className="mb-4">
          <Eyebrow>Koers · deze maand</Eyebrow>
        </div>
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {KPIS.map((k, i) => (
            <Panel key={k.label} className="p-5" drift>
              ...
              <div className="mt-3">
                <WindLine data={k.spark} tone={k.up ? C.sky : C.sandTone} id={`kpi-${i}`} />
              </div>
            </Panel>
          ))}
        </div>
        ...
        <div>
          <div className="mb-4 flex items-baseline justify-between">
            ...
            <button
              type="button"
              onClick={onOpen}
              className="rounded text-[11px] font-semibold uppercase tracking-[0.14em] transition-colors hover:brightness-90 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#0284c7] focus-visible:ring-offset-2 focus-visible:ring-offset-[#f2f7fb]"
              style={{ color: C.skyDeep, ...body }}
            >
              Alle →
            </button>
          </div>
              ...
              {OPDRACHTEN.map((o, i) => (
                <li key={o.id} style={{ borderTop: i === 0 ? "none" : `1px solid ${C.lineSoft}` }}>
                  <button
                    type="button"
                    onClick={onOpen}
                    className="group grid w-full grid-cols-[auto_1fr_auto] items-center gap-4 px-5 py-4 text-left transition-colors hover:bg-[#f4f9fc] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-[#0284c7] motion-reduce:transition-none"
                  >
                    <span
                      className="inline-flex h-10 w-10 items-center justify-center rounded-full"
                      style={{
                        background: i === 0 ? C.skyWash : C.raise,
                        border: `1px solid ${i === 0 ? C.sky : C.line}`,
                      }}
                    >
                      <span
                        className="text-[12px] font-bold leading-none"
                        style={{ color: i === 0 ? C.skyDeep : C.inkMute, ...num }}
                      >
                        {o.match}
                      </span>
                    </span>
                    ...
                    <span className="flex items-center gap-3">
                      <WindMeter value={o.match} tone={o.match >= 90 ? C.sky : C.sandTone} />
                      <ChevronRight
                        size={17}
                        aria-hidden="true"
                        className="transition-transform group-hover:translate-x-0.5 motion-reduce:transition-none"
  ...
  return (
    <div className="space-y-7">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
        ...
        >
          ...
          <input
            ...
            className="w-full bg-transparent text-[13px] outline-none placeholder:text-[#9fb2c0]"
            style={{ color: C.ink, ...body }}
          />
        </div>
      ...
      {loading ? (
        ...
        <Panel className="p-6" tone="sky">
          <div className="relative flex flex-col items-center py-14 text-center">
            <span
              className="inline-flex h-16 w-16 items-center justify-center rounded-full"
              style={{ background: C.skyWash, border: `1px solid ${C.sky}`, color: C.skyDeep }}
              aria-hidden="true"
            >
              <Search size={26} />
            </span>
            ...
            <p className="mx-auto mt-2 max-w-xs text-[13.5px]" style={{ color: C.inkSoft }}>
              Geen opdracht op {q ? `“${q}”` : "je zoekterm"} in deze wind. Verruim je zoekterm en
              probeer opnieuw.
            </p>
            <div className="mt-6">
...
function MarktKaart({
  ...
  const tone = strong ? C.sky : C.sandTone;
  const toneInk = strong ? C.skyDeep : C.sandDeep;
  const wash = strong ? C.skyWash : C.sandWash;
  return (
    <Panel className="p-6" drift>
      ...
      <div className="mt-4 flex items-center gap-3">
        <button
          ...
          onClick={() => setOpen((v) => !v)}
          aria-expanded={open}
          className="inline-flex items-center gap-1.5 rounded-full px-3.5 py-1.5 text-[11.5px] font-semibold transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#0284c7] focus-visible:ring-offset-2 focus-visible:ring-offset-[#ffffff]"
          style={{ color: C.skyDeep, border: `1px solid ${C.line}`, ...body }}
        >
...
function RedenBlok({
  ...
  return (
    <div
      ...
      style={{ background: C.soft, border: `1px solid ${C.lineSoft}` }}
    >
      <p
  ...
  return (
    <div className="space-y-6">
      <button
        type="button"
        onClick={onBack}
        className="inline-flex items-center gap-2 rounded-full px-4 py-2 text-[12.5px] font-semibold transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#0284c7] focus-visible:ring-offset-2 focus-visible:ring-offset-[#f2f7fb]"
      ...
      <Panel className="p-7 md:p-9" tone="sky" drift>
        <div className="relative flex flex-wrap items-center gap-2">
          ...
          >
            <Wind size={11} aria-hidden="true" /> {strong ? "Sterke match" : "Goede match"} ·{" "}
            {opdracht.match}%
          </span>
        </div>
        <h1
      ...
      <Panel className="p-7 md:p-9" tone="sky" drift>
        <div className="relative flex flex-wrap items-center justify-between gap-6">
          <div className="max-w-md">
            <Eyebrow>Verificatie · veilig aan boord</Eyebrow>
            <h1
            ...
            <p className="mt-3 text-[13.5px] leading-relaxed" style={{ color: C.inkSoft }}>
              <span className="font-semibold" style={{ color: C.skyDeep }}>
                {PROFIEL.trust}.
              </span>{" "}
            ...
            return (
              ...
              >
                <button
                  ...
                  className="grid w-full grid-cols-[1fr_auto] items-center gap-4 px-6 py-4 text-left transition-colors hover:bg-[#f4f9fc] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-[#0284c7] motion-reduce:transition-none sm:grid-cols-[1fr_11rem_2.5rem]"
                >
      ...
      <div>
        ...
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
          {DOCUMENTEN.map((d) => {
            ...
            return (
              <Panel key={d.naam} className="flex items-center gap-3 p-4" drift>
                <span
                  className="inline-flex h-10 w-10 items-center justify-center rounded-full"
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-2 max-w-md text-[13.5px]" style={{ color: C.inkSoft }}>
          Zet koers van boven naar beneden — zo blijf je verifieerbaar en betaald, met de wind mee.
        </p>
      </div>
      ...
      <ol className="space-y-3">
        {ACTIES.map((a, i) => {
          ...
          const ink = warn ? C.warnInk : C.skyDeep;
          const wash = warn ? C.warnWash : C.skyWash;
          return (
            <li key={a.titel}>
              <Panel className="p-6" drift>
    ...
    <div className="space-y-6">
      ...
      <section className="grid grid-cols-1 gap-4 sm:grid-cols-3">
        {[
          ...
          <Panel key={s.l} className="p-6" drift>
            <div className="flex items-center justify-between">
              <p
                className="text-[10.5px] font-semibold uppercase tracking-[0.1em]"
                style={{ color: C.inkMute, ...body }}
      ...
      <Panel>
        ...
        <ul>
          {FACTUREN.map((f, i) => {
            ...
            return (
              <li
                key={f.nr}
                className="grid grid-cols-[1fr_auto] items-center gap-3 px-6 py-4 transition-colors hover:bg-[#f4f9fc] sm:grid-cols-[8rem_1fr_5rem_9rem_6rem] sm:gap-4"
                style={{ borderTop: i === 0 ? "none" : `1px solid ${C.lineSoft}` }}
              >
                <span
        ...
        >
          <span
            ...
            style={{ color: C.inkMute, ...body }}
          >
            <Navigation size={12} aria-hidden="true" style={{ color: C.sky }} /> Totaal betaald
          </span>
