...
// Concept 446 — "Macramé" · Geknoopt touwwerk / ambachtelijk textiel.
// Warme jute/zand-neutralen dragen de interface; structurele "knopen" en gevlochten koorden dienen
// als scheidingen en dividers. Tactiele craft-warmte: aards, handgemaakt, rustig. Eén warm
// terracotta/touw-accent (#a9744f). Serif-display + Inter voor body. Animaties respecteren
...
// — Palet: jute, zand, touw-terracotta —
const C = {
  bg: "#f3ebdd",
  bgDeep: "#eadfcb",
  card: "#faf4e9",
  ...
  lineSoft: "rgba(58,47,34,0.08)",
  // touw-terracotta accent
  rope: "#a9744f",
  ropeDeep: "#875839",
  ropeWash: "rgba(169,116,79,0.14)",
  ropeMist: "rgba(169,116,79,0.07)",
  ropeLine: "rgba(169,116,79,0.45)",
  // status
  ok: "#78904f",
  okInk: "#556734",
  okWash: "rgba(120,144,79,0.16)",
  warn: "#c08a35",
  warnInk: "#8f631f",
  warnWash: "rgba(192,138,53,0.18)",
  info: "#6e8a86",
  infoInk: "#4c6763",
  infoWash: "rgba(110,138,134,0.16)",
  bad: "#b1573f",
  badInk: "#873d2a",
  badWash: "rgba(177,87,63,0.16)",
};
...
// Jute-textuur: fijne diagonale vezels op een warme grondkleur.
function jute(color: string): React.CSSProperties {
  return {
    backgroundColor: color,
    backgroundImage:
      "repeating-linear-gradient(45deg, rgba(58,47,34,0.03) 0px, rgba(58,47,34,0.03) 1px, transparent 1px, transparent 4px)," +
      "repeating-linear-gradient(-45deg, rgba(58,47,34,0.025) 0px, rgba(58,47,34,0.025) 1px, transparent 1px, transparent 4px)",
  };
}

// Gevlochten koord als horizontale divider (touw-knopenrij).
function RopeDivider({ tone = C.ropeLine }: { tone?: string }) {
  return (
    <div
      aria-hidden="true"
      className="h-[6px] w-full"
      style={{
        backgroundImage: `repeating-linear-gradient(90deg, ${tone} 0px, ${tone} 5px, transparent 5px, transparent 7px, ${tone} 7px, ${tone} 12px, transparent 12px, transparent 14px)`,
        opacity: 0.55,
      }}
    />
  );
...
// — Textiel-paneel: jute-grondkleur, zachte rand, warme schaduw —
function Weef({
  children,
  className = "",
  as: Tag = "div",
  ...
  return (
    <Tag
      ...
      style={{
        ...jute(base),
        border: `1px solid ${knot ? C.ropeLine : C.line}`,
        boxShadow: knot
          ? "0 8px 24px -14px rgba(135,88,57,0.35), inset 0 1px 0 rgba(255,255,255,0.5)"
          : "0 6px 20px -16px rgba(58,47,34,0.4), inset 0 1px 0 rgba(255,255,255,0.45)",
        color: C.ink,
      }}
    >
      {children}
...
function Eyebrow({ children, tone = C.ropeDeep }: { children: React.ReactNode; tone?: string }) {
  return (
    <p
      className="flex items-center gap-2 text-[10px] font-semibold uppercase tracking-[0.22em]"
      style={{ color: tone, ...bodyFont }}
    >
      <Link2 size={11} aria-hidden="true" strokeWidth={2.2} />
      {children}
    </p>
  );
}
...
function PrimaryButton({
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className={`group inline-flex items-center justify-center gap-2 rounded-full px-5 py-2.5 text-[13px] font-semibold text-[#faf4e9] transition-all duration-200 hover:brightness-[1.06] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#a9744f] focus-visible:ring-offset-2 focus-visible:ring-offset-[#f3ebdd] active:translate-y-px motion-reduce:transition-none ${className}`}
      style={{
        background: `linear-gradient(180deg, ${C.rope}, ${C.ropeDeep})`,
        border: `1px solid ${C.ropeDeep}`,
        boxShadow: "0 6px 16px -8px rgba(135,88,57,0.5)",
        ...bodyFont,
      }}
    >
      {children}
...
function GhostButton({
  ...
  return (
    <button
      ...
      className={`inline-flex items-center justify-center gap-2 rounded-full px-4 py-2.5 text-[12.5px] font-semibold transition-all duration-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#a9744f] focus-visible:ring-offset-2 focus-visible:ring-offset-[#f3ebdd] motion-reduce:transition-none ${className}`}
      style={{
        color: active ? C.ropeDeep : C.inkSoft,
        background: active ? C.ropeWash : C.card,
        border: `1px solid ${active ? C.ropeLine : C.line}`,
        ...bodyFont,
      }}
    >
      {children}
...
// — Koord-sparkline: warme, licht golvende draad —
function KoordLine({ data, tone, id }: { data: number[]; tone: string; id: string }) {
  ...
  return (
    ...
    >
      <defs>
        <linearGradient id={`mac-${id}`} x1="0" y1="0" x2="0" y2="1">
          <stop offset="0%" stopColor={tone} stopOpacity="0.24" />
          <stop offset="100%" stopColor={tone} stopOpacity="0" />
        </linearGradient>
      </defs>
      <polygon points={`0,${h} ${line} ${w},${h}`} fill={`url(#mac-${id})`} />
      <polyline
        points={line}
        fill="none"
        stroke={tone}
      ...
      />
      <circle cx={last[0]} cy={last[1]} r="2.8" fill={C.card} stroke={tone} strokeWidth="1.8" />
    </svg>
  );
}

function KoordMeter({ value, tone = C.rope }: { value: number; tone?: string }) {
  return (
    <span className="hidden items-center gap-2.5 sm:flex" aria-hidden="true">
      ...
      >
        <span
          ...
          style={{
            width: `${value}%`,
            background: `linear-gradient(90deg, ${C.ropeDeep}, ${tone})`,
...
// Decoratief macramé-knopenpatroon op de achtergrond (subtiel, aria-verborgen).
function KnoopVeld() {
  return (
    <svg
      aria-hidden="true"
    ...
    >
      <defs>
        <pattern id="macrame-knoop" width="44" height="44" patternUnits="userSpaceOnUse">
          <path
            d="M0 22 Q11 8 22 22 Q33 36 44 22 M22 0 Q8 11 22 22 Q36 33 22 44"
            fill="none"
            stroke="rgba(58,47,34,0.05)"
            strokeWidth="1.4"
          />
        </pattern>
      </defs>
      <rect width="100%" height="100%" fill="url(#macrame-knoop)" />
    </svg>
  );
}

export function Concept446() {
  ...
  return (
    <div
      ...
      style={{ ...bodyFont, color: C.ink, ...jute(C.bg) }}
    >
      <KnoopVeld />
      <style>{`
        @keyframes macrameRise { from { opacity: 0; transform: translateY(8px); } to { opacity: 1; transform: translateY(0); } }
        .macrame-rise { animation: macrameRise 0.5s cubic-bezier(0.22, 1, 0.36, 1) both; }
        @media (prefers-reduced-motion: reduce) { .macrame-rise { animation: none !important; } }
      `}</style>

      <div className="relative mx-auto max-w-6xl px-4 pb-24 sm:px-6 md:px-8">
        <TopBar />
        <NavBar screen={screen} setScreen={setScreen} />
        <main key={screen} className="macrame-rise pt-7">
          {screen === "dashboard" && (
  ...
  return (
    <header className="flex items-center justify-between gap-4 pt-8">
      <div className="flex items-center gap-3.5">
        <span
          className="inline-flex h-11 w-11 items-center justify-center rounded-2xl text-[#faf4e9]"
          style={{
            background: `linear-gradient(160deg, ${C.rope}, ${C.ropeDeep})`,
            boxShadow: "0 6px 16px -8px rgba(135,88,57,0.55)",
          }}
          aria-hidden="true"
        >
          <Cable size={18} strokeWidth={2.1} />
        </span>
        <div>
          <p
          ...
          <p className="mt-1.5 text-[11px] leading-none" style={{ color: C.inkMute }}>
            {PROFIEL.plaats} · geknoopt & ambachtelijk
          </p>
        </div>
      </div>
      <div className="flex items-center gap-3">
        ...
        >
          ...
          {ongelezen > 0 && (
            <span
              className="absolute -right-0.5 -top-0.5 flex h-4 w-4 items-center justify-center rounded-full text-[9px] font-bold text-[#faf4e9]"
              style={{ background: C.rope, ...num }}
              aria-hidden="true"
            >
              {ongelezen}
            </span>
        ...
        <span
          ...
          style={{ background: C.ropeWash, border: `1px solid ${C.ropeLine}`, color: C.ropeDeep }}
          aria-hidden="true"
        >
          {PROFIEL.initialen}
        </span>
        ...
        {SCREENS.map((s) => {
          ...
          return (
            <button
              ...
              className="flex shrink-0 items-center gap-2 rounded-full px-4 py-2 text-[12.5px] font-semibold transition-all duration-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#a9744f] focus-visible:ring-offset-2 focus-visible:ring-offset-[#faf4e9] motion-reduce:transition-none"
              style={{
                color: on ? "#faf4e9" : C.inkMute,
                background: on
                  ? `linear-gradient(180deg, ${C.rope}, ${C.ropeDeep})`
                  : "transparent",
                ...bodyFont,
              }}
            >
  ...
  return (
    <div className="space-y-7">
      <section className="grid grid-cols-1 gap-5 lg:grid-cols-[1.4fr_1fr]">
        <Weef className="p-7 md:p-9" fill="rope">
          <Eyebrow>Vandaag · rustig geknoopt</Eyebrow>
          ...
          <p className="mt-3 max-w-md text-[14px] leading-relaxed" style={{ color: C.inkSoft }}>
            Je praktijk als handgemaakt macramé: knoop voor knoop op orde, verifieerbaar en betaald.
            Loop rustig langs je acties — alles hangt strak in het touw.
          </p>
      ...
      <section>
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {KPIS.map((k, i) => (
            <Weef key={k.label} className="p-5">
              ...
              <div className="mt-3">
                <KoordLine data={k.spark} tone={k.up ? C.rope : C.info} id={`k446-${i}`} />
              </div>
            </Weef>
          ))}
        </div>
        ...
        <div>
          <div className="mb-4 flex items-baseline justify-between">
            ...
            <button
              type="button"
              onClick={onOpen}
              className="rounded text-[11px] font-semibold uppercase tracking-[0.14em] transition-colors hover:brightness-90 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#a9744f] focus-visible:ring-offset-2 focus-visible:ring-offset-[#f3ebdd]"
              style={{ color: C.ropeDeep }}
            >
              Alle →
            </button>
          </div>
              ...
              {OPDRACHTEN.map((o, i) => (
                <li key={o.id} style={{ borderTop: i === 0 ? "none" : `1px solid ${C.lineSoft}` }}>
                  <button
                    type="button"
                    onClick={onOpen}
                    className="group grid w-full grid-cols-[auto_1fr_auto] items-center gap-4 px-5 py-4 text-left transition-colors hover:bg-[#f0e6d4] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-[#a9744f] motion-reduce:transition-none"
                  >
                    <span
                      className="inline-flex h-10 w-10 items-center justify-center rounded-full"
                      style={{
                        background: i === 0 ? C.ropeWash : C.raise,
                        border: `1px solid ${i === 0 ? C.ropeLine : C.line}`,
                      }}
                    >
                      <span
                        className="text-[12px] font-bold leading-none"
                        style={{ color: i === 0 ? C.ropeDeep : C.inkMute, ...num }}
                      >
                        {o.match}
                      </span>
                    </span>
                    ...
                    <span className="flex items-center gap-3">
                      <KoordMeter value={o.match} tone={o.match >= 90 ? C.rope : C.info} />
                      <ChevronRight
                        size={17}
                        aria-hidden="true"
                        className="transition-transform group-hover:translate-x-0.5 motion-reduce:transition-none"
  ...
  return (
    <div className="space-y-7">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
        ...
        >
          ...
          <input
            ...
            className="w-full bg-transparent text-[13px] outline-none placeholder:text-[#b6a281]"
      ...
      {loading ? (
        ...
        <Weef className="p-6" knot>
          <div className="flex flex-col items-center py-14 text-center">
            <span
              className="inline-flex h-16 w-16 items-center justify-center rounded-full"
              style={{ background: C.card, border: `1px solid ${C.ropeLine}`, color: C.ropeDeep }}
              aria-hidden="true"
            >
              <Search size={26} />
            </span>
...
function MarktKaart({
  ...
  const tone = strong ? C.rope : C.info;
  const toneInk = strong ? C.ropeDeep : C.infoInk;
  return (
    <Weef className="p-6" knot={strong}>
      <div className="grid grid-cols-[1fr_auto] items-start gap-4">
        ...
        <div className="flex flex-col items-center gap-1.5">
          <span
            ...
            style={{ background: strong ? C.ropeWash : C.infoWash, border: `1.5px solid ${tone}` }}
          >
      ...
      <div className="mt-4 flex items-center gap-3">
        <button
          ...
          onClick={() => setOpen((v) => !v)}
          aria-expanded={open}
          className="inline-flex items-center gap-1.5 rounded-full px-3.5 py-1.5 text-[11.5px] font-semibold transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#a9744f] focus-visible:ring-offset-2 focus-visible:ring-offset-[#faf4e9]"
          style={{ color: C.ropeDeep, border: `1px solid ${C.line}`, ...bodyFont }}
        >
  ...
  return (
    <div className="space-y-6">
      <button
        type="button"
        onClick={onBack}
        className="inline-flex items-center gap-2 rounded-full px-4 py-2 text-[12.5px] font-semibold transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#a9744f] focus-visible:ring-offset-2 focus-visible:ring-offset-[#f3ebdd]"
      ...
      <Weef className="p-7 md:p-9" fill="rope" knot={strong}>
        <div className="flex flex-wrap items-center gap-2">
          <span
            className="inline-flex items-center rounded-full px-2.5 py-0.5 text-[10.5px] font-semibold"
            style={{ color: C.inkMute, border: `1px solid ${C.line}`, ...num }}
          ...
          <span
            className="inline-flex items-center gap-1 rounded-full px-2.5 py-0.5 text-[11px] font-bold text-[#faf4e9]"
            style={{ background: `linear-gradient(180deg, ${C.rope}, ${C.ropeDeep})` }}
          >
            <Link2 size={11} aria-hidden="true" /> {strong ? "Sterke match" : "Goede match"} ·{" "}
            {opdracht.match}%
          </span>
        </div>
        <h1
        ...
        <div className="flex flex-wrap items-center justify-between gap-6">
          <div className="max-w-md">
            ...
            <p className="mt-3 text-[13.5px] leading-relaxed" style={{ color: C.inkSoft }}>
              <span className="font-semibold" style={{ color: C.ropeDeep }}>
                {PROFIEL.trust}.
              </span>{" "}
            ...
            <div className="mt-4 max-w-xs">
              <KoordMeter value={ratio} tone={C.rope} />
            </div>
          </div>
          <span
            className="inline-flex h-24 w-24 flex-col items-center justify-center rounded-full"
            style={{ background: C.card, border: `1.5px solid ${C.ropeLine}` }}
          >
            <span
              className="text-[26px] font-semibold leading-none"
              style={{ color: C.ropeDeep, ...num }}
            >
              {ratio}
            </span>
            <span
            ...
            return (
              ...
              >
                <button
                  ...
                  className="grid w-full grid-cols-[1fr_auto] items-center gap-4 px-6 py-4 text-left transition-colors hover:bg-[#f0e6d4] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-[#a9744f] motion-reduce:transition-none sm:grid-cols-[1fr_11rem_2.5rem]"
                >
      ...
      <div>
        ...
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
          {DOCUMENTEN.map((d) => {
            ...
            return (
              <Weef key={d.naam} className="flex items-center gap-3 p-4">
                <span
                  className="inline-flex h-10 w-10 items-center justify-center rounded-full"
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-2 max-w-md text-[13.5px]" style={{ color: C.inkSoft }}>
          Rustig van boven naar beneden — zo blijf je verifieerbaar en betaald, knoop na knoop.
        </p>
      </div>
      ...
      <ol className="space-y-3">
        {ACTIES.map((a, i) => {
          ...
          const tone = warn ? C.warn : C.rope;
          const ink = warn ? C.warnInk : C.ropeDeep;
          const wash = warn ? C.warnWash : C.ropeWash;
          return (
            <li key={a.titel}>
              <Weef className="p-6" knot={warn}>
  ...
  return (
    <div className="space-y-6">
      ...
      <section className="grid grid-cols-1 gap-4 sm:grid-cols-3">
        {[
          { l: "Betaald (mnd)", v: totaalBetaald, sub: "3 voldaan", alarm: false, knot: false },
          { l: "Openstaand", v: "€ 1.350", sub: "1 factuur · 9 dagen", alarm: true, knot: true },
          { l: "Concept", v: "€ 880", sub: "klaar om te versturen", alarm: false, knot: false },
        ].map((s) => (
          <Weef key={s.l} className="p-6" knot={s.knot}>
            <div className="flex items-center justify-between">
              <p
                className="text-[10px] font-semibold uppercase tracking-[0.1em]"
                style={{ color: C.inkMute }}
        ...
        <ul>
          {FACTUREN.map((f, i) => {
            ...
            return (
              <li
                key={f.nr}
                className="grid grid-cols-[1fr_auto] items-center gap-3 px-6 py-4 transition-colors hover:bg-[#f0e6d4] sm:grid-cols-[8rem_1fr_5rem_9rem_6rem] sm:gap-4"
                style={{ borderTop: i === 0 ? "none" : `1px solid ${C.lineSoft}` }}
              >
                <span
