...
// Concept 435 — "Craquelé" · Craquelé-glazuur / fijne haarscheur-textuur (craft, anti-polish).
// Verouderd porselein met een fijn netwerk van haarscheuren als achtergrondtextuur op panelen:
// ambachtelijk, met-de-hand-gemaakt, warm en tactiel. Porselein-crème (#f3efe7) op inkt (#221f1a),
// koel-grijze craquelé-lijnen (#3f4a55) en een terracotta-accent (#b5654a). Imperfect-als-ambacht,
// hoog tekstcontrast. Micro-interacties respecteren reduced-motion.
...
// — Palet: porselein-crème, inkt, koel-grijze craquelé, terracotta —
const C = {
  bg: "#f3efe7",
  bgDeep: "#ebe4d8",
  panel: "#f8f4ec",
  ...
  lineSoft: "rgba(34,31,26,0.08)",
  craquele: "#3f4a55",
  terra: "#b5654a",
  terraHi: "#c8795d",
  terraDeep: "#8c4530",
  terraWash: "rgba(181,101,74,0.13)",
  terraMist: "rgba(181,101,74,0.06)",
  ok: "#5f7a45",
  okInk: "#45602f",
  okWash: "rgba(95,122,69,0.15)",
  info: "#4f5d6b",
  infoInk: "#3a4653",
  infoWash: "rgba(63,74,85,0.12)",
  warn: "#b5654a",
  warnInk: "#8c4530",
  warnWash: "rgba(181,101,74,0.15)",
  bad: "#a3432f",
  badInk: "#872f1f",
  badWash: "rgba(163,67,47,0.14)",
};
...
// — Craquelé-textuur: fijn netwerk van haarscheuren als zachte achtergrondlaag op panelen —
function Craquele({ opacity = 0.5 }: { opacity?: number }) {
  return (
    <svg
      aria-hidden="true"
      ...
      style={{ opacity }}
    >
      <g fill="none" stroke={C.craquele} strokeWidth="0.5" strokeOpacity="0.16">
        <path d="M0 34 L48 40 L92 26 L140 44 L200 30" />
        <path d="M48 40 L58 92 L34 132 L52 200" />
        <path d="M92 26 L104 78 L88 120 L110 168 L96 200" />
        <path d="M140 44 L132 96 L162 140 L150 200" />
        <path d="M0 96 L58 92 L104 78 L132 96 L200 82" />
        <path d="M0 150 L34 132 L88 120 L162 140 L200 128" />
        <path d="M58 92 L20 118 L0 116" />
        <path d="M104 78 L128 58 L140 44" />
        <path d="M162 140 L188 172 L200 176" />
        <path d="M34 132 L14 168 L26 200" />
        <path d="M88 120 L72 156 L88 200" />
        <path d="M132 96 L172 108 L200 100" />
      </g>
    </svg>
  );
}
...
function Panel({
  ...
  className?: string;
  as?: "div" | "section" | "li";
  tone?: "panel" | "accent" | "deep";
  crackle?: boolean;
}) {
  const bg =
    tone === "accent"
      ? `radial-gradient(130% 150% at 12% 0%, ${C.panel} 0%, ${C.panelSoft} 50%, ${C.terraMist} 100%)`
      : tone === "deep"
        ? C.panelSoft
        : C.panel;
  return (
    <Tag
      ...
      style={{
        ...
        boxShadow:
          "0 1px 2px rgba(34,31,26,0.05), 0 10px 26px rgba(60,52,40,0.07), inset 0 1px 0 rgba(255,255,255,0.55)",
        color: C.ink,
      }}
    >
      {crackle && <Craquele opacity={0.55} />}
      <span className="relative block">{children}</span>
    </Tag>
  );
}

function Eyebrow({ children, tone = C.terraDeep }: { children: React.ReactNode; tone?: string }) {
  return (
    <p
...
function PrimaryButton({
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className={`group inline-flex items-center justify-center gap-2 rounded-full px-5 py-2.5 text-[13px] font-semibold transition-all duration-300 hover:brightness-[1.05] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#b5654a] focus-visible:ring-offset-2 focus-visible:ring-offset-[#f3efe7] active:translate-y-px motion-reduce:transition-none ${className}`}
      style={{
        color: "#faf5ee",
        background: `linear-gradient(135deg, ${C.terraHi} 0%, ${C.terra} 55%, ${C.terraDeep} 100%)`,
        boxShadow: "0 6px 16px rgba(140,69,48,0.24)",
        ...bodyFont,
      }}
    >
      {children}
...
function GhostButton({
  ...
  return (
    <button
      ...
      className={`inline-flex items-center justify-center gap-2 rounded-full px-4 py-2.5 text-[12.5px] font-semibold transition-all duration-300 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#b5654a] focus-visible:ring-offset-2 focus-visible:ring-offset-[#f3efe7] motion-reduce:transition-none ${className}`}
      style={{
        color: active ? "#faf5ee" : C.inkSoft,
        background: active ? C.terra : C.panel,
        border: `1px solid ${active ? C.terra : C.line}`,
        ...bodyFont,
      }}
    >
      {children}
...
// — Glazuur-sparkline: fijne curve met terracotta-eindpunt —
function GlazuurLine({ data, tone, id }: { data: number[]; tone: string; id: string }) {
  ...
  return (
    ...
    >
      <defs>
        <linearGradient id={`glazuur-${id}`} x1="0" y1="0" x2="0" y2="1">
          <stop offset="0%" stopColor={tone} stopOpacity="0.28" />
          <stop offset="100%" stopColor={tone} stopOpacity="0" />
        </linearGradient>
      </defs>
      <polygon points={`0,${h} ${line} ${w},${h}`} fill={`url(#glazuur-${id})`} />
      <polyline
        points={line}
        fill="none"
        stroke={tone}
      ...
      />
      <circle cx={last[0]} cy={last[1]} r="3.2" fill={tone} opacity="0.22" />
      <circle cx={last[0]} cy={last[1]} r="2.2" fill={C.panel} stroke={tone} strokeWidth="1.5" />
    </svg>
  );
}

// — Glazuur-meter: voortgangsbalk met terracotta-verloop —
function GlazuurMeter({ value, tone = C.terra }: { value: number; tone?: string }) {
  return (
    <span className="hidden items-center gap-2.5 sm:flex" aria-hidden="true">
      <span
        ...
        style={{ background: C.raise, boxShadow: "inset 0 1px 2px rgba(34,31,26,0.12)" }}
      >
        <span
          className="block h-full rounded-full"
          style={{
            width: `${value}%`,
            background: `linear-gradient(90deg, ${C.terraHi}, ${tone})`,
  ...
  return (
    <div
      ...
      style={{
        ...bodyFont,
        color: C.ink,
        background: `radial-gradient(130% 100% at 20% -10%, ${C.bg} 0%, ${C.bgDeep} 72%, ${C.panelSoft} 100%)`,
      }}
    >
      <style>{`
        ...
        @media (prefers-reduced-motion: reduce) { .craq-rise { animation: none !important; } }
      `}</style>

      {/* fijne craquelé-textuur over de hele achtergrond */}
      <div className="pointer-events-none absolute inset-0" aria-hidden="true">
        <Craquele opacity={0.4} />
      </div>
    ...
    <header className="flex items-center justify-between gap-4 pt-8">
      <div className="flex items-center gap-3.5">
        <span
          ...
          style={{
            background: `radial-gradient(circle at 34% 28%, ${C.terraHi} 0%, ${C.terra} 58%, ${C.terraDeep} 100%)`,
            border: `1px solid ${C.terraDeep}`,
            color: "#faf5ee",
            boxShadow: "0 4px 12px rgba(140,69,48,0.28), inset 0 1px 2px rgba(255,255,255,0.35)",
          }}
          aria-hidden="true"
        >
          <Feather size={18} />
        ...
        <div>
          ...
          >
            ...
            />
            {PROFIEL.plaats} · met zorg bewaard
          </p>
        </div>
      </div>
        ...
        >
          ...
          {ongelezen > 0 && (
            <span
              ...
              style={{ background: C.terra, color: "#faf5ee", ...num }}
              aria-hidden="true"
            >
              {ongelezen}
            </span>
        ...
        {SCREENS.map((s) => {
          ...
          return (
            <button
              ...
              className="flex shrink-0 items-center gap-2 rounded-full px-4 py-2 text-[12.5px] font-semibold transition-all duration-300 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#b5654a] focus-visible:ring-offset-2 focus-visible:ring-offset-[#f8f4ec] motion-reduce:transition-none"
              style={{
                color: on ? "#faf5ee" : C.inkMute,
  ...
  return (
    <div className="space-y-7">
      <section className="grid grid-cols-1 gap-5 lg:grid-cols-[1.4fr_1fr]">
        <Panel className="p-7 md:p-9" tone="accent">
          <div className="relative">
            <Eyebrow>Vandaag · atelier</Eyebrow>
            ...
            <p className="mt-3 max-w-md text-[14px] leading-relaxed" style={{ color: C.inkSoft }}>
              Alles ligt met zorg en veilig bewaard — als geglazuurd porselein, elk stuk uniek. Loop
              rustig langs je acties en houd je praktijk verifieerbaar en betaald, ambachtelijk en
              zonder haast.
            </p>
        ...
        <Panel className="p-7">
          ...
          <div className="mt-5 pt-4" style={{ borderTop: `1px solid ${C.lineSoft}` }}>
            <p className="flex items-center gap-2 text-[12px]" style={{ color: C.inkMute, ...num }}>
              <Feather size={13} aria-hidden="true" style={{ color: C.terra }} />
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {KPIS.map((k, i) => (
            <Panel key={k.label} className="p-5">
              ...
              <div className="mt-3">
                <GlazuurLine data={k.spark} tone={k.up ? C.terra : C.craquele} id={`kpi-${i}`} />
              </div>
            </Panel>
          ))}
        </div>
        ...
        <div>
          <div className="mb-4 flex items-baseline justify-between">
            ...
            <button
              type="button"
              onClick={onOpen}
              className="rounded text-[11px] font-semibold uppercase tracking-[0.14em] transition-colors hover:brightness-90 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#b5654a] focus-visible:ring-offset-2 focus-visible:ring-offset-[#f3efe7]"
              style={{ color: C.terraDeep, ...bodyFont }}
            >
              Alle →
            </button>
          </div>
              ...
              {OPDRACHTEN.map((o, i) => (
                <li key={o.id} style={{ borderTop: i === 0 ? "none" : `1px solid ${C.lineSoft}` }}>
                  <button
                    type="button"
                    onClick={onOpen}
                    className="group grid w-full grid-cols-[auto_1fr_auto] items-center gap-4 px-5 py-4 text-left transition-colors hover:bg-[#efe9dc] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-[#b5654a] motion-reduce:transition-none"
                  >
                    <span
                      className="inline-flex h-10 w-10 items-center justify-center rounded-full"
                      style={{
                        background: i === 0 ? C.terraWash : C.raise,
                        border: `1px solid ${i === 0 ? C.terra : C.line}`,
                      }}
                    >
                      <span
                        className="text-[12px] font-bold leading-none"
                        style={{ color: i === 0 ? C.terraDeep : C.inkMute, ...num }}
                      >
                        {o.match}
                      </span>
                    </span>
                    ...
                    <span className="flex items-center gap-3">
                      <GlazuurMeter value={o.match} tone={o.match >= 90 ? C.terra : C.craquele} />
                      <ChevronRight
                        size={17}
                        aria-hidden="true"
                        className="transition-transform group-hover:translate-x-0.5 motion-reduce:transition-none"
  ...
  return (
    <div className="space-y-7">
      ...
      {loading ? (
        ...
        <Panel className="p-6" tone="accent">
...
function MarktKaart({
  ...
  const tone = strong ? C.terra : C.craquele;
  const toneInk = strong ? C.terraDeep : C.infoInk;
  return (
    <Panel className="p-6">
      <div className="grid grid-cols-[1fr_auto] items-start gap-4">
        ...
        <div className="flex flex-col items-center gap-1.5">
          <span
            ...
            style={{
              background: `radial-gradient(circle at 34% 28%, ${C.panel} 0%, ${strong ? C.terraWash : C.raise} 100%)`,
      ...
      <div className="mt-4 flex items-center gap-3">
        <button
          ...
          onClick={() => setOpen((v) => !v)}
          aria-expanded={open}
          className="inline-flex items-center gap-1.5 rounded-full px-3.5 py-1.5 text-[11.5px] font-semibold transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#b5654a] focus-visible:ring-offset-2 focus-visible:ring-offset-[#f8f4ec]"
          style={{ color: C.terraDeep, border: `1px solid ${C.line}`, ...bodyFont }}
        >
  ...
  return (
    <div className="space-y-6">
      <button
        type="button"
        onClick={onBack}
        className="inline-flex items-center gap-2 rounded-full px-4 py-2 text-[12.5px] font-semibold transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#b5654a] focus-visible:ring-offset-2 focus-visible:ring-offset-[#f3efe7]"
        style={{
          color: C.inkSoft,
          border: `1px solid ${C.line}`,
          background: C.panel,
      ...
      <Panel className="p-7 md:p-9" tone="accent">
        <div className="relative flex flex-wrap items-center gap-2">
          ...
          <span
            ...
            style={{ color: "#faf5ee", background: tone, ...bodyFont }}
          >
            <Feather size={11} aria-hidden="true" /> {strong ? "Sterke match" : "Goede match"} ·{" "}
            {opdracht.match}%
          </span>
        </div>
        <h1
      ...
      <Panel className="p-7 md:p-9" tone="accent">
        <div className="relative flex flex-wrap items-center justify-between gap-6">
          <div className="max-w-md">
            <Eyebrow>Verificatie · met zorg bewaard</Eyebrow>
            <h1
            ...
            <p className="mt-3 text-[13.5px] leading-relaxed" style={{ color: C.inkSoft }}>
              <span className="font-semibold" style={{ color: C.terraDeep }}>
                {PROFIEL.trust}.
              </span>{" "}
            ...
            <div className="mt-4 max-w-xs">
              <GlazuurMeter value={ratio} tone={C.terra} />
            </div>
          </div>
          <span
            className="relative inline-flex h-24 w-24 flex-col items-center justify-center rounded-full"
            style={{
              background: `radial-gradient(circle at 34% 28%, ${C.panel} 0%, ${C.terraWash} 100%)`,
              border: `1.5px solid ${C.terra}`,
            }}
          >
            <span
              className="text-[26px] font-semibold leading-none"
              style={{ color: C.terraDeep, ...num }}
            >
              {ratio}
            </span>
            <span
            ...
            return (
              ...
              >
                <button
                  ...
                  className="grid w-full grid-cols-[1fr_auto] items-center gap-4 px-6 py-4 text-left transition-colors hover:bg-[#efe9dc] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-[#b5654a] motion-reduce:transition-none sm:grid-cols-[1fr_11rem_2.5rem]"
                >
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-2 max-w-md text-[13.5px]" style={{ color: C.inkSoft }}>
          Rustig van boven naar beneden — zo blijf je verifieerbaar en betaald, ambachtelijk en op
          orde.
        </p>
      </div>
      ...
      <Panel>
        ...
        <ul>
          {FACTUREN.map((f, i) => {
            ...
            return (
              <li
                key={f.nr}
                className="grid grid-cols-[1fr_auto] items-center gap-3 px-6 py-4 transition-colors hover:bg-[#efe9dc] sm:grid-cols-[8rem_1fr_5rem_9rem_6rem] sm:gap-4"
                style={{ borderTop: i === 0 ? "none" : `1px solid ${C.lineSoft}` }}
              >
                <span
        ...
        >
          <span
            ...
            style={{ color: C.inkMute, ...bodyFont }}
          >
            <Feather size={12} aria-hidden="true" style={{ color: C.terra }} /> Totaal betaald
          </span>
