...
// Concept 445 — "Gletsjer" · Gelaagd gletsjerijs.
// Koele blauw-witte strata: horizontale ijs-lagen als banden, gebarsten-ijs hairlines en een
// glasachtige helderheid dragen de interface. Fris, koud, kristalhelder, licht. Eén koel-cyaan
// accent (#3aa7d4). Displayfont strak grotesk, cijfers tabulair. Animaties respecteren
...
const C = {
  ...
  inkMute: "#6b8593",
  inkFaint: "#9db6c3",
  line: "rgba(18,36,48,0.13)",
  lineSoft: "rgba(18,36,48,0.07)",
  crack: "rgba(58,167,212,0.28)",
  // cyaan accent
  cyan: "#3aa7d4",
  cyanDeep: "#1f7ba6",
  cyanWash: "rgba(58,167,212,0.13)",
  cyanMist: "rgba(58,167,212,0.06)",
  cyanLine: "rgba(58,167,212,0.4)",
  // status
  ok: "#2fa37a",
  okInk: "#1c7a58",
  okWash: "rgba(47,163,122,0.14)",
  warn: "#c98a2b",
  warnInk: "#9a6714",
  warnWash: "rgba(201,138,43,0.16)",
  info: "#3a86d4",
  infoInk: "#22609f",
  infoWash: "rgba(58,134,212,0.13)",
  bad: "#cf5a44",
  badInk: "#9c3b2a",
  badWash: "rgba(207,90,68,0.14)",
};
...
// Gelaagd ijs-oppervlak: horizontale strata + glasachtige lichtval bovenaan.
function ice(color: string): React.CSSProperties {
  return {
    backgroundColor: color,
    backgroundImage:
      "linear-gradient(180deg, rgba(255,255,255,0.7) 0%, rgba(255,255,255,0) 40%)," +
      "repeating-linear-gradient(180deg, rgba(58,167,212,0.05) 0px, rgba(58,167,212,0.05) 1px, transparent 1px, transparent 14px)",
  };
}
...
// — IJs-paneel: glasachtig, gelaagd, met een fijne cyaan-hairline (barst) —
function Pane({
  children,
  className = "",
  as: Tag = "div",
  ...
  return (
    <Tag
      ...
      style={{
        ...ice(base),
        border: `1px solid ${accent ? C.cyanLine : C.line}`,
        boxShadow: accent
          ? "0 10px 30px -16px rgba(58,167,212,0.4), inset 0 1px 0 rgba(255,255,255,0.9)"
          : "0 8px 22px -18px rgba(18,36,48,0.4), inset 0 1px 0 rgba(255,255,255,0.85)",
        color: C.ink,
      }}
    >
      {children}
...
function Eyebrow({ children, tone = C.cyanDeep }: { children: React.ReactNode; tone?: string }) {
  return (
    <p
      className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-[0.24em]"
      style={{ color: tone, ...bodyFont }}
    >
      <Snowflake size={11} aria-hidden="true" strokeWidth={2.2} />
      {children}
    </p>
  );
}
...
function PrimaryButton({
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className={`group inline-flex items-center justify-center gap-2 rounded-full px-5 py-2.5 text-[13px] font-bold text-white transition-all duration-200 hover:brightness-105 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#3aa7d4] focus-visible:ring-offset-2 focus-visible:ring-offset-[#eef5f9] active:translate-y-px motion-reduce:transition-none ${className}`}
      style={{
        background: `linear-gradient(180deg, ${C.cyan}, ${C.cyanDeep})`,
        border: `1px solid ${C.cyanDeep}`,
        boxShadow: "0 8px 18px -8px rgba(58,167,212,0.55)",
        ...bodyFont,
      }}
    >
      {children}
...
function GhostButton({
  ...
  return (
    <button
      ...
      className={`inline-flex items-center justify-center gap-2 rounded-full px-4 py-2.5 text-[12.5px] font-bold transition-all duration-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#3aa7d4] focus-visible:ring-offset-2 focus-visible:ring-offset-[#eef5f9] motion-reduce:transition-none ${className}`}
      style={{
        color: active ? C.cyanDeep : C.inkSoft,
        background: active ? C.cyanWash : C.glass,
        border: `1px solid ${active ? C.cyanLine : C.line}`,
        ...bodyFont,
      }}
    >
      {children}
...
// — IJs-sparkline: koele lijn met glinstering —
function IceLine({ data, tone, id }: { data: number[]; tone: string; id: string }) {
  ...
  return (
    ...
    >
      ...
      />
      <circle cx={last[0]} cy={last[1]} r="2.6" fill={C.glass} stroke={tone} strokeWidth="1.8" />
    </svg>
  );
}

function IceMeter({ value, tone = C.cyan }: { value: number; tone?: string }) {
  return (
    <span className="hidden items-center gap-2.5 sm:flex" aria-hidden="true">
      ...
      >
        <span
          ...
          style={{
            width: `${value}%`,
            background: `linear-gradient(90deg, ${C.cyanDeep}, ${tone})`,
...
// Decoratieve gebarsten-ijs hairlines op de achtergrond (subtiel).
function CrackField() {
  return (
    <svg
      aria-hidden="true"
    ...
    >
      {[90, 250, 410, 560].map((y, i) => (
        <path
          key={i}
          d={`M0 ${y} L320 ${y + 8} L520 ${y - 12} L760 ${y + 6} L1000 ${y - 4}`}
          fill="none"
          stroke="rgba(58,167,212,0.12)"
          strokeWidth="1"
        />
      ))}
    </svg>
...
export function Concept445() {
  ...
  return (
    ...
    >
      ...
      />
      <style>{`
        @keyframes gletsjerRise { from { opacity: 0; transform: translateY(9px); } to { opacity: 1; transform: translateY(0); } }
        .gletsjer-rise { animation: gletsjerRise 0.5s cubic-bezier(0.22, 1, 0.36, 1) both; }
        @media (prefers-reduced-motion: reduce) { .gletsjer-rise { animation: none !important; } }
      `}</style>

      <div className="relative mx-auto max-w-6xl px-4 pb-24 sm:px-6 md:px-8">
        <TopBar />
        <NavBar screen={screen} setScreen={setScreen} />
        <main key={screen} className="gletsjer-rise pt-7">
          {screen === "dashboard" && (
    ...
    <header className="flex items-center justify-between gap-4 pt-8">
      <div className="flex items-center gap-3.5">
        <span
          ...
          style={{
            background: `linear-gradient(160deg, ${C.cyan}, ${C.cyanDeep})`,
            boxShadow: "0 8px 20px -8px rgba(58,167,212,0.6)",
          }}
          aria-hidden="true"
        >
        ...
        <div>
          ...
          <p className="mt-1.5 text-[11px] leading-none" style={{ color: C.inkMute }}>
            {PROFIEL.plaats} · gelaagd kristalhelder
          </p>
        </div>
      </div>
      <div className="flex items-center gap-3">
        ...
        <span
          ...
          style={{ background: C.glass, border: `1px solid ${C.line}`, color: C.inkMute }}
        ...
        <span
          ...
          style={{ background: C.cyanWash, border: `1px solid ${C.cyanLine}`, color: C.cyanDeep }}
          aria-hidden="true"
        >
          {PROFIEL.initialen}
        </span>
        ...
        {SCREENS.map((s) => {
          ...
          return (
            <button
              ...
              className="flex shrink-0 items-center gap-2 rounded-full px-4 py-2 text-[12.5px] font-bold transition-all duration-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#3aa7d4] focus-visible:ring-offset-2 focus-visible:ring-offset-[#f7fbfd] motion-reduce:transition-none"
              style={{
                color: on ? "#fff" : C.inkMute,
                background: on
                  ? `linear-gradient(180deg, ${C.cyan}, ${C.cyanDeep})`
                  : "transparent",
                ...bodyFont,
              }}
            >
  ...
  return (
    <div className="space-y-7">
      <section className="grid grid-cols-1 gap-5 lg:grid-cols-[1.4fr_1fr]">
        <Pane className="p-7 md:p-9" fill="cyan">
          <Eyebrow>Vandaag · helder ijs</Eyebrow>
          ...
          <p className="mt-3 max-w-md text-[14px] leading-relaxed" style={{ color: C.inkSoft }}>
            Je praktijk ligt helder als gletsjerijs: elke laag zichtbaar, verifieerbaar en betaald.
            Werk de bovenste actie af en de rest blijft kristalhelder.
          </p>
      ...
      <section>
        <div className="mb-4">
          <Eyebrow>Strata · deze maand</Eyebrow>
        </div>
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {KPIS.map((k, i) => (
            <Pane key={k.label} className="p-5">
              ...
              <div className="mt-3">
                <IceLine data={k.spark} tone={k.up ? C.cyan : C.warn} id={`k445-${i}`} />
              </div>
            </Pane>
          ))}
        </div>
        ...
        <div>
          <div className="mb-4 flex items-baseline justify-between">
            ...
            <button
              type="button"
              onClick={onOpen}
              className="rounded text-[11px] font-bold uppercase tracking-[0.16em] transition-colors hover:brightness-90 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#3aa7d4] focus-visible:ring-offset-2 focus-visible:ring-offset-[#eef5f9]"
              ...
              {OPDRACHTEN.map((o, i) => (
                <li key={o.id} style={{ borderTop: i === 0 ? "none" : `1px solid ${C.lineSoft}` }}>
                  <button
                    type="button"
                    onClick={onOpen}
                    className="group grid w-full grid-cols-[auto_1fr_auto] items-center gap-4 px-5 py-4 text-left transition-colors hover:bg-[#eaf3f8] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-[#3aa7d4] motion-reduce:transition-none"
                  >
                    <span
                      className="inline-flex h-10 w-10 items-center justify-center rounded-full"
                      style={{
                        background: i === 0 ? C.cyanWash : C.raise,
                        border: `1px solid ${i === 0 ? C.cyanLine : C.line}`,
                      }}
                    >
                      <span
                        className="text-[12px] font-bold leading-none"
                        style={{ color: i === 0 ? C.cyanDeep : C.inkMute, ...num }}
                      >
                        {o.match}
                      </span>
                    </span>
                    ...
                    <span className="flex items-center gap-3">
                      <IceMeter value={o.match} tone={o.match >= 90 ? C.cyan : C.info} />
                      <ChevronRight
                        size={17}
                        aria-hidden="true"
                        className="transition-transform group-hover:translate-x-0.5 motion-reduce:transition-none"
  ...
  return (
    <div className="space-y-7">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
        ...
        >
          ...
          <input
            ...
            className="w-full bg-transparent text-[13px] outline-none placeholder:text-[#9db6c3]"
      ...
      {loading ? (
        ...
        <Pane className="p-6" accent>
          <div className="flex flex-col items-center py-14 text-center">
            <span
              className="inline-flex h-16 w-16 items-center justify-center rounded-full"
              style={{ background: C.glass, border: `1px solid ${C.cyanLine}`, color: C.cyanDeep }}
              aria-hidden="true"
            >
              <Search size={26} />
            </span>
...
function MarktKaart({
  ...
  const tone = strong ? C.cyan : C.info;
  const toneInk = strong ? C.cyanDeep : C.infoInk;
  return (
    <Pane className="p-6" accent={strong}>
      <div className="grid grid-cols-[1fr_auto] items-start gap-4">
        ...
        <div className="flex flex-col items-center gap-1.5">
          <span
            ...
            style={{ background: strong ? C.cyanWash : C.infoWash, border: `1.5px solid ${tone}` }}
          >
      ...
      <div className="mt-4 flex items-center gap-3">
        <button
          ...
          onClick={() => setOpen((v) => !v)}
          aria-expanded={open}
          className="inline-flex items-center gap-1.5 rounded-full px-3.5 py-1.5 text-[11.5px] font-bold transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#3aa7d4] focus-visible:ring-offset-2 focus-visible:ring-offset-[#f7fbfd]"
          style={{ color: C.cyanDeep, border: `1px solid ${C.line}`, ...bodyFont }}
        >
  ...
  return (
    <div className="space-y-6">
      <button
        type="button"
        onClick={onBack}
        className="inline-flex items-center gap-2 rounded-full px-4 py-2 text-[12.5px] font-bold transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#3aa7d4] focus-visible:ring-offset-2 focus-visible:ring-offset-[#eef5f9]"
        style={{ color: C.inkSoft, border: `1px solid ${C.line}`, background: C.glass }}
      >
        <ArrowRight size={13} aria-hidden="true" className="rotate-180" /> Terug naar marktplaats
      </button>

      <Pane className="p-7 md:p-9" accent={strong} fill="cyan">
        <div className="flex flex-wrap items-center gap-2">
          <span
            className="inline-flex items-center rounded-full px-2.5 py-0.5 text-[10.5px] font-bold"
            style={{ color: C.inkMute, border: `1px solid ${C.line}`, ...num }}
          ...
          <span
            ...
            style={{ background: `linear-gradient(180deg, ${C.cyan}, ${C.cyanDeep})` }}
          >
        ...
        <div className="flex flex-wrap items-center justify-between gap-6">
          <div className="max-w-md">
            ...
            <p className="mt-3 text-[13.5px] leading-relaxed" style={{ color: C.inkSoft }}>
              <span className="font-bold" style={{ color: C.cyanDeep }}>
                {PROFIEL.trust}.
              </span>{" "}
            ...
            <div className="mt-4 max-w-xs">
              <IceMeter value={ratio} tone={C.cyan} />
            </div>
          </div>
          <span
            className="inline-flex h-24 w-24 flex-col items-center justify-center rounded-full"
            style={{ background: C.glass, border: `1.5px solid ${C.cyanLine}` }}
          >
            <span
              className="text-[26px] font-bold leading-none"
              style={{ color: C.cyanDeep, ...num }}
            >
              {ratio}
            </span>
            <span
            ...
            return (
              ...
              >
                <button
                  ...
                  className="grid w-full grid-cols-[1fr_auto] items-center gap-4 px-6 py-4 text-left transition-colors hover:bg-[#eaf3f8] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-[#3aa7d4] motion-reduce:transition-none sm:grid-cols-[1fr_11rem_2.5rem]"
                >
      ...
      <div>
        ...
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
          {DOCUMENTEN.map((d) => {
            ...
            return (
              <Pane key={d.naam} className="flex items-center gap-3 p-4">
                <span
                  className="inline-flex h-10 w-10 items-center justify-center rounded-full"
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-2 max-w-md text-[13.5px]" style={{ color: C.inkSoft }}>
          Van boven naar beneden — zo blijf je verifieerbaar en betaald, laag na laag helder.
        </p>
      </div>
      ...
      <ol className="space-y-3">
        {ACTIES.map((a, i) => {
          ...
          const ink = warn ? C.warnInk : C.cyanDeep;
          const wash = warn ? C.warnWash : C.cyanWash;
          return (
            <li key={a.titel}>
              <Pane className="p-6" accent={warn}>
    ...
    <div className="space-y-6">
      ...
      <section className="grid grid-cols-1 gap-4 sm:grid-cols-3">
        {[
          ...
          <Pane key={s.l} className="p-6" accent={s.accent}>
            <div className="flex items-center justify-between">
              <p
                className="text-[10px] font-bold uppercase tracking-[0.12em]"
                style={{ color: C.inkMute }}
        ...
        <ul>
          {FACTUREN.map((f, i) => {
            ...
            return (
              <li
                key={f.nr}
                className="grid grid-cols-[1fr_auto] items-center gap-3 px-6 py-4 transition-colors hover:bg-[#eaf3f8] sm:grid-cols-[8rem_1fr_5rem_9rem_6rem] sm:gap-4"
                style={{ borderTop: i === 0 ? "none" : `1px solid ${C.lineSoft}` }}
              >
                <span
