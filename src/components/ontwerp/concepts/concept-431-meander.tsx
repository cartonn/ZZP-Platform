...
// Concept 431 — "Meander" · Vloeiende verbindingslijnen / spatial flow.
// Organische, meanderende hairline-draden verbinden gerelateerde modules; het oog wordt langs een
// rustige route geleid van begroeting → acties → opdrachten → certificaten. Teal-accent (#0d9488)
// op zacht groen-grijs papier (#f4f6f4), tekst #14201e, warm zand als secundaire toon. Zachte,
// physics-achtige entree-choreografie (staggered), curven zonder harde hoeken. Reduced-motion-veilig.
...
// — Palet: teal-stroom op zacht groen papier, warm zand secundair —
const C = {
  bg: "#f4f6f4",
  bgDeep: "#eef1ee",
  card: "#ffffff",
  ...
  lineSoft: "rgba(20,32,30,0.06)",
  teal: "#0d9488",
  tealHi: "#2dd4bf",
  tealDeep: "#0b6e66",
  tealWash: "rgba(13,148,136,0.12)",
  tealMist: "rgba(13,148,136,0.06)",
  sand: "#c8a86a",
  sandDeep: "#9a7f45",
  sandWash: "rgba(200,168,106,0.16)",
  ok: "#0d9488",
  okInk: "#0b6e66",
  okWash: "rgba(13,148,136,0.14)",
  warn: "#c8873a",
  warnInk: "#9a6320",
  warnWash: "rgba(200,135,58,0.16)",
  info: "#4d7c8a",
  infoInk: "#345863",
  infoWash: "rgba(77,124,138,0.14)",
  bad: "#c0654f",
  badInk: "#96442f",
  badWash: "rgba(192,101,79,0.16)",
};
...
// — Meander-veld: meanderende hairline-draden die de achtergrond zacht doorlopen —
function MeanderField() {
  return (
    <svg
    ...
    >
      <defs>
        <linearGradient id="mnd-thread" x1="0" y1="0" x2="1" y2="1">
          <stop offset="0%" stopColor={C.teal} stopOpacity="0" />
          <stop offset="45%" stopColor={C.teal} stopOpacity="0.5" />
          <stop offset="100%" stopColor={C.sand} stopOpacity="0" />
        </linearGradient>
      </defs>
      <path
        className="mnd-flow"
        d="M-40 120 C 220 40, 360 260, 620 200 S 980 60, 1260 240"
        fill="none"
        stroke="url(#mnd-thread)"
        strokeWidth="1.4"
      />
      <path
        className="mnd-flow"
        style={{ animationDelay: "0.4s" }}
        d="M-40 460 C 280 560, 420 320, 700 420 S 1020 620, 1260 500"
        fill="none"
        stroke="url(#mnd-thread)"
        strokeWidth="1.2"
      />
      <path
        ...
        d="M-40 780 C 240 700, 480 880, 760 760 S 1040 660, 1260 820"
        fill="none"
        stroke="url(#mnd-thread)"
        strokeWidth="1"
      />
...
function Panel({
  ...
  const bg =
    tone === "teal"
      ? `linear-gradient(135deg, ${C.card} 0%, ${C.card} 55%, ${C.tealMist} 100%)`
      : C.card;
  return (
    <Tag
      ...
      style={{
        ...
        boxShadow: "0 1px 2px rgba(20,32,30,0.03), 0 14px 34px rgba(20,32,30,0.05)",
        color: C.ink,
      }}
    >
      {flow && (
        <span
          ...
          style={{
            background: `radial-gradient(circle at center, ${C.tealHi} 0%, transparent 70%)`,
            opacity: 0.12,
            filter: "blur(24px)",
          }}
        />
...
function Eyebrow({ children, tone = C.tealDeep }: { children: React.ReactNode; tone?: string }) {
  return (
    <p
...
function PrimaryButton({
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className={`group inline-flex items-center justify-center gap-2 rounded-full px-5 py-2.5 text-[13px] font-semibold transition-all duration-300 hover:brightness-[1.06] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#0d9488] focus-visible:ring-offset-2 focus-visible:ring-offset-[#f4f6f4] active:translate-y-px motion-reduce:transition-none ${className}`}
      style={{
        color: "#ffffff",
        background: `linear-gradient(135deg, ${C.tealHi} 0%, ${C.teal} 55%, ${C.tealDeep} 100%)`,
...
function GhostButton({
  ...
  return (
    <button
      ...
      className={`inline-flex items-center justify-center gap-2 rounded-full px-4 py-2.5 text-[12.5px] font-semibold transition-all duration-300 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#0d9488] focus-visible:ring-offset-2 focus-visible:ring-offset-[#f4f6f4] motion-reduce:transition-none ${className}`}
      style={{
...
// — Stroom-sparkline: vloeiende curve met zacht verloop en teal-eindpunt —
function FlowLine({ data, tone, id }: { data: number[]; tone: string; id: string }) {
  ...
  return (
    ...
    >
      ...
      <polygon points={`0,${h} ${line} ${w},${h}`} fill={`url(#flow-${id})`} />
      <polyline
        points={line}
        fill="none"
        stroke={tone}
      ...
      />
      <circle cx={last[0]} cy={last[1]} r="3" fill={tone} opacity="0.25" />
      <circle cx={last[0]} cy={last[1]} r="2" fill={C.card} stroke={tone} strokeWidth="1.5" />
    </svg>
  );
}

// — Stroommeter: dunne voortgangslijn met bewegende gloed-punt —
    ...
    <span className="hidden items-center gap-2.5 sm:flex" aria-hidden="true">
      ...
      >
        <span
          ...
          style={{
            width: `${value}%`,
            background: `linear-gradient(90deg, ${C.tealHi}, ${tone})`,
  ...
  return (
    <div
      ...
      style={{
        ...body,
        color: C.ink,
        background: `radial-gradient(120% 90% at 15% -5%, ${C.bg} 0%, ${C.bgDeep} 70%, ${C.soft} 100%)`,
      }}
    >
      <style>{`
        @keyframes mndRise { from { opacity: 0; transform: translateY(10px); } to { opacity: 1; transform: translateY(0); } }
        .mnd-rise > * { animation: mndRise 0.55s cubic-bezier(0.22, 1, 0.36, 1) both; }
        .mnd-rise > *:nth-child(2) { animation-delay: 0.06s; }
        .mnd-rise > *:nth-child(3) { animation-delay: 0.12s; }
        .mnd-rise > *:nth-child(4) { animation-delay: 0.18s; }
        @keyframes mndDraw { from { stroke-dashoffset: 1400; } to { stroke-dashoffset: 0; } }
        .mnd-flow { stroke-dasharray: 1400; animation: mndDraw 2.4s cubic-bezier(0.22, 1, 0.36, 1) both; }
        @media (prefers-reduced-motion: reduce) {
          .mnd-rise > * { animation: none !important; }
          .mnd-flow { animation: none !important; stroke-dasharray: none; }
        }
      `}</style>
      ...
      <div className="relative mx-auto max-w-6xl px-4 pb-24 sm:px-6 md:px-8">
        ...
        <main key={screen} className="mnd-rise pt-7">
          {screen === "dashboard" && (
    ...
    <header className="flex items-center justify-between gap-4 pt-8">
      <div className="flex items-center gap-3.5">
        <span
          ...
          style={{
            background: `linear-gradient(135deg, ${C.tealHi} 0%, ${C.teal} 60%, ${C.tealDeep} 100%)`,
            color: "#ffffff",
        ...
        <div>
          ...
          >
            <Leaf size={11} aria-hidden="true" style={{ color: C.teal }} />
            {PROFIEL.plaats} · in stroom
          </p>
        </div>
      </div>
      <div className="flex items-center gap-3">
        ...
        >
          ...
          {ongelezen > 0 && (
            <span
              ...
              style={{ background: C.teal, color: "#ffffff", ...num }}
              aria-hidden="true"
            >
              {ongelezen}
            </span>
        ...
        <span
          ...
          style={{ background: C.tealWash, color: C.tealDeep, ...body }}
          aria-hidden="true"
        >
          {PROFIEL.initialen}
        </span>
        ...
        {SCREENS.map((s) => {
          ...
          return (
            <button
              ...
              className="flex shrink-0 items-center gap-2 rounded-full px-4 py-2 text-[12.5px] font-semibold transition-all duration-300 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#0d9488] focus-visible:ring-offset-2 focus-visible:ring-offset-[#ffffff] motion-reduce:transition-none"
              style={{
  ...
  return (
    <div className="space-y-7">
      <section className="grid grid-cols-1 gap-5 lg:grid-cols-[1.4fr_1fr]">
        <Panel className="p-7 md:p-9" tone="teal" flow>
          <div className="relative">
            <Eyebrow>Vandaag · jouw route</Eyebrow>
            ...
            <p className="mt-3 max-w-md text-[14px] leading-relaxed" style={{ color: C.inkSoft }}>
              Volg de lijn: van je eerstvolgende actie naar de opdrachten die het best bij je
              passen, en houd je certificaten onderweg verifieerbaar. Rustig, in één vloeiende
              beweging.
            </p>
        ...
        <Panel className="p-7">
          ...
          <div className="mt-5 pt-4" style={{ borderTop: `1px solid ${C.lineSoft}` }}>
            <p className="flex items-center gap-2 text-[12px]" style={{ color: C.inkMute, ...num }}>
              <ShieldCheck size={13} aria-hidden="true" style={{ color: C.teal }} />
      ...
      <section>
        <div className="mb-4">
          <Eyebrow>Stroom · deze maand</Eyebrow>
        </div>
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {KPIS.map((k, i) => (
            <Panel key={k.label} className="p-5">
              ...
              <div className="mt-3">
                <FlowLine data={k.spark} tone={k.up ? C.teal : C.sand} id={`kpi-${i}`} />
              </div>
            </Panel>
          ))}
        </div>
        ...
        <div>
          <div className="mb-4 flex items-baseline justify-between">
            ...
            <button
              type="button"
              onClick={onOpen}
              className="rounded text-[11px] font-semibold uppercase tracking-[0.14em] transition-colors hover:brightness-90 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#0d9488] focus-visible:ring-offset-2 focus-visible:ring-offset-[#f4f6f4]"
              style={{ color: C.tealDeep, ...body }}
            >
              Alle →
            </button>
          </div>
              ...
              {OPDRACHTEN.map((o, i) => (
                <li key={o.id} style={{ borderTop: i === 0 ? "none" : `1px solid ${C.lineSoft}` }}>
                  <button
                    type="button"
                    onClick={onOpen}
                    className="group grid w-full grid-cols-[auto_1fr_auto] items-center gap-4 px-5 py-4 text-left transition-colors hover:bg-[#f1f4f2] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-[#0d9488] motion-reduce:transition-none"
                  >
                    <span
                      className="inline-flex h-10 w-10 items-center justify-center rounded-full"
                      style={{
                        background: i === 0 ? C.tealWash : C.raise,
                        border: `1px solid ${i === 0 ? C.teal : C.line}`,
                      }}
                    >
                      <span
                        className="text-[12px] font-bold leading-none"
                        style={{ color: i === 0 ? C.tealDeep : C.inkMute, ...num }}
                      >
                        {o.match}
                      </span>
                    </span>
                    ...
                    <span className="flex items-center gap-3">
                      <Meter value={o.match} tone={o.match >= 90 ? C.teal : C.sand} />
                      <ChevronRight
                        size={17}
                        aria-hidden="true"
                        className="transition-transform group-hover:translate-x-0.5 motion-reduce:transition-none"
  ...
  return (
    <div className="space-y-7">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
        ...
        >
          ...
          <input
            ...
            className="w-full bg-transparent text-[13px] outline-none placeholder:text-[#9aa8a2]"
            style={{ color: C.ink, ...body }}
          />
        </div>
      ...
      {loading ? (
        ...
        <Panel className="p-6" tone="teal" flow>
          <div className="relative flex flex-col items-center py-14 text-center">
            <span
              className="inline-flex h-16 w-16 items-center justify-center rounded-full"
              style={{ background: C.tealWash, border: `1px solid ${C.teal}`, color: C.tealDeep }}
              aria-hidden="true"
            >
              <Search size={26} />
            </span>
            ...
            <p className="mx-auto mt-2 max-w-xs text-[13.5px]" style={{ color: C.inkSoft }}>
              Geen opdracht op {q ? `“${q}”` : "je zoekterm"} in deze stroom. Verruim je zoekterm en
              probeer opnieuw.
            </p>
            <div className="mt-6">
...
function MarktKaart({
  ...
  const tone = strong ? C.teal : C.sand;
  const toneInk = strong ? C.tealDeep : C.sandDeep;
  return (
    <Panel className="p-6">
      <div className="grid grid-cols-[1fr_auto] items-start gap-4">
        ...
        <div className="flex flex-col items-center gap-1.5">
          <span
            ...
            style={{ background: strong ? C.tealWash : C.sandWash, border: `1.5px solid ${tone}` }}
          >
      ...
      <div className="mt-4 flex items-center gap-3">
        <button
          ...
          onClick={() => setOpen((v) => !v)}
          aria-expanded={open}
          className="inline-flex items-center gap-1.5 rounded-full px-3.5 py-1.5 text-[11.5px] font-semibold transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#0d9488] focus-visible:ring-offset-2 focus-visible:ring-offset-[#ffffff]"
          style={{ color: C.tealDeep, border: `1px solid ${C.line}`, ...body }}
        >
  ...
  return (
    <div className="space-y-6">
      <button
        type="button"
        onClick={onBack}
        className="inline-flex items-center gap-2 rounded-full px-4 py-2 text-[12.5px] font-semibold transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#0d9488] focus-visible:ring-offset-2 focus-visible:ring-offset-[#f4f6f4]"
        ...
        <div className="relative flex flex-wrap items-center justify-between gap-6">
          <div className="max-w-md">
            ...
            <p className="mt-3 text-[13.5px] leading-relaxed" style={{ color: C.inkSoft }}>
              <span className="font-semibold" style={{ color: C.tealDeep }}>
                {PROFIEL.trust}.
              </span>{" "}
          ...
          <span
            ...
            style={{ background: C.tealWash, border: `1.5px solid ${C.teal}` }}
          >
            <span
              className="text-[26px] font-semibold leading-none"
              style={{ color: C.tealDeep, ...num }}
            >
              {ratio}
            </span>
            <span
            ...
            return (
              ...
              >
                <button
                  ...
                  className="grid w-full grid-cols-[1fr_auto] items-center gap-4 px-6 py-4 text-left transition-colors hover:bg-[#f1f4f2] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-[#0d9488] motion-reduce:transition-none sm:grid-cols-[1fr_11rem_2.5rem]"
                >
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-2 max-w-md text-[13.5px]" style={{ color: C.inkSoft }}>
          Volg de lijn van boven naar beneden — zo blijf je verifieerbaar en betaald, zonder haast.
        </p>
      </div>
      ...
      <ol className="space-y-3">
        {ACTIES.map((a, i) => {
          ...
          const ink = warn ? C.warnInk : C.tealDeep;
      ...
      <Panel>
        ...
        <ul>
          {FACTUREN.map((f, i) => {
            ...
            return (
              <li
                key={f.nr}
                className="grid grid-cols-[1fr_auto] items-center gap-3 px-6 py-4 transition-colors hover:bg-[#f1f4f2] sm:grid-cols-[8rem_1fr_5rem_9rem_6rem] sm:gap-4"
                style={{ borderTop: i === 0 ? "none" : `1px solid ${C.lineSoft}` }}
              >
                <span
        ...
        >
          <span
            ...
            style={{ color: C.inkMute, ...body }}
          >
            <Route size={12} aria-hidden="true" style={{ color: C.teal }} /> Totaal betaald
          </span>
