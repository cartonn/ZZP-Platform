...
// Concept 440 — "Linnen" · Natuurlijk linnen, warm ambachtelijk / textielrust.
// Tactiele materialiteit voor 2026: een subtiel geweven linnen-textuur (CSS-weefsel + korrel) draagt
// de interface. Natuurlijke neutrale tinten — ongebleekt linnen #f1ece1, taupe-accent #8a6d3b/#6b5a3a,
// tekst #2b271f, zacht groen-grijs (salie) als secundair. Warm, aards, rustig, met een kwaliteitsgevoel
// als van goed textiel. Serif-display (Spectral/Fraunces) + Inter. Animaties respecteren reduced-motion.
...
// — Palet: ongebleekt linnen, taupe, salie —
const C = {
  paperBg: "#f1ece1",
  paperBgDeep: "#e9e1d0",
  paper: "#f7f3ea",
  ...
  inkMute: "#8a7d68",
  inkFaint: "#b3a68d",
  line: "rgba(43,39,31,0.15)",
  lineSoft: "rgba(43,39,31,0.08)",
  // taupe-accent
  taupe: "#8a6d3b",
  taupeDeep: "#6b5a3a",
  taupeWash: "rgba(138,109,59,0.14)",
  taupeMist: "rgba(138,109,59,0.07)",
  // salie (zacht groen-grijs) secundair
  sage: "#7c8a72",
  sageDeep: "#5f6d56",
  sageWash: "rgba(124,138,114,0.16)",
  // status
  ok: "#6f8a56",
  okInk: "#52693f",
  okWash: "rgba(111,138,86,0.16)",
  warn: "#b08a3c",
  warnInk: "#856326",
  warnWash: "rgba(176,138,60,0.18)",
  info: "#5a7a80",
  infoInk: "#3f5a5f",
  infoWash: "rgba(90,122,128,0.14)",
  bad: "#b0553f",
  badInk: "#863c2a",
...
// Geweven linnen-textuur: fijne kruisdraden over een grondkleur — subtiel, matte korrel.
function weave(color: string): React.CSSProperties {
  return {
    backgroundColor: color,
    backgroundImage:
      "repeating-linear-gradient(0deg, rgba(43,39,31,0.035) 0px, rgba(43,39,31,0.035) 1px, transparent 1px, transparent 3px)," +
      "repeating-linear-gradient(90deg, rgba(43,39,31,0.045) 0px, rgba(43,39,31,0.045) 1px, transparent 1px, transparent 3px)," +
      "repeating-linear-gradient(90deg, rgba(255,252,246,0.35) 0px, rgba(255,252,246,0.35) 1px, transparent 1px, transparent 3px)",
    backgroundPosition: "0 0, 0 0, 1px 0",
  };
}
...
// — Linnen-paneel: geweven grondkleur, zachte hairline-rand, geen glans —
function Panel({
  children,
  className = "",
  as: Tag = "div",
  ...
  className?: string;
  as?: "div" | "section" | "li";
  fill?: "paper" | "taupe" | "sage" | "raise";
}) {
  const base = {
    paper: { color: C.paper, bd: C.line },
    taupe: { color: C.taupeMist, bd: C.taupe },
    sage: { color: C.sageWash, bd: C.sage },
  ...
  return (
    <Tag
      ...
      style={{
        ...
        boxShadow: "0 1px 2px rgba(43,39,31,0.04)",
        color: C.ink,
      }}
    >
      {children}
...
function Eyebrow({ children, tone = C.taupeDeep }: { children: React.ReactNode; tone?: string }) {
  return (
    <p
...
function PrimaryButton({
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className={`group inline-flex items-center justify-center gap-2 rounded-full px-5 py-2.5 text-[13px] font-semibold transition-all duration-200 hover:brightness-[1.06] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#8a6d3b] focus-visible:ring-offset-2 focus-visible:ring-offset-[#f1ece1] active:translate-y-px motion-reduce:transition-none ${className}`}
      style={{
        color: "#f7f3ea",
        background: C.taupe,
        border: `1px solid ${C.taupeDeep}`,
        ...bodyFont,
      }}
    >
      {children}
...
function GhostButton({
  ...
  return (
    <button
      ...
      className={`inline-flex items-center justify-center gap-2 rounded-full px-4 py-2.5 text-[12.5px] font-semibold transition-all duration-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#8a6d3b] focus-visible:ring-offset-2 focus-visible:ring-offset-[#f1ece1] motion-reduce:transition-none ${className}`}
      style={{
        color: active ? "#f7f3ea" : C.inkSoft,
...
// — Draad-sparkline: fijne matte lijn zoals een geweven draad —
function DraadLine({ data, tone, id }: { data: number[]; tone: string; id: string }) {
  ...
  return (
    ...
    >
      <defs>
        <linearGradient id={`linnen-${id}`} x1="0" y1="0" x2="0" y2="1">
          <stop offset="0%" stopColor={tone} stopOpacity="0.2" />
          <stop offset="100%" stopColor={tone} stopOpacity="0" />
        </linearGradient>
      </defs>
      <polygon points={`0,${h} ${line} ${w},${h}`} fill={`url(#linnen-${id})`} />
      <polyline
        points={line}
        fill="none"
        stroke={tone}
...
// — Draad-meter: matte voortgangsstreek —
function DraadMeter({ value, tone = C.taupe }: { value: number; tone?: string }) {
  return (
...
export function Concept440() {
  ...
  return (
    <div
      ...
      style={{ ...bodyFont, color: C.ink, ...weave(C.paperBg) }}
    >
      <style>{`
        @keyframes linnenRise { from { opacity: 0; transform: translateY(8px); } to { opacity: 1; transform: translateY(0); } }
        .linnen-rise { animation: linnenRise 0.5s cubic-bezier(0.22, 1, 0.36, 1) both; }
        @media (prefers-reduced-motion: reduce) { .linnen-rise { animation: none !important; } }
      `}</style>

      <div className="relative mx-auto max-w-6xl px-4 pb-24 sm:px-6 md:px-8">
        <TopBar />
        <NavBar screen={screen} setScreen={setScreen} />
        <main key={screen} className="linnen-rise pt-7">
          {screen === "dashboard" && (
    ...
    <header className="flex items-center justify-between gap-4 pt-8">
      <div className="flex items-center gap-3.5">
        <span
          ...
          style={{ background: C.taupe, border: `1px solid ${C.taupeDeep}`, color: "#f7f3ea" }}
          aria-hidden="true"
        >
          <Leaf size={18} />
        </span>
        <div>
          ...
          <p className="mt-1.5 text-[11px] leading-none" style={{ color: C.inkMute, ...bodyFont }}>
            {PROFIEL.plaats} · natuurlijk geweven
          </p>
        </div>
      </div>
        ...
        >
          ...
          {ongelezen > 0 && (
            <span
              ...
              style={{ background: C.taupe, color: "#f7f3ea", ...num }}
              aria-hidden="true"
            >
              {ongelezen}
            </span>
        ...
        {SCREENS.map((s) => {
          ...
          return (
            <button
              ...
              className="flex shrink-0 items-center gap-2 rounded-full px-4 py-2 text-[12.5px] font-semibold transition-all duration-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#8a6d3b] focus-visible:ring-offset-2 focus-visible:ring-offset-[#f7f3ea] motion-reduce:transition-none"
              style={{
                color: on ? "#f7f3ea" : C.inkMute,
                background: on ? C.taupe : "transparent",
                ...bodyFont,
              }}
            >
              {s.label}
  ...
  return (
    <div className="space-y-7">
      <section className="grid grid-cols-1 gap-5 lg:grid-cols-[1.4fr_1fr]">
        <Panel className="p-7 md:p-9" fill="taupe">
          <Eyebrow>Vandaag · rustig geweven</Eyebrow>
          ...
          <p className="mt-3 max-w-md text-[14px] leading-relaxed" style={{ color: C.inkSoft }}>
            Je praktijk als goed textiel: draad voor draad op orde, verifieerbaar en betaald. Loop
            rustig langs je acties — alles ligt strak geweven.
          </p>
      ...
      <section>
        <div className="mb-4">
          <Eyebrow>Weefsel · deze maand</Eyebrow>
        </div>
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {KPIS.map((k, i) => (
            <Panel key={k.label} className="p-5">
              ...
              <div className="mt-3">
                <DraadLine data={k.spark} tone={k.up ? C.taupe : C.sage} id={`k440-${i}`} />
              </div>
            </Panel>
          ))}
        </div>
        ...
        <div>
          <div className="mb-4 flex items-baseline justify-between">
            ...
            <button
              type="button"
              onClick={onOpen}
              className="rounded text-[11px] font-semibold uppercase tracking-[0.14em] transition-colors hover:brightness-90 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#8a6d3b] focus-visible:ring-offset-2 focus-visible:ring-offset-[#f1ece1]"
              style={{ color: C.taupeDeep, ...bodyFont }}
            >
              Alle →
            </button>
          </div>
              ...
              {OPDRACHTEN.map((o, i) => (
                <li key={o.id} style={{ borderTop: i === 0 ? "none" : `1px solid ${C.lineSoft}` }}>
                  <button
                    type="button"
                    onClick={onOpen}
                    className="group grid w-full grid-cols-[auto_1fr_auto] items-center gap-4 px-5 py-4 text-left transition-colors hover:bg-[#ece5d6] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-[#8a6d3b] motion-reduce:transition-none"
                  >
                    <span
                      className="inline-flex h-10 w-10 items-center justify-center rounded-full"
                      style={{
                        background: i === 0 ? C.taupeWash : C.raise,
                        border: `1px solid ${i === 0 ? C.taupe : C.line}`,
                      }}
                    >
                      <span
                        className="text-[12px] font-bold leading-none"
                        style={{ color: i === 0 ? C.taupeDeep : C.inkMute, ...num }}
                      >
                        {o.match}
                      </span>
                    </span>
                    ...
                    <span className="flex items-center gap-3">
                      <DraadMeter value={o.match} tone={o.match >= 90 ? C.taupe : C.sage} />
                      <ChevronRight
                        size={17}
                        aria-hidden="true"
                        className="transition-transform group-hover:translate-x-0.5 motion-reduce:transition-none"
  ...
  return (
    <div className="space-y-7">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
        ...
        >
          ...
          <input
            ...
            className="w-full bg-transparent text-[13px] outline-none placeholder:text-[#b3a68d]"
            style={{ color: C.ink, ...bodyFont }}
          />
        </div>
      ...
      {loading ? (
        ...
        <Panel className="p-6" fill="sage">
          <div className="flex flex-col items-center py-14 text-center">
            <span
              className="inline-flex h-16 w-16 items-center justify-center rounded-full"
              style={{ background: C.paper, border: `1px solid ${C.sage}`, color: C.sageDeep }}
              aria-hidden="true"
            >
              <Search size={26} />
            </span>
...
function MarktKaart({
  ...
  const tone = strong ? C.taupe : C.sage;
  const toneInk = strong ? C.taupeDeep : C.sageDeep;
  return (
    <Panel className="p-6">
      <div className="grid grid-cols-[1fr_auto] items-start gap-4">
        ...
        <div className="flex flex-col items-center gap-1.5">
          <span
            ...
            style={{ background: strong ? C.taupeWash : C.sageWash, border: `1.5px solid ${tone}` }}
          >
      ...
      <div className="mt-4 flex items-center gap-3">
        <button
          ...
          onClick={() => setOpen((v) => !v)}
          aria-expanded={open}
          className="inline-flex items-center gap-1.5 rounded-full px-3.5 py-1.5 text-[11.5px] font-semibold transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#8a6d3b] focus-visible:ring-offset-2 focus-visible:ring-offset-[#f7f3ea]"
          style={{ color: C.taupeDeep, border: `1px solid ${C.line}`, ...bodyFont }}
        >
  ...
  return (
    <div className="space-y-6">
      <button
        type="button"
        onClick={onBack}
        className="inline-flex items-center gap-2 rounded-full px-4 py-2 text-[12.5px] font-semibold transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#8a6d3b] focus-visible:ring-offset-2 focus-visible:ring-offset-[#f1ece1]"
        style={{
          color: C.inkSoft,
      ...
      <Panel className="p-7 md:p-9" fill="taupe">
        <div className="flex flex-wrap items-center gap-2">
          ...
          <span
            ...
            style={{ color: "#f7f3ea", background: C.taupe, ...bodyFont }}
          >
            <Leaf size={11} aria-hidden="true" /> {strong ? "Sterke match" : "Goede match"} ·{" "}
            {opdracht.match}%
          </span>
        </div>
        <h1
        ...
        <div className="flex flex-wrap items-center justify-between gap-6">
          <div className="max-w-md">
            ...
            <p className="mt-3 text-[13.5px] leading-relaxed" style={{ color: C.inkSoft }}>
              <span className="font-semibold" style={{ color: C.taupeDeep }}>
                {PROFIEL.trust}.
              </span>{" "}
            ...
            <div className="mt-4 max-w-xs">
              <DraadMeter value={ratio} tone={C.taupe} />
            </div>
          </div>
          <span
            className="inline-flex h-24 w-24 flex-col items-center justify-center rounded-full"
            style={{ background: C.paper, border: `1.5px solid ${C.taupe}` }}
          >
            <span
              className="text-[26px] font-semibold leading-none"
              style={{ color: C.taupeDeep, ...num }}
            >
              {ratio}
            </span>
            <span
            ...
            return (
              ...
              >
                <button
                  ...
                  className="grid w-full grid-cols-[1fr_auto] items-center gap-4 px-6 py-4 text-left transition-colors hover:bg-[#ece5d6] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-[#8a6d3b] motion-reduce:transition-none sm:grid-cols-[1fr_11rem_2.5rem]"
                >
    ...
    <div className="space-y-6">
      ...
      <ol className="space-y-3">
        {ACTIES.map((a, i) => {
          ...
          const tone = warn ? C.warn : C.taupe;
          const ink = warn ? C.warnInk : C.taupeDeep;
          const wash = warn ? C.warnWash : C.taupeWash;
          return (
            <li key={a.titel}>
              <Panel className="p-6">
        ...
        <ul>
          {FACTUREN.map((f, i) => {
            ...
            return (
              <li
                key={f.nr}
                className="grid grid-cols-[1fr_auto] items-center gap-3 px-6 py-4 transition-colors hover:bg-[#ece5d6] sm:grid-cols-[8rem_1fr_5rem_9rem_6rem] sm:gap-4"
                style={{ borderTop: i === 0 ? "none" : `1px solid ${C.lineSoft}` }}
              >
                <span
