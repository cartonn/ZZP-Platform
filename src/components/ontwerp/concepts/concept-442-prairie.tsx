...
// Concept 442 — "Prairie" · Weids grasland. Warme drooggras-neutralen (tarwe #d8c39a, zand #efe6d2,
// klei #b89b6a) op een licht crème doek, met hemelsblauw (#3d7dbf) als enige koele accent. Horizontale
// windlijnen als hairline-ritme dragen de layout; veel horizontale lucht en weidsheid. Rustig, aards,
// licht. Grote luchtige koppen (serif-display) boven strakke, brede horizontale rijen.
...
// — Palet: drooggras, tarwe, zand, klei — met hemelsblauw als koel accent —
const C = {
  skyBg: "#f6f1e4",
  skyBgDeep: "#efe6d2",
  paper: "#fdfaf1",
  ...
  inkMute: "#8a7c5e",
  inkFaint: "#b6a681",
  line: "rgba(58,50,34,0.14)",
  lineSoft: "rgba(58,50,34,0.07)",
  // tarwe/klei warm accent
  wheat: "#b89b6a",
  wheatDeep: "#8f7444",
  wheatWash: "rgba(184,155,106,0.18)",
  wheatMist: "rgba(184,155,106,0.09)",
  // hemelsblauw koel accent
  sky: "#3d7dbf",
  skyDeep: "#2c5f96",
  skyWash: "rgba(61,125,191,0.14)",
  // status
  ok: "#6f8a4f",
  okInk: "#54693a",
  okWash: "rgba(111,138,79,0.16)",
  warn: "#c68a34",
  warnInk: "#95651f",
  warnWash: "rgba(198,138,52,0.18)",
  info: "#3d7dbf",
  infoInk: "#2c5f96",
  infoWash: "rgba(61,125,191,0.14)",
  bad: "#b0553f",
  badInk: "#863c2a",
  badWash: "rgba(176,85,63,0.16)",
};
...
// Horizontale windlijnen — fijne, wijd uiteen liggende hairlines die het grasland-ritme dragen.
function windLines(): React.CSSProperties {
  return {
    backgroundImage:
      "repeating-linear-gradient(0deg, rgba(58,50,34,0.035) 0px, rgba(58,50,34,0.035) 1px, transparent 1px, transparent 13px)",
  };
}

// — Prairie-paneel: brede, luchtige kaart met een dunne bovenrand-accentlijn (horizon) —
function Panel({
  children,
  className = "",
  as: Tag = "div",
  ...
  className?: string;
  as?: "div" | "section" | "li";
  fill?: "paper" | "wheat" | "sky" | "soft";
  horizon?: string;
}) {
  const base = {
    paper: { color: C.paper, bd: C.line },
    wheat: { color: C.wheatMist, bd: C.wheat },
    sky: { color: C.skyWash, bd: C.sky },
    soft: { color: C.paperSoft, bd: C.lineSoft },
  }[fill];
  return (
    <Tag
      ...
      style={{
        ...
        boxShadow: "0 1px 2px rgba(58,50,34,0.04)",
        color: C.ink,
      }}
    >
      {horizon && (
        <span
          ...
          style={{ background: `linear-gradient(90deg, ${horizon}, transparent)` }}
        />
      )}
      {children}
    </Tag>
...
function Eyebrow({ children, tone = C.wheatDeep }: { children: React.ReactNode; tone?: string }) {
  return (
    <p
...
function PrimaryButton({
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className={`group inline-flex items-center justify-center gap-2 rounded-full px-5 py-2.5 text-[13px] font-semibold transition-all duration-200 hover:brightness-[1.06] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#3d7dbf] focus-visible:ring-offset-2 focus-visible:ring-offset-[#f6f1e4] active:translate-y-px motion-reduce:transition-none ${className}`}
      style={{ color: "#fdfaf1", background: C.sky, border: `1px solid ${C.skyDeep}`, ...bodyFont }}
    >
      {children}
    </button>
  );
...
function GhostButton({
  ...
  return (
    <button
      ...
      className={`inline-flex items-center justify-center gap-2 rounded-full px-4 py-2.5 text-[12.5px] font-semibold transition-all duration-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#3d7dbf] focus-visible:ring-offset-2 focus-visible:ring-offset-[#f6f1e4] motion-reduce:transition-none ${className}`}
      style={{
        color: active ? "#fdfaf1" : C.inkSoft,
        background: active ? C.wheat : C.paper,
        border: `1px solid ${active ? C.wheatDeep : C.line}`,
        ...bodyFont,
      }}
    >
      {children}
...
// — Windvlaag-sparkline: brede, laaggehouden lijn zoals wind over gras —
function WindLine({ data, tone, id }: { data: number[]; tone: string; id: string }) {
  ...
  return (
    ...
    >
      <defs>
        <linearGradient id={`prairie-${id}`} x1="0" y1="0" x2="0" y2="1">
          <stop offset="0%" stopColor={tone} stopOpacity="0.24" />
          <stop offset="100%" stopColor={tone} stopOpacity="0" />
        </linearGradient>
      </defs>
      <polygon points={`0,${h} ${line} ${w},${h}`} fill={`url(#prairie-${id})`} />
      <polyline
        points={line}
        fill="none"
        stroke={tone}
...
export function Concept442() {
  ...
  return (
    <div
      ...
      style={{ ...bodyFont, color: C.ink, background: C.skyBg, ...windLines() }}
    >
      <style>{`
        @keyframes prairieRise { from { opacity: 0; transform: translateY(8px); } to { opacity: 1; transform: translateY(0); } }
        .prairie-rise { animation: prairieRise 0.5s cubic-bezier(0.22, 1, 0.36, 1) both; }
        @keyframes prairieDrift { 0% { transform: translateX(-6px); } 100% { transform: translateX(6px); } }
        .prairie-drift { animation: prairieDrift 11s ease-in-out infinite alternate; }
        @media (prefers-reduced-motion: reduce) { .prairie-rise, .prairie-drift { animation: none !important; } }
      `}</style>

      {/* Weidse hemel-gloed bovenaan */}
      <div
        className="pointer-events-none absolute inset-x-0 top-0 h-64"
        aria-hidden="true"
        style={{ background: "linear-gradient(180deg, rgba(61,125,191,0.08), transparent)" }}
      />

      <div className="relative mx-auto max-w-6xl px-4 pb-24 sm:px-6 md:px-8">
        <TopBar />
        <NavBar screen={screen} setScreen={setScreen} />
        <main key={screen} className="prairie-rise pt-7">
          {screen === "dashboard" && (
    ...
    <header className="flex items-center justify-between gap-4 pt-8">
      <div className="flex items-center gap-3.5">
        <span
          ...
          style={{ background: C.sky, border: `1px solid ${C.skyDeep}`, color: "#fdfaf1" }}
          aria-hidden="true"
        >
          <Wind size={19} />
        </span>
      ...
      <div className="flex items-center gap-3">
        ...
        >
          ...
          {ongelezen > 0 && (
            <span
              ...
              style={{ background: C.sky, color: "#fdfaf1", ...num }}
              aria-hidden="true"
            >
              {ongelezen}
            </span>
        ...
        <span
          ...
          style={{ background: C.wheatWash, border: `1px solid ${C.wheat}`, color: C.wheatDeep }}
          aria-hidden="true"
        >
          {PROFIEL.initialen}
        </span>
        ...
        {SCREENS.map((s) => {
          ...
          return (
            <button
              ...
              className="flex shrink-0 items-center gap-2 rounded-full px-4 py-2 text-[12.5px] font-semibold transition-all duration-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#3d7dbf] focus-visible:ring-offset-2 focus-visible:ring-offset-[#fdfaf1] motion-reduce:transition-none"
              style={{ color: on ? "#fdfaf1" : C.inkMute, background: on ? C.sky : "transparent" }}
            >
              {s.label}
            </button>
          );
  ...
  return (
    <div className="space-y-7">
      <section className="grid grid-cols-1 gap-5 lg:grid-cols-[1.4fr_1fr]">
        <Panel className="p-7 md:p-9" fill="wheat" horizon={C.wheat}>
          <Eyebrow>Vandaag · wijd en open</Eyebrow>
          <div className="mt-4 flex items-start justify-between gap-4">
            ...
            <Wind
              ...
              aria-hidden="true"
              style={{ color: C.wheat }}
              className="prairie-drift hidden sm:block"
            />
          </div>
          <p className="mt-3 max-w-md text-[14px] leading-relaxed" style={{ color: C.inkSoft }}>
            Je praktijk als een weids grasland: overzichtelijk tot aan de horizon, rustig
            meebewegend met de wind. Alles op orde, verifieerbaar en betaald — loop er kalm
            doorheen.
          </p>
        ...
        <Panel className="p-7" fill="sky" horizon={C.sky}>
      ...
      <section>
        <div className="mb-4">
          <Eyebrow>Weidsheid · deze maand</Eyebrow>
        </div>
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {KPIS.map((k, i) => (
            <Panel key={k.label} className="p-5">
              ...
              <div className="mt-3">
                <WindLine data={k.spark} tone={k.up ? C.sky : C.wheat} id={`k442-${i}`} />
              </div>
            </Panel>
          ))}
        </div>
        ...
        <div>
          <div className="mb-4 flex items-baseline justify-between">
            ...
            <button
              type="button"
              onClick={onOpen}
              className="rounded text-[11px] font-semibold uppercase tracking-[0.14em] transition-colors hover:brightness-90 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#3d7dbf] focus-visible:ring-offset-2 focus-visible:ring-offset-[#f6f1e4]"
              ...
              {OPDRACHTEN.map((o, i) => (
                <li key={o.id} style={{ borderTop: i === 0 ? "none" : `1px solid ${C.lineSoft}` }}>
                  <button
                    type="button"
                    onClick={onOpen}
                    className="group grid w-full grid-cols-[auto_1fr_auto] items-center gap-4 px-5 py-4 text-left transition-colors hover:bg-[#f4ecd9] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-[#3d7dbf] motion-reduce:transition-none"
                  >
                    <span
                      className="inline-flex h-10 w-10 items-center justify-center rounded-full"
                      style={{
                        background: i === 0 ? C.skyWash : C.paperSoft,
                    ...
                    <span className="flex items-center gap-3">
                      <WindMeter value={o.match} tone={o.match >= 90 ? C.sky : C.wheat} />
                      <ChevronRight
                        size={17}
                        aria-hidden="true"
                        className="transition-transform group-hover:translate-x-0.5 motion-reduce:transition-none"
  ...
  return (
    <div className="space-y-7">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
        ...
        >
          ...
          <input
            ...
            className="w-full bg-transparent text-[13px] outline-none placeholder:text-[#b6a681]"
            style={{ color: C.ink }}
          />
        </div>
      ...
      {loading ? (
            ...
            <li key={i}>
              <Panel className="p-6">
                <div className="animate-pulse space-y-3 motion-reduce:animate-none">
                  <div className="h-3 w-24 rounded-full" style={{ background: C.paperSoft }} />
                  <div className="h-5 w-2/3 rounded-full" style={{ background: C.skyBgDeep }} />
                  <div className="h-3 w-1/2 rounded-full" style={{ background: C.paperSoft }} />
                  <div className="h-2 w-full rounded-full" style={{ background: C.paperSoft }} />
                </div>
              </Panel>
            </li>
          ))}
        ...
        <Panel className="p-6" fill="wheat">
          <div className="flex flex-col items-center py-14 text-center">
            <span
              className="inline-flex h-16 w-16 items-center justify-center rounded-full"
              style={{ background: C.paper, border: `1px solid ${C.wheat}`, color: C.wheatDeep }}
              aria-hidden="true"
            >
              <Search size={26} />
            </span>
...
function MarktKaart({
  ...
  const tone = strong ? C.sky : C.wheat;
  const toneInk = strong ? C.skyDeep : C.wheatDeep;
  return (
    <Panel className="p-6" horizon={strong ? C.sky : undefined}>
      <div className="grid grid-cols-[1fr_auto] items-start gap-4">
        ...
        <div className="flex flex-col items-center gap-1.5">
          <span
            ...
            style={{ background: strong ? C.skyWash : C.wheatWash, border: `1.5px solid ${tone}` }}
          >
      ...
      <div className="mt-4 flex items-center gap-3">
        <button
          ...
          onClick={() => setOpen((v) => !v)}
          aria-expanded={open}
          className="inline-flex items-center gap-1.5 rounded-full px-3.5 py-1.5 text-[11.5px] font-semibold transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#3d7dbf] focus-visible:ring-offset-2 focus-visible:ring-offset-[#fdfaf1]"
          style={{ color: C.skyDeep, border: `1px solid ${C.line}` }}
        >
  ...
  return (
    <div className="space-y-6">
      <button
        type="button"
        onClick={onBack}
        className="inline-flex items-center gap-2 rounded-full px-4 py-2 text-[12.5px] font-semibold transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#3d7dbf] focus-visible:ring-offset-2 focus-visible:ring-offset-[#f6f1e4]"
        style={{ color: C.inkSoft, border: `1px solid ${C.line}`, background: C.paper }}
      >
      ...
      <Panel className="p-7 md:p-9" fill="wheat" horizon={C.wheat}>
        <div className="flex flex-wrap items-center gap-2">
          ...
          <span
            ...
            style={{ color: "#fdfaf1", background: C.sky }}
          >
        ...
        <div className="flex flex-wrap items-center justify-between gap-6">
          <div className="max-w-md">
            ...
            <p className="mt-3 text-[13.5px] leading-relaxed" style={{ color: C.inkSoft }}>
              <span className="font-semibold" style={{ color: C.wheatDeep }}>
                {PROFIEL.trust}.
              </span>{" "}
          ...
          <span
            ...
            style={{ background: C.paper, border: `1.5px solid ${C.sky}` }}
          >
            <span
            ...
            return (
              ...
              >
                <button
                  ...
                  className="grid w-full grid-cols-[1fr_auto] items-center gap-4 px-6 py-4 text-left transition-colors hover:bg-[#f4ecd9] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-[#3d7dbf] motion-reduce:transition-none sm:grid-cols-[1fr_11rem_2.5rem]"
                >
    ...
    <div className="space-y-6">
      ...
      <ol className="space-y-3">
        {ACTIES.map((a, i) => {
          ...
          return (
            <li key={a.titel}>
              <Panel className="p-6" horizon={warn ? C.warn : C.sky}>
...
function factuurTone(status: string): {
  ...
  return { ink: C.inkMute, wash: C.paperSoft, tone: C.line, Icon: FileText };
}
        ...
        <ul>
          {FACTUREN.map((f, i) => {
            ...
            return (
              <li
                key={f.nr}
                className="grid grid-cols-[1fr_auto] items-center gap-3 px-6 py-4 transition-colors hover:bg-[#f4ecd9] sm:grid-cols-[8rem_1fr_5rem_9rem_6rem] sm:gap-4"
                style={{ borderTop: i === 0 ? "none" : `1px solid ${C.lineSoft}` }}
              >
                <span
