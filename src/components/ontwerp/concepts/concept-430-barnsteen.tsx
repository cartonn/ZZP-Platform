...
// Concept 430 — "Barnsteen" · Hars / warm doorschijnend goudlicht.
// Een premium, warme interface in de taal van barnsteen: honing-goud tinten (#c8811e) op warm
// papier (#faf4e8), tekst #2b2114. Zachte radiale binnen-gloed alsof licht door hars valt.
// Verificaties en certificaten worden "insluitsels" — kostbaar en veilig bewaard in de hars, een
// rustige metafoor voor veilig documentbeheer. Ronde vormen, hoog-contrast tekst, tijdloos-luxe —
// geen glans-kitsch. Micro-interacties (warme gloed, zachte rijs) respecteren reduced-motion.
...
// — Palet: barnsteen-amber, honing, warm papier, diep hars-ink —
const C = {
  paperBg: "#faf4e8",
  paperBgDeep: "#f4ead4",
  paper: "#fdf8ee",
  ...
  inkMute: "#8a7550",
  inkFaint: "#b6a17a",
  line: "rgba(43,33,20,0.12)",
  lineSoft: "rgba(43,33,20,0.07)",
  amber: "#c8811e",
  amberHi: "#e0a542",
  amberDeep: "#9a6015",
  amberWash: "rgba(200,129,30,0.13)",
  amberMist: "rgba(200,129,30,0.07)",
  honey: "#d9a04a",
  ok: "#7f9a3f",
  okInk: "#5e7328",
  okWash: "rgba(127,154,63,0.16)",
  warn: "#c8811e",
  warnInk: "#9a6015",
  warnWash: "rgba(200,129,30,0.16)",
  info: "#b07b3a",
  infoInk: "#875a22",
  infoWash: "rgba(176,123,58,0.14)",
  bad: "#bd6b52",
  badInk: "#96472f",
  badWash: "rgba(189,107,82,0.16)",
};
...
// — Harsgloed: zachte radiale binnen-gloed alsof licht door barnsteen valt —
function Glow({
  className = "",
  tone = C.amberHi,
  intensity = 0.16,
...
// — Insluitsel-badge: een klein "in hars gevangen" merkje, rond met zachte ring —
function Insluitsel({ Icon, tone, size = 40 }: { Icon: LucideIcon; tone: string; size?: number }) {
  return (
    <span
      ...
      style={{
        width: size,
        height: size,
        background: `radial-gradient(circle at 35% 30%, ${C.paper} 0%, ${C.amberMist} 55%, ${C.amberWash} 100%)`,
        border: `1px solid ${C.line}`,
        boxShadow: `inset 0 1px 3px rgba(255,255,255,0.6), inset 0 -3px 8px ${C.amberWash}`,
        color: tone,
      }}
      aria-hidden="true"
    >
      <Icon size={size * 0.42} />
      <span
        className="absolute left-[26%] top-[22%] block h-1.5 w-1.5 rounded-full"
...
function Panel({
  ...
  className?: string;
  as?: "div" | "section" | "li";
  tone?: "paper" | "amber" | "deep";
  glow?: boolean;
}) {
  const bg =
    tone === "amber"
      ? `radial-gradient(120% 140% at 15% 0%, ${C.paper} 0%, ${C.paperSoft} 45%, ${C.amberMist} 100%)`
      : tone === "deep"
        ? C.paperSoft
        : C.paper;
  return (
    <Tag
      ...
      style={{
        ...
        border: `1px solid ${C.line}`,
        boxShadow:
          "0 1px 2px rgba(43,33,20,0.04), 0 12px 32px rgba(154,96,21,0.07), inset 0 1px 0 rgba(255,255,255,0.5)",
        color: C.ink,
      }}
    >
      {glow && <Glow className="-right-16 -top-16 h-56 w-56" />}
      {children}
    </Tag>
  );
}

function Eyebrow({ children, tone = C.amberDeep }: { children: React.ReactNode; tone?: string }) {
  return (
    <p
...
function PrimaryButton({
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className={`group inline-flex items-center justify-center gap-2 rounded-full px-5 py-2.5 text-[13px] font-semibold transition-all duration-300 hover:brightness-[1.05] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#c8811e] focus-visible:ring-offset-2 focus-visible:ring-offset-[#faf4e8] active:translate-y-px motion-reduce:transition-none ${className}`}
      style={{
        color: "#fdf8ee",
        background: `linear-gradient(135deg, ${C.amberHi} 0%, ${C.amber} 55%, ${C.amberDeep} 100%)`,
        boxShadow: "0 6px 18px rgba(154,96,21,0.28)",
        ...body,
      }}
    >
      {children}
...
function GhostButton({
  ...
  return (
    <button
      ...
      className={`inline-flex items-center justify-center gap-2 rounded-full px-4 py-2.5 text-[12.5px] font-semibold transition-all duration-300 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#c8811e] focus-visible:ring-offset-2 focus-visible:ring-offset-[#faf4e8] motion-reduce:transition-none ${className}`}
      style={{
        color: active ? "#fdf8ee" : C.inkSoft,
        background: active ? C.amber : C.paper,
...
// — Honing-sparkline: warme curve met zachte gloed en amber-eindpunt —
function HoningLine({ data, tone, id }: { data: number[]; tone: string; id: string }) {
  ...
  return (
    ...
    >
      <defs>
        <linearGradient id={`honing-${id}`} x1="0" y1="0" x2="0" y2="1">
          <stop offset="0%" stopColor={tone} stopOpacity="0.3" />
          <stop offset="100%" stopColor={tone} stopOpacity="0" />
        </linearGradient>
      </defs>
      <polygon points={`0,${h} ${line} ${w},${h}`} fill={`url(#honing-${id})`} />
      <polyline
        points={line}
        fill="none"
        stroke={tone}
      ...
      <circle cx={last[0]} cy={last[1]} r="2.2" fill={C.paper} stroke={tone} strokeWidth="1.5" />
    </svg>
  );
}

// — Harspeil-meter: warme voortgangsbalk met binnen-gloed —
function HarsMeter({ value, tone = C.amber }: { value: number; tone?: string }) {
  return (
    <span className="hidden items-center gap-2.5 sm:flex" aria-hidden="true">
      <span
        ...
        style={{ background: C.raise, boxShadow: `inset 0 1px 2px ${C.amberWash}` }}
      >
        <span
          className="block h-full rounded-full"
          style={{
            width: `${value}%`,
            background: `linear-gradient(90deg, ${C.honey}, ${tone})`,
  ...
  return (
    <div
      ...
      style={{
        ...body,
        color: C.ink,
        background: `radial-gradient(130% 100% at 20% -10%, ${C.paperBg} 0%, ${C.paperBgDeep} 70%, ${C.paperSoft} 100%)`,
      }}
    >
      <style>{`
        @keyframes barnRise { from { opacity: 0; transform: translateY(8px); } to { opacity: 1; transform: translateY(0); } }
        .barn-rise { animation: barnRise 0.5s cubic-bezier(0.22, 1, 0.36, 1) both; }
        @media (prefers-reduced-motion: reduce) { .barn-rise { animation: none !important; } }
      `}</style>

      {/* warme radiale binnen-gloed, alsof licht door hars valt */}
      <Glow className="-left-24 top-10 h-96 w-96" tone={C.amberHi} intensity={0.14} />
      <Glow className="right-0 top-40 h-80 w-80" tone={C.honey} intensity={0.1} />

      <div className="relative mx-auto max-w-6xl px-4 pb-24 sm:px-6 md:px-8">
        <TopBar />
        <NavBar screen={screen} setScreen={setScreen} />
        <main key={screen} className="barn-rise pt-7">
          {screen === "dashboard" && (
    ...
    <header className="flex items-center justify-between gap-4 pt-8">
      <div className="flex items-center gap-3.5">
        <span
          ...
          style={{
            background: `radial-gradient(circle at 35% 30%, ${C.amberHi} 0%, ${C.amber} 60%, ${C.amberDeep} 100%)`,
            border: `1px solid ${C.amberDeep}`,
            color: "#fdf8ee",
            boxShadow: `0 4px 12px rgba(154,96,21,0.3), inset 0 1px 2px rgba(255,255,255,0.4)`,
          }}
          aria-hidden="true"
        >
          <Gem size={18} />
        ...
        <div>
          ...
          >
            <Sun size={11} aria-hidden="true" style={{ color: C.amber }} />
            {PROFIEL.plaats} · warm bewaard
          </p>
        </div>
      </div>
        ...
        >
          ...
          {ongelezen > 0 && (
            <span
              ...
              style={{ background: C.amber, color: "#fdf8ee", ...num }}
              aria-hidden="true"
            >
              {ongelezen}
            </span>
        ...
        {SCREENS.map((s) => {
          ...
          return (
            <button
              ...
              className="flex shrink-0 items-center gap-2 rounded-full px-4 py-2 text-[12.5px] font-semibold transition-all duration-300 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#c8811e] focus-visible:ring-offset-2 focus-visible:ring-offset-[#fdf8ee] motion-reduce:transition-none"
              style={{
                color: on ? "#fdf8ee" : C.inkMute,
  ...
  return (
    <div className="space-y-7">
      <section className="grid grid-cols-1 gap-5 lg:grid-cols-[1.4fr_1fr]">
        <Panel className="p-7 md:p-9" tone="amber" glow>
          <div className="relative">
            <Eyebrow>Vandaag · warm licht</Eyebrow>
            ...
            <p className="mt-3 max-w-md text-[14px] leading-relaxed" style={{ color: C.inkSoft }}>
              Alles ligt warm en veilig bewaard, als insluitsels in barnsteen. Loop rustig langs je
              acties en houd je praktijk verifieerbaar en betaald — tijdloos en zonder haast.
            </p>
        ...
        <Panel className="p-7">
          ...
          <div className="mt-5 pt-4" style={{ borderTop: `1px solid ${C.lineSoft}` }}>
            <p className="flex items-center gap-2 text-[12px]" style={{ color: C.inkMute, ...num }}>
              <Gem size={13} aria-hidden="true" style={{ color: C.amber }} />
              {verified}/{CREDENTIALS.length} certificaten ingesloten · 7 open reacties
            </p>
          </div>
        </Panel>
      </section>
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {KPIS.map((k, i) => (
            <Panel key={k.label} className="p-5">
              ...
              <div className="mt-3">
                <HoningLine data={k.spark} tone={k.up ? C.amber : C.honey} id={`kpi-${i}`} />
              </div>
            </Panel>
          ))}
        </div>
      ...
      <section className="grid grid-cols-1 gap-5 lg:grid-cols-[1.45fr_1fr]">
        <div>
          <div className="mb-4 flex items-baseline justify-between">
            ...
            <button
              type="button"
              onClick={onOpen}
              className="rounded text-[11px] font-semibold uppercase tracking-[0.14em] transition-colors hover:brightness-90 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#c8811e] focus-visible:ring-offset-2 focus-visible:ring-offset-[#faf4e8]"
              style={{ color: C.amberDeep, ...body }}
            >
              Alle →
            </button>
          </div>
              ...
              {OPDRACHTEN.map((o, i) => (
                <li key={o.id} style={{ borderTop: i === 0 ? "none" : `1px solid ${C.lineSoft}` }}>
                  <button
                    type="button"
                    onClick={onOpen}
                    className="group grid w-full grid-cols-[auto_1fr_auto] items-center gap-4 px-5 py-4 text-left transition-colors hover:bg-[#f7eeda] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-[#c8811e] motion-reduce:transition-none"
                  >
                    <span
                      className="inline-flex h-10 w-10 items-center justify-center rounded-full"
                      style={{
                        background: i === 0 ? C.amberWash : C.raise,
                        border: `1px solid ${i === 0 ? C.amber : C.line}`,
                      }}
                    >
                      <span
                        className="text-[12px] font-bold leading-none"
                        style={{ color: i === 0 ? C.amberDeep : C.inkMute, ...num }}
                      >
                        {o.match}
                      </span>
                    </span>
                    ...
                    <span className="flex items-center gap-3">
                      <HarsMeter value={o.match} tone={o.match >= 90 ? C.amber : C.honey} />
                      <ChevronRight
                        size={17}
                        aria-hidden="true"
                        className="transition-transform group-hover:translate-x-0.5 motion-reduce:transition-none"
        ...
        <div>
          <div className="mb-4">
            <Eyebrow>Insluitsels · certificaten</Eyebrow>
          </div>
          <Panel className="p-5">
            <ul className="space-y-1">
              {CREDENTIALS.map((c, i) => {
                ...
                return (
                  ...
                  >
                    <Insluitsel Icon={st.Icon} tone={st.ink} size={34} />
  ...
  return (
    <div className="space-y-7">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
        ...
        >
          ...
          <input
            ...
            className="w-full bg-transparent text-[13px] outline-none placeholder:text-[#b6a17a]"
            style={{ color: C.ink, ...body }}
          />
        </div>
      ...
      {loading ? (
        ...
        <Panel className="p-6" tone="amber" glow>
          <div className="relative flex flex-col items-center py-14 text-center">
            <Insluitsel Icon={Search} tone={C.amberDeep} size={64} />
            ...
            <p className="mx-auto mt-2 max-w-xs text-[13.5px]" style={{ color: C.inkSoft }}>
              Geen opdracht bij {q ? `“${q}”` : "je zoekterm"} bleef hangen. Verruim je zoekterm en
              probeer opnieuw.
            </p>
            <div className="mt-6">
...
function MarktKaart({
  ...
  const tone = strong ? C.amber : C.honey;
  const toneInk = strong ? C.amberDeep : C.warnInk;
  return (
    <Panel className="p-6">
      <div className="grid grid-cols-[1fr_auto] items-start gap-4">
        ...
        <div className="flex flex-col items-center gap-1.5">
          <span
            ...
            style={{
              background: `radial-gradient(circle at 35% 30%, ${C.paper} 0%, ${strong ? C.amberWash : C.amberMist} 100%)`,
              border: `1.5px solid ${tone}`,
              boxShadow: `inset 0 -3px 8px ${C.amberWash}`,
            }}
          >
      ...
      <div className="mt-4 flex items-center gap-3">
        <button
          ...
          onClick={() => setOpen((v) => !v)}
          aria-expanded={open}
          className="inline-flex items-center gap-1.5 rounded-full px-3.5 py-1.5 text-[11.5px] font-semibold transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#c8811e] focus-visible:ring-offset-2 focus-visible:ring-offset-[#fdf8ee]"
          style={{ color: C.amberDeep, border: `1px solid ${C.line}`, ...body }}
        >
  ...
  return (
    <div className="space-y-6">
      <button
        type="button"
        onClick={onBack}
        className="inline-flex items-center gap-2 rounded-full px-4 py-2 text-[12.5px] font-semibold transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#c8811e] focus-visible:ring-offset-2 focus-visible:ring-offset-[#faf4e8]"
      ...
      <Panel className="p-7 md:p-9" tone="amber" glow>
        <div className="relative flex flex-wrap items-center gap-2">
          ...
          <span
            ...
            style={{ color: "#fdf8ee", background: tone, ...body }}
          >
      ...
      <section>
        ...
        <p className="mt-3 max-w-xl text-[13.5px] leading-relaxed" style={{ color: C.inkSoft }}>
          Afgelezen van je geverifieerde profiel — wat je meebrengt én waar de aandacht ligt, warm
          en transparant, zonder verborgen score.
        </p>
...
function Verificatie() {
  ...
  return (
    <div className="space-y-6">
      <Panel className="p-7 md:p-9" tone="amber" glow>
        <div className="relative flex flex-wrap items-center justify-between gap-6">
          <div className="max-w-md">
            <Eyebrow>Verificatie · veilig ingesloten</Eyebrow>
            <h1
            ...
            <p className="mt-3 text-[13.5px] leading-relaxed" style={{ color: C.inkSoft }}>
              <span className="font-semibold" style={{ color: C.amberDeep }}>
                {PROFIEL.trust}.
              </span>{" "}
              {verified} van {CREDENTIALS.length} certificaten liggen veilig ingesloten en
              geverifieerd. Eén verloopt binnenkort en vraagt om vernieuwing. Documenten blijven
              versleuteld en privé.
            </p>
            <div className="mt-4 max-w-xs">
              <HarsMeter value={ratio} tone={C.amber} />
            </div>
          </div>
          <span
            className="relative inline-flex h-24 w-24 flex-col items-center justify-center rounded-full"
            style={{
              background: `radial-gradient(circle at 35% 30%, ${C.paper} 0%, ${C.amberWash} 100%)`,
              border: `1.5px solid ${C.amber}`,
              boxShadow: `inset 0 -4px 12px ${C.amberWash}`,
            }}
          >
            <span
              className="text-[26px] font-semibold leading-none"
              style={{ color: C.amberDeep, ...num }}
            >
              {ratio}
            </span>
            <span
            ...
            return (
              ...
              >
                <button
                  ...
                  className="grid w-full grid-cols-[1fr_auto] items-center gap-4 px-6 py-4 text-left transition-colors hover:bg-[#f7eeda] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-[#c8811e] motion-reduce:transition-none sm:grid-cols-[1fr_11rem_2.5rem]"
                >
                  <span className="flex min-w-0 items-center gap-3">
                    <Insluitsel Icon={st.Icon} tone={st.ink} size={38} />
                    <span className="min-w-0">
                      <span
                        className="block truncate text-[15px] font-semibold"
                        style={{ color: C.ink, ...body }}
      ...
      <div>
        <div className="mb-4">
          <Eyebrow tone={C.amberDeep}>Documentenkast</Eyebrow>
        </div>
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-2 max-w-md text-[13.5px]" style={{ color: C.inkSoft }}>
          Rustig van boven naar beneden — zo blijf je verifieerbaar en betaald, warm en op orde.
        </p>
      </div>
      ...
      <ol className="space-y-3">
        {ACTIES.map((a, i) => {
          ...
          const ink = warn ? C.warnInk : C.amberDeep;
      ...
      <Panel>
        ...
        <ul>
          {FACTUREN.map((f, i) => {
            ...
            return (
              <li
                key={f.nr}
                className="grid grid-cols-[1fr_auto] items-center gap-3 px-6 py-4 transition-colors hover:bg-[#f7eeda] sm:grid-cols-[8rem_1fr_5rem_9rem_6rem] sm:gap-4"
                style={{ borderTop: i === 0 ? "none" : `1px solid ${C.lineSoft}` }}
              >
                <span
        ...
        >
          <span
            ...
            style={{ color: C.inkMute, ...body }}
          >
            <Gem size={12} aria-hidden="true" style={{ color: C.amber }} /> Totaal betaald
          </span>
