 ...
 */
export async function changePassword(_prev: ChangePasswordState | undefined, formData: FormData): Promise<ChangePasswordState> {
  ...
  await prisma.user.update({ where: { id: actor.id }, data: { passwordHash, mustChangePassword: false } });

  const meta = await requestMeta();
  await audit({ actorId: actor.id, action: "PASSWORD_CHANGED", entityType: "User", entityId: actor.id, ...meta });
