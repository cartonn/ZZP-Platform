...
export async function importDienstenAction(
  ...
  if (col.status !== "ACTIVE") {
    return { imported: 0, skipped: 0, errors: ["De samenwerking moet actief zijn om diensten te importeren."] };
  }
  ...
  for (const parsed of parsedShifts) {
    try {
      ...
      const perfId = await createPerformance(actor, {
        ...
        periodStart,
        periodEnd,
        description: parsed.description || `Geïmporteerde dienst ${parsed.start.toLocaleDateString("nl-NL")}`,
      });
      ...
      errors.push(
        `Dienst ${parsed.start.toLocaleDateString("nl-NL")}: ${msg}`,
      );
      skipped++;
    }
  }
