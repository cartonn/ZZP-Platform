...
import { getTenantFeeLines } from "@/lib/franchise/billing";
import { exportTenantFeesCsv } from "@/lib/tenant-billing/tenant-fee-lines";
...
export async function GET() {
  ...
  const limited = await enforceRateLimit(exportRateLimiter, `franchise-fees:${actor.id}`);
  if (limited) return limited;

  const lines = await getTenantFeeLines(actor);
  const csv = exportTenantFeesCsv(lines);
  ...
  await prisma.auditLog.create({
    data: auditData({
      actorId: actor.id,
      action: "TENANT_FEES_EXPORTED",
      entityType: "CollaborationFee",
      entityId: "self",
  ...
  return new NextResponse(csv, {
    headers: {
      ...
      "Content-Disposition": `attachment; filename="bemiddeling-fees-${date}.csv"`,
    },
  });
}
