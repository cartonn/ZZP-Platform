    ...
    <div className="mx-auto max-w-2xl space-y-6">
      <header className="space-y-1">
        ...
        <p className="text-sm text-muted-foreground">
          Samenwerkingen met een open dispuut. Het werkproces is bevroren tot het platform bemiddelt en het dispuut oplost.
        </p>
      </header>

      {disputed.length === 0 ? (
        <Card>
          <EmptyState icon={AlertTriangle} title="Geen open disputen" description="Er zijn op dit moment geen bevroren samenwerkingen." />
        </Card>
      ) : (
          ...
          {disputed.map((c) => (
            <Card key={c.id}>
              <CardContent className="space-y-2 py-3">
                ...
                <Link href={`/samenwerkingen/${c.id}`} className="inline-flex text-sm font-medium underline underline-offset-4">
