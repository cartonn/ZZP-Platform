...
const STATUS_LABEL: Record<string, { label: string; variant: "muted" | "warning" | "success" | "danger" | "default" }> = {
    ...
    <div className="mx-auto max-w-3xl space-y-6">
      <header className="space-y-1">
        ...
        <p className="text-sm text-muted-foreground">
          Overzicht van alle samenwerkingen — contract tot betaling. Klik om naar het werkproces te gaan.
        </p>
      </header>

      {collaborations.length === 0 ? (
        <Card>
          <EmptyState icon={Handshake} title="Geen samenwerkingen" description="Er zijn nog geen samenwerkingen aangemaakt." />
        </Card>
      ) : (
        <div className="space-y-2">
          {collaborations.map((c) => {
            ...
            const paidInv = c.invoices.filter((i) => ["PAID", "PROCESSED"].includes(i.lifecycleStatus ?? "")).length;

            return (
              <Card key={c.id} className="card-interactive">
                <CardContent className="py-3">
                  <Link href={`/samenwerkingen/${c.id}`} className="block space-y-2 hover:no-underline">
                    <div className="flex flex-wrap items-center justify-between gap-2">
                      <div className="flex flex-wrap items-center gap-2 min-w-0">
                        <span className="font-medium truncate">{c.job.title}</span>
                    ...
                    <div className="flex flex-wrap items-center gap-3 text-xs text-muted-foreground">
                      {pendingPerf > 0 && (
                        <span className="text-warning">{pendingPerf} prestatie(s) wachten op goedkeuring</span>
                      )}
                      {pendingInv > 0 && (
                        <span className="text-warning">{pendingInv} factuur/facturen wachten op goedkeuring</span>
                      )}
                      {paidInv > 0 && <span>{paidInv} betaalde factuur/facturen</span>}
                      {dba.level !== "LAAG" && (
                        <Badge variant={DBA_VARIANT[dba.level] ?? "muted"}>DBA {DBA_LEVEL_LABEL[dba.level]}</Badge>
                      )}
                      {c.startDate && (
                        <span>Gestart: {fmt(c.startDate)}</span>
                      )}
                    </div>
                  </Link>
                </CardContent>
