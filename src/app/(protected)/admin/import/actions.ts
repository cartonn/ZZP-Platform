...
async function readCsv(formData: FormData): Promise<{ text: string } | { error: string }> {
  ...
  if (file.size > MAX_CSV_BYTES)
    return { error: `Bestand te groot (max ${MAX_CSV_BYTES / 1024 / 1024} MB).` };
...
export async function previewImport(
  _prev: ServerImportPreview | null,
  formData: FormData,
  ...
  const empty = {
    rows: [],
    summary: { total: 0, importable: 0, errors: 0, warnings: 0, duplicatesInFile: 0, existing: 0 },
  };
  if ("error" in read) return { ...empty, fileError: read.error };

  const records = parseCsvRecords(read.text);
  if (records.length < 2)
    return { ...empty, fileError: "Het bestand bevat geen datarijen onder de kopregel." };
  const cols = mapColumns(records[0] ?? []);
  if (cols.name < 0 || cols.email < 0 || cols.role < 0) {
    return {
      ...empty,
      fileError:
        "Verplichte kolommen ontbreken. Zorg voor minstens 'naam', 'email' en 'rol' — gebruik de voorbeeld-CSV.",
    };
  }
  if (records.length - 1 > MAX_ROWS) {
    return {
      ...empty,
      fileError: `Te veel rijen (${records.length - 1}). Splits het bestand in delen van maximaal ${MAX_ROWS}.`,
    };
  }
  ...
  for (let idx = 0; idx < toCreate.length; idx++) {
    ...
    try {
      ...
      if (mailer) {
        try {
          await mailer.send(
            buildWelcomeEmail({ name: row.name, email: row.email, tempPassword, loginUrl: url }),
          );
          emailSent = true;
        } catch (mailErr) {
      ...
      result.failed.push({
        rowNumber: row.rowNumber,
        email: row.email,
        reason: "Aanmaken mislukt (technische fout).",
      });
    }
  }
