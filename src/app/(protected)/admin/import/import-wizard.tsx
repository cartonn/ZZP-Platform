...
function CopyButton({ text, label = "Kopiëren" }: { text: string; label?: string }) {
  ...
  return (
    <button
      ...
      className="focus-ring inline-flex items-center gap-1 rounded-md border border-input bg-background px-2 py-1 text-xs hover:bg-accent"
    >
      {copied ? (
...
export function ImportWizard({ emailConfigured }: { emailConfigured: boolean }) {
  ...
  if (result) {
    ...
    const credentials = needsManualShare
      ...
      <div className="space-y-4">
        <div className="flex flex-wrap items-center gap-2">
          ...
          {result.emailFailures > 0 && (
            <Badge variant="warning">{result.emailFailures} mail mislukt</Badge>
          )}
          {result.skippedExisting.length > 0 && (
            <Badge variant="muted">{result.skippedExisting.length} bestond al</Badge>
          )}
          {result.failed.length > 0 && (
            <Badge variant="danger">{result.failed.length} mislukt</Badge>
          )}
        </div>
    ...
    return (
      <div className="space-y-4">
              ...
              {preview.rows.map((row) => {
                ...
                const notes = [
                  ...
                  ...(row.unknownSkills.length > 0
                    ? [`Onbekende vaardigheden: ${row.unknownSkills.join(", ")}.`]
                    : []),
                ];
                return (
        ...
        {emailConfigured ? (
          ...
          <p className="text-xs text-muted-foreground">
            E-mailverzending is niet geconfigureerd; je krijgt de tijdelijke wachtwoorden op het
            scherm te zien.
          </p>
        )}
        ...
        <div className="flex flex-wrap items-center gap-2">
          <Button
            onClick={doCommit}
            disabled={pending || summary.importable - summary.existing === 0}
          >
            {pending
              ? "Bezig…"
              : `Bevestig import (${summary.importable - summary.existing} aanmaken)`}
          </Button>
          ...
          {summary.importable - summary.existing === 0 && (
            <span className="text-xs text-muted-foreground">
              Geen nieuwe accounts om aan te maken.
            </span>
          )}
        </div>
      </div>
  ...
  return (
    <form onSubmit={doPreview} className="space-y-4 rounded-lg border border-border bg-card p-5">
      ...
      <p className="flex items-center gap-3 text-xs text-muted-foreground">
        ...
        <span className="inline-flex items-center gap-1">
          <AlertTriangle className="size-3.5 text-warning" aria-hidden /> Waarschuwing (wordt
          aangemaakt)
        </span>
        <span className="inline-flex items-center gap-1">
