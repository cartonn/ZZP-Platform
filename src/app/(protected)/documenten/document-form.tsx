...
export function DocumentForm() {
  const [state, formAction, isPending] = useActionState<DocumentState, FormData>(uploadDocument, undefined);
  ...
  return (
    <form ref={formRef} action={formAction} className="space-y-4 rounded-lg border border-border bg-card p-5">
      ...
      <div className="grid gap-4 sm:grid-cols-[10rem_1fr]">
        ...
        <Field label="Bestand" htmlFor="document" error={fe.document} hint="PDF, PNG, JPEG of WEBP, max 10 MB. Privé.">
          <Input id="document" name="document" type="file" accept="application/pdf,image/png,image/jpeg,image/webp" className="h-auto py-1.5" />
        </Field>
      </div>
      ...
      <div className="flex items-center gap-3">
        <Button type="submit" disabled={isPending}>{isPending ? "Uploaden…" : "Uploaden"}</Button>
