...
export function MessageComposer({ conversationId }: { conversationId: string }) {
  ...
  return (
    <form ref={formRef} action={formAction} className="flex items-end gap-2 border-t border-border pt-3">
      <Textarea name="body" rows={2} required maxLength={5000} placeholder="Schrijf een bericht…" className="flex-1" />
      <Button type="submit" disabled={isPending}>{isPending ? "…" : "Versturen"}</Button>
