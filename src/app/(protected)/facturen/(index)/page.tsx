      ...
      {invoices.length === 0 ? (
        ...
        <div className="divide-y divide-border overflow-hidden rounded-lg border border-border bg-card">
          {invoices.map((inv) => {
            ...
            const href = cascade && inv.collaborationId ? `/samenwerkingen/${inv.collaborationId}` : `/facturen/${inv.id}`;
            const cascadeMeta = cascade ? CASCADE_LABEL[inv.lifecycleStatus as InvoiceLifecycleState] : null;
            const displayNumber = cascade ? inv.partyInvoiceNumber ?? "Concept-factuur" : inv.number;
            return (
              <Link key={inv.id} href={href} className="card-interactive flex items-center justify-between gap-4 px-4 py-3">
