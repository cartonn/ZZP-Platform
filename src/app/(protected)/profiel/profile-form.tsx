    ...
    <form action={formAction} className="space-y-6">
      <div className="grid gap-4 sm:grid-cols-2">
        <div className="sm:col-span-2">
          <Field label="Functietitel" htmlFor="headline" error={fe.headline} hint="Bijv. Senior Frontend Developer">
        ...
        <Field label="Uurtarief (€)" htmlFor="hourlyRate" error={fe.hourlyRate}>
          <Input id="hourlyRate" name="hourlyRate" type="number" min={0} max={2000} defaultValue={initial.hourlyRate} />
        </Field>
        ...
        <div className="sm:col-span-2">
          <Field label="Talen" htmlFor="languages" error={fe.languages} hint="Komma-gescheiden, bijv. nl, en, de">
            <Input id="languages" name="languages" defaultValue={initial.languages} placeholder="nl, en" />
          </Field>
        </div>
      ...
      <fieldset>
        ...
        {skills.length === 0 ? (
          ...
          <div className="flex flex-wrap gap-2">
            {skills.map((s) => (
              <CheckChip key={s.id} name="skillIds" value={s.id} label={s.name} defaultChecked={initial.skillIds.includes(s.id)} />
            ))}
          </div>
        )}
      </fieldset>
      ...
      <fieldset>
        ...
        {industries.length === 0 ? (
          ...
          <div className="flex flex-wrap gap-2">
            {industries.map((i) => (
              <CheckChip key={i.id} name="industryIds" value={i.id} label={i.name} defaultChecked={initial.industryIds.includes(i.id)} />
            ))}
          </div>
        )}
      </fieldset>
      ...
      <fieldset className="space-y-2">
        ...
        <label className="flex items-center gap-2 text-sm">
          <input type="radio" name="visibility" value="PUBLIC" defaultChecked={initial.visibility === "PUBLIC"} />
          Openbaar — opdrachtgevers kunnen mijn profiel vinden
        </label>
        <label className="flex items-center gap-2 text-sm">
          <input type="radio" name="visibility" value="PRIVATE" defaultChecked={initial.visibility === "PRIVATE"} />
