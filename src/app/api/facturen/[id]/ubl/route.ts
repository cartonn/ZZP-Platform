...
import { buildInvoiceUbl } from "@/lib/invoice-ubl";
...
// UBL 2.1 e-factuur van dezelfde factuur, on-demand samengesteld. Alleen de betrokken partijen
// (uitschrijver, opdrachtgever) of een admin mogen 'm inzien (server-side waarheid, CLAUDE.md regel 1+2).
export async function GET(_req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  // On-demand generatie kan een scripted loop lokken; rem 'm (authz blijft de bron van toegang).
  ...
  if (!inv) {
    ...
    await auditDeniedAccess({
      actorId: actor.id,
      action: "INVOICE_UBL_ACCESS_DENIED",
      entityType: "Invoice",
      entityId: id,
      outcome: "not-found",
  ...
  // Cross-Origin-Resource-Policy same-origin (geen cross-origin embedding van deze factuur-UBL).
  return new NextResponse(xml, {
    headers: privateFileHeaders("application/xml", `factuur-${safeNumber}.xml`),
  });
}
