...
export function upcomingWindows<T extends WindowLike>(windows: readonly T[], now: Date = new Date()): T[] {
  return windows
...
export function currentOrNextAvailable<T extends WindowLike>(windows: readonly T[], now: Date = new Date()): T | null {
  const usable = upcomingWindows(windows, now).filter((w) => w.type !== "UNAVAILABLE");
  const covering = usable.find((w) => w.startDate.getTime() <= now.getTime() && w.endDate.getTime() >= now.getTime());
...
export function summarizeAvailability(windows: readonly WindowLike[], now: Date = new Date()): string | null {
