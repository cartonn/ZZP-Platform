...
export async function runConceptInvoiceReminderTask(opts: {
  actorId?: string | null;
  now?: Date;
}): Promise<ConceptReminderResult> {
      ...
      await prisma.$transaction([
        ...
        ...admins.map((a) =>
          prisma.notification.create({
            data: {
              userId: a.id,
              type: "INVOICE_DRAFT_ESCALATION",
              title: "Concept-factuur blijft liggen",
              body: `${num} staat al ${e.daysSince} dagen op concept en is niet ingediend.`,
              link: "/admin/disputen",
            },
          }),
        ),
        prisma.auditLog.create({
          data: auditData({
            actorId: opts.actorId ?? null,
            action: "INVOICE_DRAFT_ESCALATED",
            entityType: "Invoice",
