...
export function assessCollaborationDba(input: DbaMonitorInput, now: Date = new Date()): DbaAssessment {
  ...
  if (input.sameFunctionAsEmployees) {
    signals.push({ key: "same-function", level: "VERHOOGD", message: "Dezelfde functie als werknemers in dienst kan richting een dienstverband wijzen." });
  }
  if (input.fixedSchedule) {
    signals.push({ key: "fixed-schedule", level: "VERHOOGD", message: "Een vaste roosterstructuur lijkt op een dienstverband." });
  }
  if (input.directionAndSupervision) {
    signals.push({ key: "supervision", level: "HOOG", message: "Kenmerken van leiding en toezicht wijzen op een gezagsverhouding — een werknemerskenmerk." });
  }
