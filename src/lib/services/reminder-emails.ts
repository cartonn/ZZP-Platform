...
export function buildCredentialExpiryWarningEmail(input: CredentialExpiryWarningInput): MailMessage {
...
export function buildPaymentOverdueEmail(input: PaymentOverdueEmailInput): MailMessage {
  ...
  const text = [`Hallo ${input.name},`, "", body, "", `Facturen bekijken: ${input.loginUrl}/facturen`, "", "Met vriendelijke groet,", platform].join("\n");
  return {
    to: recipient(input.name, input.email),
    subject,
    text,
...
export function buildConceptInvoiceReminderEmail(input: ConceptInvoiceReminderEmailInput): MailMessage {
  const platform = PLATFORM;
