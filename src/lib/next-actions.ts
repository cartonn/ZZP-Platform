...
export const P = {
  ...
  performanceApprovalOverdue: 67, // ingediende urenstaat/oplevering wacht al ≥ PERFORMANCE_WAIT_ATTENTION_DAYS (7) op goedkeuring door de opdrachtgever — voorbij de dag-3/7-herinneringscadans blokkeert het stil de facturatie-cascade én de cashflow van de ZZP'er. Escaleert boven een verse te-keuren indiening (approve-band 65) zodat het actiecentrum de fase van /prestaties ("Wacht al X dagen op goedkeuring") en de e-mailnudge niet tegenspreekt, maar blijft ONDER het af-te-ronden-hire-cluster (proposeCollaboration 68): een nieuwe inzet vastleggen deblokkeert een relatie die nog helemaal niet loopt. Rol-geïsoleerd (CLIENT).
  ...
  credentialExpiringDuringPlacement: 71, // vereist certificaat van een lopende samenwerking verloopt ná het 30-daagse venster maar vóór de einddatum van de plaatsing (einddatum-verankerd; ZZP'er-spiegel van de opdrachtgever-waarschuwing expiringDuringPlacement). Plaatsing-gebonden (concreet inzet-/inkomensrisico), dus BOVEN een generiek verlopend certificaat (credentialExpiring 70) — zelfde principe als credentialExpiringForCollab(73)>credentialExpiring(70) — maar ONDER een contract-ter-ondertekening (contractSign 72) en de binnen-venster-variant (credentialExpiringForCollab 73: die is imminenter, ≤30 dagen)
  ...
  billingProfileIncomplete: 47, // ZZP'er factureert al, maar het profiel mist een hard facturatie-gegeven (btw-id/IBAN) → élke uitgaande (auto-)factuur is juridisch onvolledig (art. 35a Wet OB) of onbetaalbaar. Een staande, systemische administratie-blokkade (raakt álle facturen), dus urgenter dan een aflopende relatie (collaborationRenewal 46), een koud-lopende opdracht (jobNeedsAttention 44) of cosmetische compleetheid (completeness 30) — maar minder acuut dan concreet openstaand/verstreken geld (overdueInvoice 60 / conceptInvoiceAging 59) en de harde fiscale deadlines: die dragen een directe boete-/cashflow-klok, dit is een eenmalige profielfix. Rol-geïsoleerd (ZZP'er).
  ...
  openCapacity: 42, // open (onbenutte) beschikbare dagen zonder opdracht — een concrete, plan-bare inkomstenkans ("je hebt N open dagen — pak werk op") is actueler dan de passieve "je gedeelde beschikbaarheid is verlopen"-nudge (availabilityStale 40), maar blijft een zachte kans, dus ONDER de concrete attentie-/inbound-taken (jobNeedsAttention 44). Rol-geïsoleerd (ZZP'er).
  ...
  franchiserRosterReengagement: 54, // bench-ZZP'er stilgevallen (geen lopende opdracht, ≥DORMANT_IDLE_DAYS niet ingelogd) — de aanbod-spiegel van franchiserClientReengagement: een warme, bestaande roster-relatie opnieuw benaderen vóór hij afkoelt/wegdrijft. ONDER de klant-re-engagement (55): een stilgevallen opdrachtgever is een hele vraag-relatie die meerdere diensten kan opleveren, een enkele bench-vakmens is één aanbod-eenheid. BOVEN de koude lead-opvolging (50): een bestaande roster-relatie warm houden weegt zwaarder dan koude acquisitie (zelfde principe als de klant-re-engagement). Rol-geïsoleerd (alleen franchiser-taken); deelt de waarde met respondInvitation (54, FREELANCER) — die worden nooit samen gerangschikt
