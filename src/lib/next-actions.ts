...
export const P = {
  ...
  clientCascadePaymentDueSoon: 56, // cascade-factuur van de opdrachtgever vervalt BINNENKORT (goedgekeurd, nog niet te laat) — pre-due tegenhanger van clientCascadeOverduePayment (59). Deelt de band met de legacy paymentDueSoon (56): beide zijn CLIENT pre-due betaal-nudges (legacy = roll-up → /facturen, cascade = per-factuur deep-link → samenwerking). Ze kunnen coëxisteren maar zijn even urgent; gelijke band met deterministische tie-break (rankTasks: insertion order) is correct en flicker-vrij. Post-due (clientCascadeOverduePayment 59) blijft strikt boven deze pre-due nudge (post-due > pre-due-principe).
