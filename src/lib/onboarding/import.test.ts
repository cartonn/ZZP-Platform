...
describe("buildImportPreview", () => {
  ...
  it("opdrachtgever zonder bedrijfsnaam is een fout", () => {
    ...
    const p = buildImportPreview(csv);
    expect(p.rows[0]!.issues.some((i) => i.field === "companyName" && i.level === "error")).toBe(true);
  ...
  it("dubbel e-mailadres binnen het bestand → tweede rij fout", () => {
    const csv = [
      header,
      "Anna;dup@example.nl;ZZP'er;;;",
      "Bram;dup@example.nl;ZZP'er;;;",
    ].join("\r\n");
    const p = buildImportPreview(csv);
  ...
  it("onleesbaar uurtarief is een waarschuwing, niet een fout", () => {
    ...
    expect(p.rows[0]!.issues.some((i) => i.level === "warning" && i.field === "hourlyRate")).toBe(true);
  });
