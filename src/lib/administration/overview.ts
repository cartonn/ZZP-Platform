...
function netDebit(entries: readonly LedgerEntry[], party: LedgerParty, account: LedgerAccount): number {
  return norm(
    entries
...
export function revenueCents(entries: readonly LedgerEntry[], party: LedgerParty, year: number): number {
...
export function vatReturn(
  ...
  const inPeriod = entries.filter((e) => e.occurredAt.getFullYear() === year && quarterOf(e.occurredAt) === quarter);
  ...
  return { year, quarter, party, payableCents, deductibleCents, balanceCents: norm(payableCents - deductibleCents) };
}
