...
describe("EventBus", () => {
  ...
  it("weigert dezelfde handlernaam tweemaal voor één type", () => {
    ...
    expect(() => bus.register({ name: "dup", type: "CONTRACT_SIGNED", handle: vi.fn() })).toThrowError(
      /al geregistreerd/,
    );
  });
  ...
  it("groepeert events van één cascade via correlationId", async () => {
    ...
    await bus.publish(baseInput({ correlationId: "corr-1", type: "PERFORMANCE_SUBMITTED", subjectType: "Performance" }));
