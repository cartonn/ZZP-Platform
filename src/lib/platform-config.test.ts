...
function mapCfgToThresholds(cfg: {
  ...
  return {
    ...
    durationStrongSignalMonths: cfg?.dbaStrongDurationMonths ?? DBA_THRESHOLDS.durationStrongSignalMonths,
    revenueConcentrationPct: cfg?.dbaRevenueConcentrationPct ?? DBA_THRESHOLDS.revenueConcentrationPct,
  };
}
...
describe("mapCfgToThresholds (getDbaThresholds mapping logic)", () => {
  ...
  it("gebruikt de databasewaarden als de configuratierij bestaat", () => {
    const cfg = { dbaMinDurationMonths: 3, dbaStrongDurationMonths: 9, dbaRevenueConcentrationPct: 70 };
