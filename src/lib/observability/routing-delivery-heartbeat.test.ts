...
// verifiëren zonder echte databank. De module logt bewust via de logger zodat een DB-storing in de
// observability-laag een reistijd-lookup nooit alsnog laat falen.
const findUnique = vi.hoisted(() => vi.fn());
const upsert = vi.hoisted(() => vi.fn(async () => ({})));
const loggerError = vi.hoisted(() => vi.fn());

vi.mock("@/lib/db", () => ({
  prisma: { routingDeliveryHeartbeat: { findUnique, upsert } },
}));
...
import {
  ROUTING_DELIVERY_CHANNEL,
  __resetRoutingHeartbeatCoalescingForTests,
  getRoutingDeliveryFreshness,
...
beforeEach(() => {
  ...
  __resetRoutingHeartbeatCoalescingForTests();
  process.env.ROUTING_HEARTBEAT_COALESCE_MS = "15000";
});

afterEach(() => {
  delete process.env.ROUTING_HEARTBEAT_COALESCE_MS;
});

describe("record success/failure — upsert-vorm", () => {
  it("succes zet lastOk=true, teller op 0, driver geoapify, op het singleton-kanaal", async () => {
    await recordRoutingDeliverySuccess(T0);
    expect(upsert).toHaveBeenCalledWith(
      expect.objectContaining({
        where: { channel: ROUTING_DELIVERY_CHANNEL },
  ...
  it("mislukking telt de teller atomair op en zet lastOk=false", async () => {
    await recordRoutingDeliveryFailure(T0);
    expect(upsert).toHaveBeenCalledWith(
      expect.objectContaining({
        update: expect.objectContaining({
          lastOk: false,
...
describe("write-coalescing", () => {
  it("coalesceert opeenvolgende successen binnen het venster (max één schrijf)", async () => {
    await recordRoutingDeliverySuccess(T0);
    await recordRoutingDeliverySuccess(at(5_000));
    await recordRoutingDeliverySuccess(at(14_999));
    expect(upsert).toHaveBeenCalledTimes(1);
  });

  it("schrijft opnieuw zodra het venster verstreken is", async () => {
    await recordRoutingDeliverySuccess(T0);
    await recordRoutingDeliverySuccess(at(15_001));
    expect(upsert).toHaveBeenCalledTimes(2);
  });

  it("een mislukking schrijft altijd direct (nooit gecoalesceerd) en een herstel ook", async () => {
    await recordRoutingDeliverySuccess(T0); // schrijf 1
    await recordRoutingDeliveryFailure(at(1_000)); // schrijf 2 (direct)
    await recordRoutingDeliverySuccess(at(2_000)); // schrijf 3 (herstel, direct — binnen venster)
    expect(upsert).toHaveBeenCalledTimes(3);
  });
});

describe("fail-open — een DB-storing mag nooit doorbreken", () => {
  it("een upsert-fout wordt geslikt en via de logger gemeld", async () => {
    upsert.mockRejectedValueOnce(new Error("db down"));
    await expect(recordRoutingDeliverySuccess(T0)).resolves.toBeUndefined();
  ...
  it("een leesfout geeft 'never' terug i.p.v. te werpen", async () => {
    ...
    const f = await getRoutingDeliveryFreshness(T0);
  ...
  it("na een mislukte success-schrijf coalesceert de volgende success niet weg", async () => {
    ...
    await recordRoutingDeliverySuccess(T0);
    await recordRoutingDeliverySuccess(at(1_000)); // moet opnieuw proberen (binnen venster)
    expect(upsert).toHaveBeenCalledTimes(2);
  });
});
...
describe("freshness-lezing", () => {
  it("leest het singleton-kanaal en beoordeelt failing bij lastOk=false", async () => {
    ...
    const f = await getRoutingDeliveryFreshness(T0);
    expect(findUnique).toHaveBeenCalledWith({ where: { channel: ROUTING_DELIVERY_CHANNEL } });
