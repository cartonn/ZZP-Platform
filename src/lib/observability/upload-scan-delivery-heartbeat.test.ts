...
// observability-laag een upload nooit alsnog laat falen.
...
import {
  UPLOAD_SCAN_CHANNEL,
  __resetUploadScanHeartbeatCoalescingForTests,
  getUploadScanDeliveryFreshness,
...
beforeEach(() => {
  ...
  __resetUploadScanHeartbeatCoalescingForTests();
  process.env.UPLOAD_SCAN_HEARTBEAT_COALESCE_MS = "15000";
});

afterEach(() => {
  delete process.env.UPLOAD_SCAN_HEARTBEAT_COALESCE_MS;
});

describe("record success/failure — upsert-vorm", () => {
  it("succes zet lastOk=true, teller op 0, driver clamav, op het singleton-kanaal", async () => {
    await recordUploadScanDeliverySuccess(T0);
    expect(upsert).toHaveBeenCalledWith(
      expect.objectContaining({
        where: { channel: UPLOAD_SCAN_CHANNEL },
  ...
  it("mislukking telt de teller atomair op en zet lastOk=false", async () => {
    await recordUploadScanDeliveryFailure(T0);
    expect(upsert).toHaveBeenCalledWith(
      expect.objectContaining({
        update: expect.objectContaining({
          lastOk: false,
...
describe("write-coalescing", () => {
  it("coalesceert opeenvolgende successen binnen het venster (max één schrijf)", async () => {
    await recordUploadScanDeliverySuccess(T0);
    await recordUploadScanDeliverySuccess(at(5_000));
    await recordUploadScanDeliverySuccess(at(14_999));
    expect(upsert).toHaveBeenCalledTimes(1);
  });

  it("schrijft opnieuw zodra het venster verstreken is", async () => {
    await recordUploadScanDeliverySuccess(T0);
    await recordUploadScanDeliverySuccess(at(15_001));
    expect(upsert).toHaveBeenCalledTimes(2);
  });

  it("een mislukking schrijft altijd direct (nooit gecoalesceerd) en een herstel ook", async () => {
    await recordUploadScanDeliverySuccess(T0); // schrijf 1
    await recordUploadScanDeliveryFailure(at(1_000)); // schrijf 2 (direct)
    await recordUploadScanDeliverySuccess(at(2_000)); // schrijf 3 (herstel, direct — binnen venster)
...
describe("fail-open — een DB-storing mag nooit doorbreken", () => {
  it("een upsert-fout wordt geslikt en via de logger gemeld", async () => {
    ...
    await expect(recordUploadScanDeliverySuccess(T0)).resolves.toBeUndefined();
    expect(loggerError).toHaveBeenCalledWith(
      "upload-scan-delivery-heartbeat",
  ...
  it("een leesfout geeft 'never' terug i.p.v. te werpen", async () => {
    ...
    const f = await getUploadScanDeliveryFreshness(T0);
  ...
  it("na een mislukte success-schrijf coalesceert de volgende success niet weg", async () => {
    ...
    await recordUploadScanDeliverySuccess(T0);
    await recordUploadScanDeliverySuccess(at(1_000)); // moet opnieuw proberen (binnen venster)
    expect(upsert).toHaveBeenCalledTimes(2);
  });
});
...
describe("freshness-lezing", () => {
  it("leest het singleton-kanaal en beoordeelt failing bij lastOk=false", async () => {
    ...
    const f = await getUploadScanDeliveryFreshness(T0);
    expect(findUnique).toHaveBeenCalledWith({ where: { channel: UPLOAD_SCAN_CHANNEL } });
