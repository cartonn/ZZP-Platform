  ...
  prisma: {
    ...
    collaboration: {
      findUnique: vi.fn().mockResolvedValue({
        ...
        job: { title: "Test opdracht", credentialRequirements: [] },
      }),
    },
...
describe("signContract — DB-niveau idempotentie via dedupeKey", () => {
  beforeEach(() => {
    ...
    mockTransaction.mockImplementation(async (fn: (tx: unknown) => Promise<unknown>) => {
      const fakeTx = {
        domainEvent: {
          create: vi.fn(
            ({ data }: { data: { dedupeKey?: string | null; [k: string]: unknown } }) => {
              const event = { id: "event-1", dedupeKey: data.dedupeKey ?? null, ...data };
              if (data.dedupeKey)
                domainEventStore.set(data.dedupeKey, { id: "event-1", dedupeKey: data.dedupeKey });
  ...
  it("tweede aanroep met dezelfde collaborationId slaat de transactie over (idempotent)", async () => {
    ...
    domainEventStore.set("contract-signed-col-abc", {
      id: "event-1",
