...
async function persistEventAndEffects(
  ...
  refs: { owners: PartyOwners; invoiceId?: string | null; performanceId?: string | null; correlationId?: string | null },
  opts?: { allocate?: { issuerKey: string; year: number; invoiceId: string } },
): Promise<PersistResult> {
  if (eventInput.dedupeKey) {
    const existing = await prisma.domainEvent.findUnique({ where: { dedupeKey: eventInput.dedupeKey } });
  ...
  return prisma.$transaction(async (tx) => {
    ...
    if (opts?.allocate) {
      const { number } = await allocateInvoiceNumber(tx, opts.allocate.issuerKey, opts.allocate.year);
      ...
      for (const sc of effects.statusChanges) {
        if (sc.entity === "Invoice" && sc.id === opts.allocate.invoiceId) {
          sc.set = { ...(sc.set ?? {}), partyInvoiceNumber: number, number: `${opts.allocate.issuerKey}:${number}` };
        }
      }
    }
...
async function assertNotDisputed(collaborationId: string): Promise<void> {
  const col = await prisma.collaboration.findUnique({ where: { id: collaborationId }, select: { disputedAt: true } });
  if (col?.disputedAt) {
    throw new CascadeError("De samenwerking is bevroren wegens een open dispuut. Los het dispuut eerst op.");
  }
}
...
export async function signContract(actor: Actor, collaborationId: string): Promise<void> {
  ...
  await persistEventAndEffects(
    ...
    effects,
    { owners: { FREELANCER: col.freelancer.userId, CLIENT: col.company.userId }, correlationId: collaborationId },
  );
}
...
export async function createPerformance(actor: Actor, input: CreatePerformanceInput): Promise<string> {
  const col = await prisma.collaboration.findUnique({
...
export async function submitPerformance(actor: Actor, performanceId: string): Promise<void> {
  ...
  await persistEventAndEffects(
    { type: "PERFORMANCE_SUBMITTED", actorRole: actor.role, actorId: actor.id, subjectType: "Performance", subjectId: performanceId, correlationId: perf.collaborationId },
    effects,
    { owners: { FREELANCER: perf.freelancerUserId, CLIENT: perf.clientUserId }, correlationId: perf.collaborationId, performanceId },
  );
}
...
export async function approvePerformance(actor: Actor, performanceId: string): Promise<void> {
  ...
  await persistEventAndEffects(
    { type: "PERFORMANCE_APPROVED", actorRole: actor.role, actorId: actor.id, subjectType: "Performance", subjectId: performanceId, correlationId: perf.collaborationId },
    effects,
    { owners: { FREELANCER: perf.freelancerUserId, CLIENT: perf.clientUserId }, correlationId: perf.collaborationId, performanceId },
  );
}
...
export async function rejectPerformance(actor: Actor, performanceId: string, reason: string): Promise<void> {
  const perf = await loadPerformance(performanceId);
  if (actor.role !== "ADMIN" && actor.id !== perf.clientUserId) {
  ...
  await persistEventAndEffects(
    { type: "PERFORMANCE_REJECTED", actorRole: actor.role, actorId: actor.id, subjectType: "Performance", subjectId: performanceId, correlationId: perf.collaborationId },
    effects,
    { owners: { FREELANCER: perf.freelancerUserId, CLIENT: perf.clientUserId }, correlationId: perf.collaborationId, performanceId },
  );
}
...
export async function submitInvoice(actor: Actor, invoiceId: string): Promise<void> {
  ...
  const effects = planInvoiceSubmittedEvent({
    invoice: { id: invoiceId, lifecycleStatus: inv.lifecycleStatus, subtotalCents: inv.subtotalCents, vatCents: inv.vatCents, totalCents: inv.totalCents },
  ...
  await persistEventAndEffects(
    { type: "INVOICE_SUBMITTED", actorRole: actor.role, actorId: actor.id, subjectType: "Invoice", subjectId: invoiceId, correlationId: inv.correlationId },
    effects,
...
export async function approveInvoice(actor: Actor, invoiceId: string): Promise<void> {
  ...
  await persistEventAndEffects(
    { type: "INVOICE_APPROVED", actorRole: actor.role, actorId: actor.id, subjectType: "Invoice", subjectId: invoiceId, correlationId: inv.correlationId },
    effects,
    { owners: { FREELANCER: inv.issuerUserId, CLIENT: inv.counterpartyUserId }, correlationId: inv.correlationId, invoiceId },
  );
}
...
export async function rejectInvoice(actor: Actor, invoiceId: string, reason: string): Promise<void> {
  const inv = await loadCascadeInvoice(invoiceId);
  if (actor.role !== "ADMIN" && actor.id !== inv.counterpartyUserId) {
  ...
  await persistEventAndEffects(
    { type: "INVOICE_REJECTED", actorRole: actor.role, actorId: actor.id, subjectType: "Invoice", subjectId: invoiceId, correlationId: inv.correlationId },
    effects,
    { owners: { FREELANCER: inv.issuerUserId, CLIENT: inv.counterpartyUserId }, correlationId: inv.correlationId, invoiceId },
  );
}
...
export async function confirmPayment(actor: Actor, invoiceId: string): Promise<void> {
  ...
  if (actor.role !== "ADMIN" && actor.id !== inv.issuerUserId && actor.id !== inv.counterpartyUserId) {
    throw new CascadeError("Geen toegang tot deze factuur.");
  }
  if (!inv.collaborationId) throw new CascadeError("Factuur is niet aan een samenwerking gekoppeld.");
  await assertNotDisputed(inv.collaborationId);
  const col = await prisma.collaboration.findUnique({ where: { id: inv.collaborationId }, select: { id: true, status: true } });
  if (!col) throw new CascadeError("Samenwerking niet gevonden.");
...
export async function openDispute(actor: Actor, collaborationId: string, reason: string): Promise<void> {
  if (!reason?.trim()) throw new CascadeError("Een dispuut vereist een toelichting.");
  const col = await prisma.collaboration.findUnique({
    where: { id: collaborationId },
    include: { company: { select: { userId: true } }, freelancer: { select: { userId: true } }, job: { select: { title: true } } },
  });
  if (!col) throw new CascadeError("Samenwerking niet gevonden.");
  assertParty(actor, col.freelancer.userId, col.company.userId);
  if (col.disputedAt) throw new CascadeError("Er is al een open dispuut.");

  const otherUserId = actor.id === col.freelancer.userId ? col.company.userId : col.freelancer.userId;
  const admins = await prisma.user.findMany({ where: { role: "ADMIN", status: "ACTIVE" }, select: { id: true } });

  await prisma.$transaction([
    prisma.collaboration.update({ where: { id: collaborationId }, data: { disputedAt: new Date(), disputeReason: reason.trim() } }),
    prisma.domainEvent.create({
      data: { type: "DISPUTE_OPENED", actorRole: actor.role, actorId: actor.id, subjectType: "Collaboration", subjectId: collaborationId, payload: JSON.stringify({ reason: reason.trim() }), correlationId: collaborationId },
    }),
    prisma.notification.create({ data: { userId: otherUserId, type: "DISPUTE_OPENED", title: "Dispuut geopend", body: `Er is een dispuut geopend voor "${col.job.title}". De cascade is bevroren tot het is opgelost.`, link: `/samenwerkingen/${collaborationId}` } }),
    ...admins.map((a) => prisma.notification.create({ data: { userId: a.id, type: "DISPUTE_OPENED", title: "Dispuut — bemiddeling nodig", body: `Dispuut bij "${col.job.title}": ${reason.trim()}`, link: `/samenwerkingen/${collaborationId}` } })),
    prisma.auditLog.create({ data: auditData({ actorId: actor.id, action: "DISPUTE_OPENED", entityType: "Collaboration", entityId: collaborationId, metadata: { reason: reason.trim() } }) }),
  ]);
}
...
export async function resolveDispute(actor: Actor, collaborationId: string): Promise<void> {
  if (actor.role !== "ADMIN") throw new CascadeError("Alleen het platform kan een dispuut oplossen.");
  const col = await prisma.collaboration.findUnique({
    where: { id: collaborationId },
  ...
  await prisma.$transaction([
    prisma.collaboration.update({ where: { id: collaborationId }, data: { disputedAt: null, disputeReason: null } }),
    prisma.domainEvent.create({
      data: { type: "DISPUTE_RESOLVED", actorRole: actor.role, actorId: actor.id, subjectType: "Collaboration", subjectId: collaborationId, payload: "{}", correlationId: collaborationId },
    }),
    prisma.notification.create({ data: { userId: col.freelancer.userId, type: "DISPUTE_RESOLVED", title: "Dispuut opgelost", body: "Het dispuut is opgelost; je kunt het werkproces hervatten.", link: `/samenwerkingen/${collaborationId}` } }),
    prisma.notification.create({ data: { userId: col.company.userId, type: "DISPUTE_RESOLVED", title: "Dispuut opgelost", body: "Het dispuut is opgelost; je kunt het werkproces hervatten.", link: `/samenwerkingen/${collaborationId}` } }),
    prisma.auditLog.create({ data: auditData({ actorId: actor.id, action: "DISPUTE_RESOLVED", entityType: "Collaboration", entityId: collaborationId }) }),
  ]);
}
...
async function loadCascadeInvoice(invoiceId: string): Promise<LoadedInvoice> {
  ...
  if (
    !inv.lifecycleStatus || inv.issuerUserId == null || inv.counterpartyUserId == null ||
    inv.issuerKey == null || inv.subtotalCents == null || inv.vatCents == null
  ) {
