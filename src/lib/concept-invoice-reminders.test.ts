...
import { planConceptInvoiceReminders, daysSince, type ConceptInvoiceCandidate } from "@/lib/concept-invoice-reminders";

const now = new Date("2026-05-29T12:00:00Z");

function cand(overrides: Partial<ConceptInvoiceCandidate> & Pick<ConceptInvoiceCandidate, "invoiceId">): ConceptInvoiceCandidate {
  return {
...
describe("planConceptInvoiceReminders", () => {
  ...
  it("herinnert niet op een tussenliggende dag", () => {
    const plan = planConceptInvoiceReminders([cand({ invoiceId: "i5", createdAt: daysAgo(5) })], now);
    expect(plan.reminders).toHaveLength(0);
    expect(plan.escalations).toHaveLength(0);
  });

  it("escaleert naar het platform na de laatste herinnering (>7 dagen)", () => {
    const plan = planConceptInvoiceReminders([cand({ invoiceId: "iLate", createdAt: daysAgo(9) })], now);
